...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/TunnelsAndTrolls.toml");
    ...
    /// `test/data/TunnelsAndTrolls.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/TunnelsAndTrolls.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("TunnelsAndTrolls.toml must parse");
        assert_eq!(
            data.tests.len(),
            141,
            "case count in test/data/TunnelsAndTrolls.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "TunnelsAndTrolls",
                "unexpected game system in TunnelsAndTrolls.toml"
            );
            ...
            match eval_command(&GameSystemId::new("TunnelsAndTrolls"), &tc.input, &mut src) {
