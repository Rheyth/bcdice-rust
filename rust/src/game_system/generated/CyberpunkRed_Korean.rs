...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/CyberpunkRed_Korean.toml");
    ...
    /// `test/data/CyberpunkRed_Korean.toml` の全ケースが通ること。
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/CyberpunkRed_Korean.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("CyberpunkRed_Korean.toml must parse");
        assert_eq!(
            data.tests.len(),
            56,
            "case count in test/data/CyberpunkRed_Korean.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "CyberpunkRed:Korean",
                "unexpected game system in CyberpunkRed_Korean.toml"
            );
            ...
            match eval_command(
                &GameSystemId::new("CyberpunkRed:Korean"),
                &tc.input,
                &mut src,
            ) {
        ...
        assert!(
            failures.is_empty(),
            "{}/{} CyberpunkRed_Korean cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
