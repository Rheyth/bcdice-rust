//! P4で手書き移植した `lib/bcdice/game_system/Amadeus.rb`。
//!
...
//!
//! 移植したもの:
//! - `Amadeus#roll_amadeus`（判定 `Rx±y@z>=t`）
//! - `Amadeus::TABLES` と `Base#roll_tables`（各種表）
//!
//! 表と定型文は Ruby が `I18n` から組み立てる（`Table.from_i18n` /
//! `D66Table.from_i18n` / `translate`）。ここでは `ja_jp` ロケールの値を
//! `i18n/Amadeus/ja_jp.yml` と `i18n/ja_jp.yml` から写して `static` に展開した。
//! `Amadeus_Korean` が `ko_kr` の値を差し替えられるよう、判定と表引きは
...
/// 1ロケール分の表と定型文。`Amadeus` と `Amadeus_Korean` はこれだけが違う。
pub(crate) struct SystemTables {
    ...
    /// i18n `Amadeus.inga_table`（出目1〜6に対応するインガ）
    pub(crate) inga_table: &'static [&'static str],
    /// i18n `Amadeus.special`
...
/// Ruby `Amadeus#eval_game_system_specific_command`。
pub(crate) fn eval_specific_command(
    sys: &SystemTables,
    command: &str,
    rng: &mut Randomizer,
) -> Result<Option<SpecificCommandOutput>, EvalError> {
    if let Some(result) = roll_amadeus(sys, command, rng)? {
        return Ok(Some(SpecificCommandOutput::result(result)));
    }
    Ok(table_helpers::roll_table(command, sys.tables, rng)?.map(SpecificCommandOutput::text))
}

/// Ruby `roll_amadeus` の判定コマンド。
///
/// Ruby: `/^R([A-DS])([+\-\d]*)(@(\d))?((>=?)([+\-\d]*))?(@(\d))?$/i`
/// スペシャル最低値 `@z` は比較演算子の前後どちらにも書ける（`m[4] || m[9]`）。
fn amadeus_pattern() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| {
        Regex::new(r"(?i)^R([A-DS])([+\-\d]*)(@(\d))?((>=?)([+\-\d]*))?(@(\d))?$")
            .expect("valid regex")
    })
}

/// Ruby `CHECK_DICE_COUNT`。能力ランクごとの判定ダイス数。
fn check_dice_count(rank: u8) -> i64 {
    match rank {
        b'S' => 4,
        b'A' => 3,
        b'B' => 2,
        b'C' => 1,
        // b'D'。正規表現が `[A-DS]` に限っているので他の値は来ない。
        _ => 2,
    }
}

/// Ruby `Amadeus#roll_amadeus`。
fn roll_amadeus(
    sys: &SystemTables,
    command: &str,
    rng: &mut Randomizer,
) -> Result<Option<EvalResult>, EvalError> {
    let Some(m) = amadeus_pattern().captures(command) else {
        return Ok(None);
    };

    // `@enabled_upcase_input` が真なのでここに来る時点で大文字だが、
    // Ruby の `/i` に合わせて明示的に揃える。
    let rank = m[1].to_ascii_uppercase();
    let rank_byte = rank.as_bytes()[0];

    // Ruby: `ArithmeticEvaluator.eval` は不正な式（空文字列を含む）に 0 を返す。
    let modifier_value = arithmetic::eval(&m[2], RoundType::Floor)?
        .as_ref()
        .map(crate::randomizer::sat_i64)
        .unwrap_or(0);

    // Ruby: `m[6] ? Normalize.comparison_operator(m[6]) : :>=`
    // m[6] は `>` か `>=` なので必ず解決する。
    let cmp_op = match m.get(6) {
        Some(op) => normalize::comparison_operator(op.as_str()).unwrap_or(CmpOp::Ge),
        None => CmpOp::Ge,
    };

    // Ruby: `m[7] ? ArithmeticEvaluator.eval(m[7]) : 4`
    // 目標値が空欄（`RC>=`）のときは Ruby でも空文字列が truthy なので 0 になる。
    let target = match m.get(7) {
        Some(t) => arithmetic::eval(t.as_str(), RoundType::Floor)?
            .as_ref()
            .map(crate::randomizer::sat_i64)
            .unwrap_or(0),
        None => 4,
    ...
    let dice_list = rng.roll_barabara(check_dice_count(rank_byte), 6)?;
    let dice_text = join_comma(&dice_list);
    let special_text = if special == 6 {
        String::new()
    } else {
        format!("@{special}")
    };

    // Ruby: `dice_list = [dice_list.min] if rank == "D"`
    // `dice_text` はこの絞り込みより前に作るので、Dランクでも2個とも表示される。
    let judged: Vec<i64> = if rank_byte == b'D' {
        dice_list.iter().min().copied().into_iter().collect()
    } else {
        dice_list
    };
    let available_inga = judged.len() > 1;

    let mut success = false;
    let mut critical = false;
    let mut fumble = false;

    let mut results: Vec<String> = Vec::with_capacity(judged.len());
    for dice in judged {
        let total = dice + modifier_value;
        let result = if dice == 1 {
            ...
            success = true;
            sys.special
        } else if cmp_op.apply(&I::from(total), &I::from(target)) {
            success = true;
            sys.success
        } else {
            sys.failure
        ...
        if available_inga {
            // Ruby: `inga_table[dice - 1]`。出目は1〜6なので必ず範囲内。
            let inga = usize::try_from(dice - 1)
                .ok()
                .and_then(|i| sys.inga_table.get(i))
                .copied()
                .unwrap_or("");
            results.push(format!("{total}_{result}[{dice}{inga}]"));
        } else {
            results.push(format!("{total}_{result}[{dice}]"));
        }
    }

    let modifier_text = modifier(&crate::Int::from(modifier_value));
    let sequence = [
        format!(
            "(R{rank}{modifier_text}{special_text}{}{target})",
            cmp_op.symbol_str()
        ),
        format!("[{dice_text}]{modifier_text}"),
        results.join(" / "),
    ];
...
/// Ruby `dice_list.join(",")`。
fn join_comma(dice_list: &[i64]) -> String {
    dice_list
        .iter()
        .map(|d| d.to_string())
        .collect::<Vec<_>>()
...
/// i18n `ja_jp.success`。
const JA_SUCCESS: &str = "成功";
/// i18n `ja_jp.failure`。
const JA_FAILURE: &str = "失敗";

/// i18n `Amadeus.table.ECT`（ja_jp）。
pub(crate) static JA_TABLE_ECT: Table = Table::from_dice(
    "境遇表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.BST`（ja_jp）。
pub(crate) static JA_TABLE_BST: Table = Table::from_dice(
    "戦場表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.RT`（ja_jp）。
pub(crate) static JA_TABLE_RT: Table = Table::from_dice(
    "関係表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.PRT`（ja_jp）。
pub(crate) static JA_TABLE_PRT: Table = Table::from_dice(
    "親心表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.FT`（ja_jp）。
pub(crate) static JA_TABLE_FT: Table = Table::from_dice(
    "ファンブル表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.BT`（ja_jp）。
pub(crate) static JA_TABLE_BT: D66Table = D66Table::new(
    "休憩表",
    D66SortType::Asc,
    &[
        (11, TableItem::Text("土着の怪物が襲いかかってきた！　なんとか撃退するが、傷を負ってしまった。1D6点のダメージを受ける。")),
        (12, TableItem::Text("美女の沐浴をのぞいてしまった。1D6を振る。1～2なら「堕落」、3～4なら「恥辱」、5～6なら「重傷1」の変調を受ける。")),
        (13, TableItem::Text("強欲な商人に出会う。このシーンに登場したキャラクターは、アイテムを購入することができる。ただし、すべてのアイテムの価格は通常の値段より1高い。")),
        (14, TableItem::Text("自分の過去の話をする。なんで、こんな話しちゃったんだろう？　PCの中から、自分に対してもっとも高い【想い】を持つPC全員の、関係の属性が反転する。")),
        (15, TableItem::Text("周囲の空気が変わった。運命の輪が動き出す予感！運命の輪の上にある赤の領域のインガを青の領域に、青の領域のインガを緑の領域に、緑の領域のインガを白の領域に、白の領域のインガを赤の領域に、それぞれ同時に移動させる。")),
        (16, TableItem::Text("突然の空腹に襲われる。このシーン中に食事を行わなかったPCは「重傷2」の変調を受ける。この変調は、食事を行うと回復できる（ほかの方法でも回復可能）。")),
        (22, TableItem::Text("＜絶界＞の外の世界の友人のことを思い出す。みんなは元気にしているだろうか？　この出目を振ったプレイヤーのPCは、【日常】で判定を行うことができる。成功すると、自分の変調を1つ回復するか、黒の領域からインガを1つ取り除くことができる。")),
        (23, TableItem::Text("奇妙な商人に出会う。このシーンに登場したキャラクターは、アイテムを購入することができる。")),
        (24, TableItem::Text("喋る動物に願いごとをされる。動物たちも、この神話災害で苦しんでいるようだ。あなたは、「この怪物の本体の【生命力】を0にする」という追加の【任務】を受けることができる。この【任務】を達成すると、追加で経験値を20点もらうことができる。")),
        (25, TableItem::Text("素敵な夢を見る。この出目を振ったプレイヤーは、自分のPC以外の好きなキャラクター一人に対する【想い】が1点上昇する。")),
        (26, TableItem::Text("大切な人から、あなたを心配するメールが届いていた。なんて返そう？　この出目を振ったプレイヤーのPCは、【日常】で判定を行うことができる。成功すると、自分の変調を1つ回復するか、自分の好きなパトスマークについたチェックを1つ消す。")),
        (33, TableItem::Text("親切な商人に出会う。このシーンに登場したキャラクターは、アイテムを購入することができる。ただし、すべてのアイテムの価格は通常の値段より1低い（価格1未満にはならない）。")),
        (34, TableItem::Text("武器の手入れを行う。この出目を振ったプレイヤーのPCは、【日常】で判定を行うことができる。成功すると、そのセッションの間、自分の武器1つの威力を1点上昇させることができる。")),
        (35, TableItem::Text("最後に買い物をしたときに、おつりを多くもらっていたことに気がついた。1神貨を手に入れる。")),
        (36, TableItem::Text("食事をしながら、仲間と大いに語り合う。このシーンに登場して、食事を行ったキャラクターは、自分の好きなパトスマークについたチェックを一つ消す。")),
        (44, TableItem::Text("一眠りしてしまったのか、不思議な夢を見る。この出目を振ったプレイヤーのPCは、【霊力】で判定を行うことができる。成功すると、好きな予言カード1枚を選び、その【真実】を見ることができる。")),
        (45, TableItem::Text("チンピラにからまれている異性の子を見つける。この出目を振ったプレイヤーのPCは、【武勇】で判定を行うことができる。成功すると、その子は、そのPCに対して【想い】を1点獲得する。この子を協力者にするなら、そのプレイヤーはその子の名前と関係を自由に決めること。")),
        (46, TableItem::Text("心地よい風に吹かれる。運命があなたに味方してくれるような気がした。好きな領域にインガを1つ配置する。")),
        (55, TableItem::Text("目が覚めると、枕元に贈り物が。誰からだろう……？　この出目を振ったプレイヤーのPCは、「ランダムアイテム表」を使用する。")),
        (56, TableItem::Text("困っている神話生物を助けてあげた。【日常】で判定を行う。成功すると、次に移動判定を行うことになったとき、自動的にそれを成功にすることができる（達成値が必要な場合6として扱う）。")),
        (66, TableItem::Text("親神が、あなたに話しかけてくる。親子の会話だ。この出目を振ったプレイヤーのPCは、【日常】で判定を行うことができる。成功すると、自分の親神に対する【想い】か、親神の自分に対する【想い】のいずれかを1点上昇する。")),
    ],
);

/// i18n `Amadeus.table.FWT`（ja_jp）。
pub(crate) static JA_TABLE_FWT: Table = Table::from_dice(
    "致命傷表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.BRT`（ja_jp）。
pub(crate) static JA_TABLE_BRT: Table = Table::from_dice(
    "戦果表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.RIT`（ja_jp）。
pub(crate) static JA_TABLE_RIT: Table = Table::from_dice(
    "ランダムアイテム表",
    2,
    6,
    &[
...
/// i18n `Amadeus.table.WT`（ja_jp）。
pub(crate) static JA_TABLE_WT: Table = Table::from_dice(
    "損傷表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.NMT`（ja_jp）。
pub(crate) static JA_TABLE_NMT: Table = Table::from_dice(
    "悪夢表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.TGT`（ja_jp）。
pub(crate) static JA_TABLE_TGT: Table = Table::from_dice(
    "目標表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.CST`（ja_jp）。
pub(crate) static JA_TABLE_CST: Table = Table::from_dice(
    "制約表",
    1,
    6,
    &["短命", "誘惑", "悪影響", "束縛", "喧嘩", "干渉"],
...
/// i18n `Amadeus.table.GCVT`（ja_jp）。
pub(crate) static JA_TABLE_GCVT: Table = Table::from_dice(
    "ギリシャ神群試練表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.YCVT`（ja_jp）。
pub(crate) static JA_TABLE_YCVT: Table = Table::from_dice(
    "ヤマト神群試練表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.ECVT`（ja_jp）。
pub(crate) static JA_TABLE_ECVT: Table = Table::from_dice(
    "エジプト神群試練表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.CCVT`（ja_jp）。
pub(crate) static JA_TABLE_CCVT: Table = Table::from_dice(
    "クトゥルフ神群試練表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.NCVT`（ja_jp）。
pub(crate) static JA_TABLE_NCVT: Table = Table::from_dice(
    "北欧神群試練表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.DGVT`（ja_jp）。
pub(crate) static JA_TABLE_DGVT: Table = Table::from_dice(
    "ダンジョン試練表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.DAVT`（ja_jp）。
pub(crate) static JA_TABLE_DAVT: Table = Table::from_dice(
    "日常試練表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.PRCT`（ja_jp）。
pub(crate) static JA_TABLE_PRCT: Table = Table::from_dice(
    "挑戦テーマ表【武勇】",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.TCCT`（ja_jp）。
pub(crate) static JA_TABLE_TCCT: Table = Table::from_dice(
    "挑戦テーマ表【技術】",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.INCT`（ja_jp）。
pub(crate) static JA_TABLE_INCT: Table = Table::from_dice(
    "挑戦テーマ表【頭脳】",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.PSCT`（ja_jp）。
pub(crate) static JA_TABLE_PSCT: Table = Table::from_dice(
    "挑戦テーマ表【霊力】",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.LVCT`（ja_jp）。
pub(crate) static JA_TABLE_LVCT: Table = Table::from_dice(
    "挑戦テーマ表【愛】",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.DACT`（ja_jp）。
pub(crate) static JA_TABLE_DACT: Table = Table::from_dice(
    "挑戦テーマ表【日常】",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.RGT`（ja_jp）。
pub(crate) static JA_TABLE_RGT: Table = Table::from_dice(
    "ランダムギフト表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.FBT`（ja_jp）。
pub(crate) static JA_TABLE_FBT: Table = Table::from_dice(
    "決戦戦果表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.CHVT`（ja_jp）。
pub(crate) static JA_TABLE_CHVT: Table = Table::from_dice(
    "中華神群試練表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.LCVT`（ja_jp）。
pub(crate) static JA_TABLE_LCVT: Table = Table::from_dice(
    "ラストクロニクル神群試練表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.KCVT`（ja_jp）。
pub(crate) static JA_TABLE_KCVT: Table = Table::from_dice(
    "ケルト神群試練表",
    1,
    6,
    &[
...
/// i18n `Amadeus.table.SAT`（ja_jp）。
pub(crate) static JA_TABLE_SAT: D66Table = D66Table::new(
    "店内雰囲気表",
    D66SortType::Asc,
    &[
        (11, TableItem::Text("山の中にある峠の茶屋")),
        (
            12,
            TableItem::Text("煙草の煙がたゆたうセルフ式コーヒーチェーン店"),
        ),
        (13, TableItem::Text("山上りの座卓がある和風茶寮")),
        (
            14,
            TableItem::Text("ロココ調の優美で華やかな雰囲気の高級サロン"),
        ),
        (
            15,
            TableItem::Text("ヴィクトリア調のシックで上品なティーハウス"),
        ),
        (
            16,
            TableItem::Text("古い歌謡曲がかかり、テーブル筐体が懐かしい純喫茶"),
        ),
        (
            22,
            TableItem::Text("若い子で賑わうポップな内装のいかにもなチェーン店"),
        ),
        (
            23,
            TableItem::Text("ミッドセンチュリーの家具がお洒落なカフェ"),
        ),
        (
            24,
            TableItem::Text("ジュークボックスが目立つアメリカンダイナー"),
        ),
        (
            25,
            TableItem::Text("ピアノ曲が静かに流れる落ち着いた喫茶店"),
        ),
        (
            26,
            TableItem::Text("ノマドな若者が目立つコーヒーチェーン店"),
        ),
        (
            33,
            TableItem::Text("かわいい給仕さんがたくさんいるメイドカフェ"),
        ),
        (
            34,
            TableItem::Text("独特の茶器でゆったりお茶を楽しめる中国茶館"),
        ),
        (
            35,
            TableItem::Text("アースカラーが懐かしい健康志向なナチュラルカフェ"),
        ),
        (
            36,
            TableItem::Text("見晴らしのいいカフェテラスが自慢のオープンカフェ"),
        ),
        (
            44,
            TableItem::Text("壁一面にレコードが飾られているジャズ喫茶"),
        ),
        (45, TableItem::Text("24時間営業で荒れた雰囲気の漫画喫茶")),
        (
            46,
            TableItem::Text("色々なアナログゲームが楽しめるゲームカフェ"),
        ),
        (
            55,
            TableItem::Text("ショッピングモールのフードコート内にある出店"),
        ),
        (
            56,
            TableItem::Text("ビジネスマンたちが商談にいそしむ談話室"),
        ),
        (
            66,
            TableItem::Text("水タバコを吸いながら会話に興じる中東風カフェ"),
        ),
    ],
);

/// i18n `Amadeus.table.SMT`（ja_jp）。
pub(crate) static JA_TABLE_SMT: D66Table = D66Table::new(
    "特殊メニュー表",
    D66SortType::Asc,
    &[
        (11, TableItem::Text("アド・パトレス（ケルト）")),
        (12, TableItem::Text("アムブロシア（ギリシア）")),
        (13, TableItem::Text("ネクタル（ギリシア）")),
        (14, TableItem::Text("アムリタ（インド）")),
        (15, TableItem::Text("ソーマ（インド）")),
        (16, TableItem::Text("ヤシオリ（ヤマト）")),
        (22, TableItem::Text("変若水／おちみず（ヤマト）")),
        (23, TableItem::Text("天舐酒／あまのたむけざけ（ヤマト）")),
        (24, TableItem::Text("エリクサー（錬金術）")),
        (25, TableItem::Text("ハオマ（ペルシア）")),
        (26, TableItem::Text("金丹（中華）")),
        (33, TableItem::Text("神農茶（中華）")),
        (34, TableItem::Text("チョコラトル（アステカ）")),
        (35, TableItem::Text("クヴァシル（北欧）")),
        (36, TableItem::Text("ヘイズルーンの乳（北欧）")),
        (44, TableItem::Text("ホワイト・ドロップ（エジプト）")),
        (45, TableItem::Text("リキッド・ゴールド（エジプト）")),
        (46, TableItem::Text("ジヴァヤ・ヴォジャ（スラブ）")),
        (55, TableItem::Text("スハルジク（メソポタミア）")),
        (56, TableItem::Text("ニンフサグのミルク（メソポタミア）")),
        (66, TableItem::Text("黄金の蜂蜜酒（クトゥルフ）")),
    ],
);
...
/// i18n `Amadeus.inga_table`（ja_jp）。
static JA_INGA_TABLE: &[&str] = &["黒", "赤", "青", "緑", "白", "任意"];
/// i18n `Amadeus.special`（ja_jp）。
const JA_SPECIAL: &str = "スペシャル！";
/// i18n `Amadeus.fumble`（ja_jp）。
const JA_FUMBLE: &str = "ファンブル！";

/// `ja_jp` ロケールの表と定型文一式。
static JA_SYSTEM: SystemTables = SystemTables {
    tables: JA_TABLES,
    inga_table: JA_INGA_TABLE,
    special: JA_SPECIAL,
    fumble: JA_FUMBLE,
    success: JA_SUCCESS,
    failure: JA_FAILURE,
...
/// Ruby `BCDice::GameSystem::Amadeus`（ID: `Amadeus`）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Amadeus;

impl GameSystem for Amadeus {
...
mod tests {
    ...
    /// `test/data/Amadeus.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Amadeus.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Amadeus.toml must parse");
        assert_eq!(data.tests.len(), 87, "case count in test/data/Amadeus.toml");

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Amadeus",
                "unexpected game system in Amadeus.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Amadeus"), &tc.input, &mut src) {
    ...
    ///
    /// `test/data/Amadeus.toml` に `secret = true` のケースが1件も無いので、
    /// ここで押さえておく。`roll_amadeus` は `Result` を返す唯一の経路なので、
    /// `Base#dice_command` が `Result#secret` を立てるところまで通る。
    /// 表コマンド（文字列を返す経路）も併せて見る。
    #[test]
    fn secret_prefix_is_stripped_and_propagated() {
        fn eval(input: &str, rands: Vec<(i64, i64)>) -> crate::result::EvalResult {
            let mut src = SeededRandomizer::new(rands);
            let out = eval_command(&GameSystemId::new("Amadeus"), input, &mut src)
                .expect("must not error")
                .unwrap_or_else(|| panic!("{input} must produce output"));
            assert!(src.is_empty(), "unconsumed rands for {input}");
            out
        }

        let plain = eval("RC>=5", vec![(5, 6)]);
        assert!(!plain.secret, "RC>=5 は secret ではない");

        let secret = eval("SRC>=5", vec![(5, 6)]);
        assert!(secret.secret, "SRC>=5 は secret になる");
        assert_eq!(secret.text, plain.text);
        assert!(secret.success);

        // 表コマンド（`SpecificCommandOutput::Text` を返す経路）
        let secret_table = eval("SRT", vec![(1, 6)]);
        assert!(secret_table.secret, "SRT は secret になる");
        assert_eq!(
            secret_table.text,
            "関係表(1) ＞ 恋心（プラス）／殺意（マイナス）"
        );
    }
}
