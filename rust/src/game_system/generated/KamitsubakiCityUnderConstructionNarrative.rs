...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/KamitsubakiCityUnderConstructionNarrative.toml");
    ...
    /// `test/data/KamitsubakiCityUnderConstructionNarrative.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/KamitsubakiCityUnderConstructionNarrative.toml not found");
            return;
        };

        let data = TestDataFile::load(&path)
            .expect("KamitsubakiCityUnderConstructionNarrative.toml must parse");
        assert_eq!(
            data.tests.len(),
            113,
            "case count in test/data/KamitsubakiCityUnderConstructionNarrative.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "KamitsubakiCityUnderConstructionNarrative",
                "unexpected game system in KamitsubakiCityUnderConstructionNarrative.toml"
            );
            ...
            match eval_command(
                &GameSystemId::new("KamitsubakiCityUnderConstructionNarrative"),
                &tc.input,
                &mut src,
            ) {
            ...
            if !reasons.is_empty() {
                failures.push(format!(
                    "FAIL KamitsubakiCityUnderConstructionNarrative:{}:{}\n  - {}",
                    i + 1,
                    tc.input,
                    reasons.join("\n  - ")
                ));
        ...
        assert!(
            failures.is_empty(),
            "{}/{} KamitsubakiCityUnderConstructionNarrative cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
