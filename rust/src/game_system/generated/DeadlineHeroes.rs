...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/DeadlineHeroes.toml");
    ...
    /// `test/data/DeadlineHeroes.toml` の全ケースが通ること。
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/DeadlineHeroes.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("DeadlineHeroes.toml must parse");
        assert_eq!(
            data.tests.len(),
            113,
            "case count in test/data/DeadlineHeroes.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "DeadlineHeroes",
                "unexpected game system in DeadlineHeroes.toml"
            );
            ...
            match eval_command(&GameSystemId::new("DeadlineHeroes"), &tc.input, &mut src) {
