...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/HatsuneMiku.toml");
    ...
    /// `test/data/HatsuneMiku.toml` の全ケースが通ること。
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/HatsuneMiku.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("HatsuneMiku.toml must parse");
        assert_eq!(
            data.tests.len(),
            57,
            "case count in test/data/HatsuneMiku.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "HatsuneMiku",
                "unexpected game system in HatsuneMiku.toml"
            );
            ...
            match eval_command(&GameSystemId::new("HatsuneMiku"), &tc.input, &mut src) {
