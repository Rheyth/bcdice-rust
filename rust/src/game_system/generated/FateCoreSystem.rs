...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/FateCoreSystem.toml");
    ...
    /// `test/data/FateCoreSystem.toml` の全ケースが通ること。
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            // worktree外でクレート単体ビルドされた場合
            eprintln!("skip: test/data/FateCoreSystem.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("FateCoreSystem.toml must parse");
        assert_eq!(
            data.tests.len(),
            22,
            "case count in test/data/FateCoreSystem.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "FateCoreSystem",
                "unexpected game system in FateCoreSystem.toml"
            );
            ...
            match eval_command(&GameSystemId::new("FateCoreSystem"), &tc.input, &mut src) {
