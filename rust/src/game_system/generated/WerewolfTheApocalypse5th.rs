...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/WerewolfTheApocalypse5th.toml");
    ...
    /// `test/data/WerewolfTheApocalypse5th.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/WerewolfTheApocalypse5th.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("WerewolfTheApocalypse5th.toml must parse");
        assert_eq!(
            data.tests.len(),
            200,
            "case count in test/data/WerewolfTheApocalypse5th.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "WerewolfTheApocalypse5th",
                "unexpected game system in WerewolfTheApocalypse5th.toml"
            );
            ...
            match eval_command(
                &GameSystemId::new("WerewolfTheApocalypse5th"),
                &tc.input,
                &mut src,
            ) {
        ...
        assert!(
            failures.is_empty(),
            "{}/{} WerewolfTheApocalypse5th cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
