//! P4で手書き移植した `lib/bcdice/game_system/KurayamiCrying.rb`。
//!
...
//!
//! 移植したもの:
//! - `KurayamiCrying#eval_game_system_specific_command`
//!   （`ACTn` による項目直接指定と、`TABLES` の `roll_tables`）
//! - `TABLES`（`ACT` / `EMO` / `SST` / `SRT` / `SAE1`〜`SAE5`）
//!
//! # 原典との差異
//!
//! `ACT0` のように添字が負になる指定は、Ruby では `Array#[]` の回り込みで
//! 末尾の項目が選ばれる。[`Table::choice`] は回り込みを再現せず該当なし（空文字列）にする
//! （`dice_table` モジュールの方針。TOMLに該当ケースはない）。
...
pub struct KurayamiCrying;

impl GameSystem for KurayamiCrying {
...
"
    ...
    fn prefixes(&self) -> &'static [&'static str] {
        &[
            "ACT", "EMO", "SST", "SRT", "SAE1", "SAE2", "SAE3", "SAE4", "SAE5",
        ]
    }

    crate::impl_prefixes_pattern!();

    /// Ruby `KurayamiCrying#eval_game_system_specific_command`。
    fn eval_game_system_specific_command(
        &self,
        command: &str,
        rng: &mut Randomizer,
    ) -> Result<Option<SpecificCommandOutput>, EvalError> {
        if let Some(m) = act_pattern().captures(command) {
            // Ruby: m[1].to_i。i64 に収まらない指定は該当なしに落ちる。
            let number: i64 = m[1].parse().unwrap_or(i64::MAX);
            return Ok(Some(SpecificCommandOutput::text(
                ACT_TABLE.choice(number).to_string(),
            )));
        }

        // Ruby: else -> roll_tables(command, TABLES)
        roll_tables(command, rng)
    }
}

/// Ruby `/^ACT(\d+)$/i`。
fn act_pattern() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r"(?i)^ACT(\d+)$").expect("valid regex"))
}
...
/// Ruby `TABLES["ACT"]`（`1D10`）。
static ACT_TABLE: Table = Table::from_dice("アクシデント表", 1, 10, ACT_ITEMS);

/// Ruby `TABLES["EMO"]`（`1D10`）の項目。
static EMO_ITEMS: &[&str] = &[
    "(正)信頼／(負)依存",
    "(正)安心／(負)不安",
    "(正)友情／(負)憤怒",
    "(正)尊敬／(負)恐怖",
...
/// Ruby `TABLES["EMO"]`（`1D10`）。
static EMO_TABLE: Table = Table::from_dice("感情表", 1, 10, EMO_ITEMS);

/// Ruby `TABLES["SST"]`（`1D10`）の項目。
static SST_ITEMS: &[&str] = &[
...
/// Ruby `TABLES["SST"]`（`1D10`）。
static SST_TABLE: Table = Table::from_dice("幕間シーン表（情景）", 1, 10, SST_ITEMS);

/// Ruby `TABLES["SRT"]`（`1D10`）の項目。
static SRT_ITEMS: &[&str] = &[
...
/// Ruby `TABLES["SRT"]`（`1D10`）。
static SRT_TABLE: Table = Table::from_dice("幕間シーン表（関係）", 1, 10, SRT_ITEMS);

/// Ruby `TABLES["SAE1"]`（`1D10`）の項目。
static SAE1_ITEMS: &[&str] = &[
...
/// Ruby `TABLES["SAE1"]`（`1D10`）。
static SAE1_TABLE: Table =
    Table::from_dice("大事故イベント表1　日常と正気の破壊", 1, 10, SAE1_ITEMS);

/// Ruby `TABLES["SAE2"]`（`1D10`）の項目。
static SAE2_ITEMS: &[&str] = &[
...
/// Ruby `TABLES["SAE2"]`（`1D10`）。
static SAE2_TABLE: Table =
    Table::from_dice("大事故イベント表2　アメリカンB級ホラー", 1, 10, SAE2_ITEMS);

/// Ruby `TABLES["SAE3"]`（`1D10`）の項目。
static SAE3_ITEMS: &[&str] = &[
...
/// Ruby `TABLES["SAE3"]`（`1D10`）。
static SAE3_TABLE: Table = Table::from_dice("大事故イベント表3　牙をむく異世界", 1, 10, SAE3_ITEMS);

/// Ruby `TABLES["SAE4"]`（`1D10`）の項目。
static SAE4_ITEMS: &[&str] = &[
...
/// Ruby `TABLES["SAE4"]`（`1D10`）。
static SAE4_TABLE: Table = Table::from_dice("大事故イベント表4　悪意のおとぎ話", 1, 10, SAE4_ITEMS);

/// Ruby `TABLES["SAE5"]`（`1D10`）の項目。
static SAE5_ITEMS: &[&str] = &[
...
/// Ruby `TABLES["SAE5"]`（`1D10`）。
static SAE5_TABLE: Table = Table::from_dice("大事故イベント表5　悲劇の運命", 1, 10, SAE5_ITEMS);
...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/KurayamiCrying.toml");
    ...
    /// `test/data/KurayamiCrying.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/KurayamiCrying.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("KurayamiCrying.toml must parse");
        assert_eq!(
            data.tests.len(),
            19,
            "case count in test/data/KurayamiCrying.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "KurayamiCrying",
                "unexpected game system in KurayamiCrying.toml"
            );
            ...
            match eval_command(&GameSystemId::new("KurayamiCrying"), &tc.input, &mut src) {
