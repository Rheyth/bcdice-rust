...
mod tests {
    ...
    /// `test/data/Avandner.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Avandner.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Avandner.toml must parse");
        assert_eq!(
            data.tests.len(),
            21,
            "case count in test/data/Avandner.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Avandner",
                "unexpected game system in Avandner.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Avandner"), &tc.input, &mut src) {
