...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/BlindMythos.toml");
        path.exists().then_some(path)
    }
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else { return };
        let data = TestDataFile::load(&path).expect("BlindMythos.toml must parse");
        assert_eq!(
            data.tests.len(),
            39,
            "case count in test/data/BlindMythos.toml"
        );
        let mut failures = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(tc.game_system, "BlindMythos");
            let mut src = SeededRandomizer::new(tc.rands.iter().map(|r| (r.value, r.sides)));
            match eval_command(&GameSystemId::new("BlindMythos"), &tc.input, &mut src) {
