...
mod tests {
    ...
    /// `test/data/Torg.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Torg.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Torg.toml must parse");
        assert_eq!(data.tests.len(), 117, "case count in test/data/Torg.toml");

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Torg",
                "unexpected game system in Torg.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Torg"), &tc.input, &mut src) {
