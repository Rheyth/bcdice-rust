...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/WorldEndScrapyard.toml");
    ...
    /// `test/data/WorldEndScrapyard.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/WorldEndScrapyard.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("WorldEndScrapyard.toml must parse");
        assert_eq!(
            data.tests.len(),
            19,
            "case count in test/data/WorldEndScrapyard.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "WorldEndScrapyard",
                "unexpected game system in WorldEndScrapyard.toml"
            );
            ...
            match eval_command(&GameSystemId::new("WorldEndScrapyard"), &tc.input, &mut src) {
        ...
        assert!(
            failures.is_empty(),
            "{}/{} WorldEndScrapyard cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
