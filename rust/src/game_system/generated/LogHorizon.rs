...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/LogHorizon.toml");
    ...
    /// `test/data/LogHorizon.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/LogHorizon.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("LogHorizon.toml must parse");
        assert_eq!(
            data.tests.len(),
            268,
            "case count in test/data/LogHorizon.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "LogHorizon",
                "unexpected game system in LogHorizon.toml"
            );
            ...
            match eval_command(&GameSystemId::new("LogHorizon"), &tc.input, &mut src) {
