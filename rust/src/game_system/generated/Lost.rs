...
mod tests {
    ...
    /// `test/data/Lost.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Lost.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Lost.toml must parse");
        assert_eq!(data.tests.len(), 17, "case count in test/data/Lost.toml");

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Lost",
                "unexpected game system in Lost.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Lost"), &tc.input, &mut src) {
