...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/DiceOfTheDead.toml");
    ...
    /// `test/data/DiceOfTheDead.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/DiceOfTheDead.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("DiceOfTheDead.toml must parse");
        assert_eq!(
            data.tests.len(),
            12,
            "case count in test/data/DiceOfTheDead.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "DiceOfTheDead",
                "unexpected game system in DiceOfTheDead.toml"
            );
            ...
            match eval_command(&GameSystemId::new("DiceOfTheDead"), &tc.input, &mut src) {
