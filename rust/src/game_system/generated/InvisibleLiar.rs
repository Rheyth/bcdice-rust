...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/InvisibleLiar.toml");
    ...
    /// `test/data/InvisibleLiar.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/InvisibleLiar.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("InvisibleLiar.toml must parse");
        assert_eq!(
            data.tests.len(),
            41,
            "case count in test/data/InvisibleLiar.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "InvisibleLiar",
                "unexpected game system in InvisibleLiar.toml"
            );
            ...
            match eval_command(&GameSystemId::new("InvisibleLiar"), &tc.input, &mut src) {
