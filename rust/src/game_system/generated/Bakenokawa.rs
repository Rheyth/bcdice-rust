...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/Bakenokawa.toml");
    ...
    /// `test/data/Bakenokawa.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Bakenokawa.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Bakenokawa.toml must parse");
        assert_eq!(
            data.tests.len(),
            69,
            "case count in test/data/Bakenokawa.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Bakenokawa",
                "unexpected game system in Bakenokawa.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Bakenokawa"), &tc.input, &mut src) {
