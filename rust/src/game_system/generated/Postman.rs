...
mod tests {
    ...
    /// `test/data/Postman.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Postman.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Postman.toml must parse");
        assert_eq!(data.tests.len(), 31, "case count in test/data/Postman.toml");

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Postman",
                "unexpected game system in Postman.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Postman"), &tc.input, &mut src) {
