...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/MarvelHeroicRoleplaying.toml");
    ...
    /// `test/data/MarvelHeroicRoleplaying.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/MarvelHeroicRoleplaying.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("MarvelHeroicRoleplaying.toml must parse");
        assert_eq!(
            data.tests.len(),
            8,
            "case count in test/data/MarvelHeroicRoleplaying.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "MarvelHeroicRoleplaying",
                "unexpected game system in MarvelHeroicRoleplaying.toml"
            );
            ...
            match eval_command(
                &GameSystemId::new("MarvelHeroicRoleplaying"),
                &tc.input,
                &mut src,
            ) {
        ...
        assert!(
            failures.is_empty(),
            "{}/{} MarvelHeroicRoleplaying cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
