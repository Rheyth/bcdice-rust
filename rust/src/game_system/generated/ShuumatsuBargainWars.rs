...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/ShuumatsuBargainWars.toml");
    ...
    /// `test/data/ShuumatsuBargainWars.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/ShuumatsuBargainWars.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("ShuumatsuBargainWars.toml must parse");
        assert_eq!(
            data.tests.len(),
            11,
            "case count in test/data/ShuumatsuBargainWars.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "ShuumatsuBargainWars",
                "unexpected game system in ShuumatsuBargainWars.toml"
            );
            ...
            match eval_command(
                &GameSystemId::new("ShuumatsuBargainWars"),
                &tc.input,
                &mut src,
            ) {
        ...
        assert!(
            failures.is_empty(),
            "{}/{} ShuumatsuBargainWars cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
