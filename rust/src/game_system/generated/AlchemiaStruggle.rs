...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/AlchemiaStruggle.toml");
    ...
    /// `test/data/AlchemiaStruggle.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/AlchemiaStruggle.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("AlchemiaStruggle.toml must parse");
        assert_eq!(
            data.tests.len(),
            44,
            "case count in test/data/AlchemiaStruggle.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "AlchemiaStruggle",
                "unexpected game system in AlchemiaStruggle.toml"
            );
            ...
            match eval_command(&GameSystemId::new("AlchemiaStruggle"), &tc.input, &mut src) {
