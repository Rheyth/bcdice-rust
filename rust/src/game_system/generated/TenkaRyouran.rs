//! P4で手書き移植した `lib/bcdice/game_system/TenkaRyouran.rb`。
//!
...
//! - `TenkaRyouran` 自身の `set_aliases_for_srs_roll('TR')`（＝判定コマンドの表記に
//!   `2D6` の別名 `TR` を足す設定）
//!
//! # SRS判定の置き場所
//!
//! Ruby側は `SRS` クラスに判定本体があり、`TenkaRyouran` は別名を足すだけ。
//! Rust側の `SRS.rs` はまだ生成スタブのままで別バッチの担当なので、ここでは
//! 判定本体をこのファイルに持つ（`MetallicGuardian.rs` も同じ理由で同じ実装を持つ）。
//! `SRS` 本体が移植されたら、両者をそちらへ寄せて重複を畳める。
//!
//! ロケール差のある文言は [`SystemTables`] に束ね、
//! `TenkaRyouran_Korean`（`ko_kr`）が同じ関数群を使い回す。
...
/// 1ロケール分の設定と定型文。`TenkaRyouran` と `TenkaRyouran_Korean` はこれだけが違う。
...
/// Ruby `BCDice::GameSystem::TenkaRyouran`（ID: `TenkaRyouran`）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct TenkaRyouran;

impl GameSystem for TenkaRyouran {
...
　　例) 2d6+2@12#4 　修正値+2、クリティカル値12、ファンブル値4で判定
　　例) TR　　　　　 2d6[]と同じ（TRが2D6のショートカットコマンド）
　　例) TR+2@12#4　　2d6+2@12#4と同じ（TRが2D6のショートカットコマンド）
...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/TenkaRyouran.toml");
    ...
    /// `test/data/TenkaRyouran.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/TenkaRyouran.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("TenkaRyouran.toml must parse");
        assert_eq!(
            data.tests.len(),
            22,
            "case count in test/data/TenkaRyouran.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "TenkaRyouran",
                "unexpected game system in TenkaRyouran.toml"
            );
            ...
            match eval_command(&GameSystemId::new("TenkaRyouran"), &tc.input, &mut src) {
