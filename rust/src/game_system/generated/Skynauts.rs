...
mod tests {
    ...
    /// `test/data/Skynauts.toml` の全ケースが通ること。
    ///
    /// 判定項目は `rust/tests/toml_harness.rs::run_case` と同じ
    /// （出力文字列・5フラグ・注入乱数を使い切ったか）。`tc.input` はそのまま渡す
    ///（タブ以降は Preprocessor が落とす）。
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/Skynauts.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Skynauts.toml must parse");
        assert_eq!(
            data.tests.len(),
            47,
            "case count in test/data/Skynauts.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Skynauts",
                "unexpected game system in Skynauts.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Skynauts"), &tc.input, &mut src) {
