//! P4で手書き移植した `lib/bcdice/game_system/TacticalExorcist.rb`。
//!
...
//!
//! 移植したもの:
//! - `#eval_game_system_specific_command`（`@` 付き簡易入力 → 通常の成功判定 → 定型文の順）
//! - `#proc_dice_command_with_at`（`K3,H3@4,5` を `K3,H3[=4,5]` へ読み替えて投げ直す）
//! - `#check_difficulty_te` / `#proc_appendix`（`[=4,5]` 形式の追加判定を親へ足したもの）
//! - 親 `NinjaSlayer2` から継承する `#proc_dice_2nd` / `#proc_text` 一式
//!
//! # 親クラスの扱い
//!
//! Ruby側は `NinjaSlayer2` を継承する。Rust側の [`super::NinjaSlayer2`] は移植済みだが
//! 補助関数を公開していないので、親の実装をこのファイルへ複製している
//! （P4の方針どおりコード重複を許容する。親を公開形へ整理したら差し替えられる）。
//!
//! 親から変えているのは次の3点だけ:
//! 1. `RE_JUDGE_DICEROLL` から `S…` / `C…`（サツバツ・クリティカル）の追加判定を外し、
//!    代わりに `(=)([1-6](,[1-6])+)`（`[=4,5]` 形式）を足したもの
//! 2. `RE_JUDGE_DICEROLL_TE`（`@` 付き簡易入力）とその処理
//! 3. `#proc_appendix` に `RE_COUNT_JUDGE_TE` の分岐を `RE_COUNT_JUDGE` の**前**に挿入
//!
//! 1 の結果、`K3[S5]` のようなサツバツ系コマンドは判定にマッチせず定型文処理へ落ち、
//! そこでも該当が無いので `nil` になる（＝ゲーム上不要なので表示されない）。
//!
//! # 表データ
//!
//! `#proc_satz_batz` 等が使う表は親の i18n キー（`NinjaSlayer2.table.…`）のままなので、
//! `i18n/NinjaSlayer2/ja_jp.yml` の値を `static` として持つ（値は1文字も変えていない）。
//! なお登録接頭辞が `UH` / `[KENHU]` だけなので、`SB` / `WS` / `WSE` は
//! `Base#dice_command` の接頭辞判定を通らず、実際に届くのは `NRS…` だけである。
//!
//! # バラバラロールの取り込み
//!
...
//! `TacticalExorcist` は `sort_barabara_dice` が既定（false）なので出目は振った順のまま。
...
// Ruby `TacticalExorcist::RE_COUNT_JUDGE_TE`。`[=4,5]` 形式の追加判定。
re!(re_count_judge_te, r"(=)([1-6](,[1-6])+)");
// Ruby `TacticalExorcist::RE_JUDGE_DICEROLL`。親の `S…` / `C…` が無い代わりに
// `(=)([1-6](,[1-6])+)` を持つ。
re!(
    re_judge_diceroll,
    r"(?i)^((?:(?:UH|[KENHU])?\d+,?)+)(?:\[((?:(?:(=|!=|>=|>|<=|<)([1-6]+\+?)|(=)([1-6](,[1-6])+))(?:\]\[)?)+)\])?$"
);
// Ruby `TacticalExorcist::RE_JUDGE_DICEROLL_TE`。`@` 付きの簡易入力。
re!(
    re_judge_diceroll_te,
    r"(?i)^((?:(?:UH|[KENHU])?\d+,?)+)(@[1-6](,[1-6])*)$"
);
...
///
/// 難易度は `,` 区切りの複数指定で、出目×難易度の二重ループなので
/// 1つの出目が複数の難易度を満たすと重複して積まれる（原典どおり）。
fn check_difficulty_te(dice: &[i64], difficultys: &str, cmp_op: CmpOp) -> Vec<i64> {
    let diff_array = ruby_split(difficultys, ",");
    let mut success_dice = Vec::new();
    for dice_value in dice {
        for diff_value in &diff_array {
            // ここに来る文字列は `[1-6]` 由来なので単純にパースできる。
            let target = diff_value.parse::<i64>().unwrap_or(0);
            if cmp_op.apply(&I::from(*dice_value), &I::from(target)) {
...
/// Ruby `TacticalExorcist#proc_appendix(roll_result, ap_command)`。
///
/// 親との違いは `RE_COUNT_JUDGE_TE` の分岐が `RE_COUNT_JUDGE` の前に入ること
/// （Rubyの `case` と同じ順序: SATZ_BATZ → CRITICAL → JUDGE_TE → JUDGE）。
fn proc_appendix(dice: &[i64], ap_command: &str) -> String {
    ...
    if let Some(caps) = re_count_satz_batz().captures(ap_command) {
    ...
    } else if let Some(caps) = re_count_judge_te().captures(ap_command) {
        let diff_type = &caps[1];
        let diff_condition = &caps[2];
        ...
        // 追加判定カウント
        let diff_result = check_difficulty_te(dice, diff_condition, cmp_op);
        output_text += &format!(
            ", 追加判定[{ap_command}]:{}{}",
            diff_result.len(),
            descending_bracket(&diff_result)
...
/// Ruby `TacticalExorcist#proc_dice_command_with_at(at_command)`。
///
/// `K3,H3@4,5` を `K3,H3[=4,5]` へ読み替えて `#proc_dice_2nd` に投げ直す。
/// Ruby は読み替え後がマッチしなければ `nil` を返し、呼び出し元の `proc_result[1]` が
/// `NoMethodError` になって `rescue` が `nil` を返す。ここでは `Ok(None)` で表す。
fn proc_dice_command_with_at(
    at_command: &str,
    rng: &mut Randomizer,
) -> Result<Option<(String, i64)>, EvalError> {
    let parts = ruby_split(at_command, "@");
    let head = parts.first().copied().unwrap_or("");
    let eq_param = parts.get(1).copied().unwrap_or("");
    let command = format!("{head}[={eq_param}]");

    let Some(caps) = re_judge_diceroll().captures(&command) else {
        return Ok(None);
    };
    proc_dice_2nd(&caps, rng)
}

/// Ruby `#proc_dice_2nd` の戻り値（テキストと達成値）を `Result` に畳む。
fn judge_result(text: String, total_success_num: i64) -> SpecificCommandOutput {
    if total_success_num > 0 {
        SpecificCommandOutput::result(EvalResult::success(text))
    } else {
        SpecificCommandOutput::result(EvalResult::failure(text))
    }
}

/// Ruby `TacticalExorcist#eval_game_system_specific_command`。
///
/// Ruby は解析できずに例外が出たら構文ミスとみなして `nil` を返す
/// （`rescue StandardError`）。Rust側はその経路を `Ok(None)` で表す。
fn eval_specific_command(
    command: &str,
    rng: &mut Randomizer,
) -> Result<Option<SpecificCommandOutput>, EvalError> {
    if re_judge_diceroll_te().is_match(command) {
        // タクティカル祓魔師TRPGで追加されたコマンド
        let Some((text, total)) = proc_dice_command_with_at(command, rng)? else {
            return Ok(None);
        };
        return Ok(Some(judge_result(text, total)));
    }

    if let Some(caps) = re_judge_diceroll().captures(command) {
        // ソウルワイヤードTRPGフレームワーク2版(ニンジャスレイヤーTRPG第2版)用のダイス判定
        let Some((text, total)) = proc_dice_2nd(&caps, rng)? else {
            return Ok(None);
        };
        return Ok(Some(judge_result(text, total)));
    }
...
/// Ruby `BCDice::GameSystem::TacticalExorcist`（ID: `TacticalExorcist`）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct TacticalExorcist;

impl GameSystem for TacticalExorcist {
...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/TacticalExorcist.toml");
    ...
    /// `test/data/TacticalExorcist.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/TacticalExorcist.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("TacticalExorcist.toml must parse");
        assert_eq!(
            data.tests.len(),
            20,
            "case count in test/data/TacticalExorcist.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "TacticalExorcist",
                "unexpected game system in TacticalExorcist.toml"
            );
            ...
            match eval_command(&GameSystemId::new("TacticalExorcist"), &tc.input, &mut src) {
