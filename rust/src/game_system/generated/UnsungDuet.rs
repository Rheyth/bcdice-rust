...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/UnsungDuet.toml");
    ...
    /// `test/data/UnsungDuet.toml` の全ケースが通ること。
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/UnsungDuet.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("UnsungDuet.toml must parse");
        assert_eq!(
            data.tests.len(),
            28,
            "case count in test/data/UnsungDuet.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "UnsungDuet",
                "unexpected game system in UnsungDuet.toml"
            );
            ...
            match eval_command(&GameSystemId::new("UnsungDuet"), &tc.input, &mut src) {
