...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/CharonSanctions.toml");
    ...
    /// `test/data/CharonSanctions.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/CharonSanctions.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("CharonSanctions.toml must parse");
        assert_eq!(
            data.tests.len(),
            17,
            "case count in test/data/CharonSanctions.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "CharonSanctions",
                "unexpected game system in CharonSanctions.toml"
            );
            ...
            match eval_command(&GameSystemId::new("CharonSanctions"), &tc.input, &mut src) {
