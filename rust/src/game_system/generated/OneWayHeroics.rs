...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/OneWayHeroics.toml");
    ...
    /// `test/data/OneWayHeroics.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/OneWayHeroics.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("OneWayHeroics.toml must parse");
        assert_eq!(
            data.tests.len(),
            76,
            "case count in test/data/OneWayHeroics.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "OneWayHeroics",
                "unexpected game system in OneWayHeroics.toml"
            );
            ...
            match eval_command(&GameSystemId::new("OneWayHeroics"), &tc.input, &mut src) {
