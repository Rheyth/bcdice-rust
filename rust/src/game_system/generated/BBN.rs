...
mod tests {
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/BBN.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("BBN.toml must parse");
        assert_eq!(data.tests.len(), 29, "case count in test/data/BBN.toml");

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(tc.game_system, "BBN", "unexpected game system in BBN.toml");

            let mut reasons: Vec<String> = Vec::new();
            let rands: Vec<(i64, i64)> = tc.rands.iter().map(|r| (r.value, r.sides)).collect();
            let mut src = SeededRandomizer::new(rands);

            match eval_command(&GameSystemId::new("BBN"), &tc.input, &mut src) {
