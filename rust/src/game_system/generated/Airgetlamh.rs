...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/Airgetlamh.toml");
    ...
    /// `test/data/Airgetlamh.toml` の全ケースが通ること。
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/Airgetlamh.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Airgetlamh.toml must parse");
        assert_eq!(
            data.tests.len(),
            29,
            "case count in test/data/Airgetlamh.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Airgetlamh",
                "unexpected game system in Airgetlamh.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Airgetlamh"), &tc.input, &mut src) {
