...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/KemonoNoMori_Korean.toml");
    ...
    /// `test/data/KemonoNoMori_Korean.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/KemonoNoMori_Korean.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("KemonoNoMori_Korean.toml must parse");
        assert_eq!(
            data.tests.len(),
            89,
            "case count in test/data/KemonoNoMori_Korean.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "KemonoNoMori:Korean",
                "unexpected game system in KemonoNoMori_Korean.toml"
            );
            ...
            match eval_command(
                &GameSystemId::new("KemonoNoMori:Korean"),
                &tc.input,
                &mut src,
            ) {
        ...
        assert!(
            failures.is_empty(),
            "{}/{} KemonoNoMori:Korean cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
