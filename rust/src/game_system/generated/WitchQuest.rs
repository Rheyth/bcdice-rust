...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/WitchQuest.toml");
    ...
    /// `test/data/WitchQuest.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/WitchQuest.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("WitchQuest.toml must parse");
        assert_eq!(
            data.tests.len(),
            10,
            "case count in test/data/WitchQuest.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "WitchQuest",
                "unexpected game system in WitchQuest.toml"
            );
            ...
            match eval_command(&GameSystemId::new("WitchQuest"), &tc.input, &mut src) {
