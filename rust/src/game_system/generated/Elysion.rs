//! P4で手書き移植した `lib/bcdice/game_system/Elysion.rb`。
//!
...
//!
//! 移植したもの:
//! - `Elysion#eval_game_system_specific_command`（`EL` 判定・`DATE` 系・`checkAnyCommand`・`TABLES`）
//! - `#check` / `#getCheckResult` / `#getSuccessResult` / `#getSuccessRank` /
//!   `#getSpecialResult` / `#getFambleResultText`（ゾロ目分岐）
//! - `#getDateBothResult` / `#getDateResult` / `#getDateTableByType` / `#changePcName`
//! - `#getBreakTable`（休憩表・2D6）/ `#getD6Table`（1D6）/ `#getRandomNpc`（D66昇順）/
//!   `#getEncountTableResult`（1D6+レベル）
//!
//! 表データは `lib/bcdice/game_system/Elysion.rb` から機械的に書き出したもので、
//! 値は1文字も変えていない。
...
/// Ruby `BCDice::GameSystem::Elysion`（ID: `Elysion`）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Elysion;

impl GameSystem for Elysion {
    ...
    /// Ruby `Elysion#eval_game_system_specific_command`。
...
/// Ruby `Elysion#eval_game_system_specific_command`。
///
/// Ruby は分岐の戻り値が `Result` / `String` の2種で、`Result` ならそのまま返し、
/// 空文字なら `nil` 相当、それ以外は `"#{command} ＞ #{result}"` で包む。
fn eval_specific_command(
    command: &str,
    rng: &mut Randomizer,
) -> Result<Option<SpecificCommandOutput>, EvalError> {
    // Ruby: when /^EL(\d*)(\+\d+)?/i
    if let Some((base, modify)) = match_el(command) {
        return Ok(Some(SpecificCommandOutput::result(check(
            base, modify, rng,
        )?)));
    }

    // Ruby: when /^(F|O|M)?DATE\[(.*),(.*)\]/i
    if let Some((date_type, pc1, pc2)) = match_date_both(command) {
        let body = get_date_both_result(date_type, pc1, pc2, rng)?;
        return Ok(wrap(command, body));
    }

    // Ruby: when /^(F|O|M)?DATE(\d\d)(\[(.*),(.*)\])?/i
    if let Some((date_type, number, pc1, pc2)) = match_date_number(command) {
        let body = get_date_result(date_type, number, pc1, pc2);
        return Ok(wrap(command, body));
    }

    // Ruby: when /^(F|O|M)?DATE/i
    if match_date_bare(command) {
        let body = rng.roll_once(6)?.to_string();
        return Ok(wrap(command, body));
    }
    ...
    Ok(wrap(command, result))
}

/// Ruby: `return '' if result.empty?` ／ `return "#{command} ＞ #{result}"`。
fn wrap(command: &str, body: String) -> Option<SpecificCommandOutput> {
    if body.is_empty() {
        return None;
    }
...
// ---------------------------------------------------------------------------
// コマンド解析（Ruby の case/when 正規表現に対応）
// ---------------------------------------------------------------------------

/// Ruby `/^EL(\d*)(\+\d+)?/i`。末尾は自由（アンカーなし）。
///
/// 返り値は `(base, modify)`。Ruby の `getValue` は `nil` を 0 にするので、
/// 未マッチの捕獲は 0 として返す。
fn match_el(command: &str) -> Option<(i64, i64)> {
    let rest = strip_prefix_ci(command, "EL")?;
    let (digits, rest) = take_while_digits(rest);
    let base = digits.parse::<i64>().unwrap_or(0);

    // Ruby: (\+\d+)? — "+" に続けて1桁以上の数字があるときだけ捕獲される
    let modify = match rest.strip_prefix('+') {
        Some(after) => {
            let (d, _) = take_while_digits(after);
...
/// Ruby `/^(F|O|M)?DATE\[(.*),(.*)\]/i`。
///
/// `(.*)` は貪欲なので、`[a,b,c]` は `pc1="a,b"` / `pc2="c"` になる。
/// また `\]` の後ろは自由なので、最後の `]` ではなく**最後のカンマ**で切れる点に注意。
fn match_date_both(command: &str) -> Option<(Option<char>, &str, &str)> {
    let (date_type, rest) = match_date_head(command)?;
    let rest = rest.strip_prefix('[')?;
    // 貪欲な (.*),(.*) の後に \] が続く。=> 最後の "]" の手前までを2つに割る。
    let close = rest.rfind(']')?;
    let inner = &rest[..close];
    let comma = inner.rfind(',')?;
    Some((date_type, &inner[..comma], &inner[comma + 1..]))
}

/// Ruby `/^(F|O|M)?DATE(\d\d)(\[(.*),(.*)\])?/i`。
fn match_date_number(command: &str) -> Option<(Option<char>, i64, &str, &str)> {
    let (date_type, rest) = match_date_head(command)?;
    let bytes = rest.as_bytes();
    if bytes.len() < 2 || !bytes[0].is_ascii_digit() || !bytes[1].is_ascii_digit() {
        return None;
    }
    let number = rest[..2].parse::<i64>().unwrap_or(0);

    // Ruby: (\[(.*),(.*)\])? — 丸ごと省略可能。中身が取れなければ pc1/pc2 は nil。
    let (pc1, pc2): (&str, &str) = rest[2..]
        .strip_prefix('[')
        .and_then(|after| {
        ...
        .unwrap_or_default();
    Some((date_type, number, pc1, pc2))
}

/// Ruby `/^(F|O|M)?DATE/i`。
fn match_date_bare(command: &str) -> bool {
    match_date_head(command).is_some()
}

/// Ruby `(F|O|M)?DATE` の共通部分。`(種別, 残り)` を返す。
fn match_date_head(command: &str) -> Option<(Option<char>, &str)> {
    for prefix in ['F', 'O', 'M'] {
        if let Some(rest) = strip_prefix_ci(command, &prefix.to_string()) {
            if let Some(rest) = strip_prefix_ci(rest, "DATE") {
                // Ruby は捕獲した文字をそのまま返す（小文字入力なら小文字）。
                // 判定側 `getDateTableByType` は /^F$/i なので大文字へ寄せてよい。
                return Some((Some(prefix), rest));
            }
        }
    }
    strip_prefix_ci(command, "DATE").map(|rest| (None, rest))
}

/// ASCII 大文字小文字を無視した `str::strip_prefix`。
...
/// 先頭の連続する ASCII 数字を切り出す。
fn take_while_digits(s: &str) -> (&str, &str) {
    let end = s
        .bytes()
...
/// Ruby `#SUCCESS_MAX`。
const SUCCESS_MAX: i64 = 5;

/// Ruby `#check`。
fn check(base: i64, modify: i64, rng: &mut Randomizer) -> Result<EvalResult, EvalError> {
    let dice1 = rng.roll_once(6)?;
    let dice2 = rng.roll_once(6)?;

    let dice_total = dice1 + dice2;
    let add_total = base + modify;
    let total = dice_total + add_total;

    let mut result = EvalResult::new();
    result.text.push_str("EL");
    if base != 0 {
        result.text.push_str(&base.to_string());
    }
    if modify != 0 {
        result.text.push_str(&value_string(modify));
    }
    ...
    result.text.push_str(&format!(
        " ＞ {dice_total}[{dice1},{dice2}]{} ＞ {total}",
        value_string(add_total)
    ));

    if dice1 == dice2 {
        get_special_result(dice1, total, &mut result);
        return Ok(result);
    }

    get_check_result(total, &mut result);
    Ok(result)
}

/// Ruby `#getValueString`。
fn value_string(value: i64) -> String {
    if value > 0 {
        format!("+{value}")
    } else if value < 0 {
        // Ruby 原文ママ（`"-#{value}"` なので値が負だと "--3" のようになる）。
        format!("-{value}")
    } else {
        String::new()
    }
...
/// Ruby `#getCheckResult`。
fn get_check_result(total: i64, result: &mut EvalResult) {
    let success = get_success_rank(total);

    if success == 0 {
        result.failure = true;
        result.text.push_str(" ＞ 失敗");
        return;
    }

    get_success_result(success, result);
}

/// Ruby `#getSuccessResult`。
fn get_success_result(success: i64, result: &mut EvalResult) {
    result.success = true;
    result.text.push_str(&format!(" ＞ 成功度{success}"));
    if success >= SUCCESS_MAX {
        result.critical = true;
        result.text.push_str(" ＞ 大成功 《アウル》2点獲得");
    }
}

/// Ruby `#getSuccessRank`: `((total - 9) / 5.0).ceil` を 0..=SUCCESS_MAX に丸める。
///
/// Ruby は浮動小数の `ceil` なので、整数除算では負側の答えが合わない。
fn get_success_rank(total: i64) -> i64 {
    let success = (((total - 9) as f64) / 5.0).ceil() as i64;
    success.clamp(0, SUCCESS_MAX)
}

/// Ruby `#getSpecialResult`（ゾロ目のとき）。
fn get_special_result(number: i64, total: i64, result: &mut EvalResult) {
    if number == 6 {
        // Ruby `#getCriticalResult`
        get_success_result(SUCCESS_MAX, result);
        return;
    }
    get_famble_result_text(number, total, result);
}

/// Ruby `#getFambleResultText`（原文ママ・タイポ）。
fn get_famble_result_text(number: i64, total: i64, result: &mut EvalResult) {
    if number == 1 {
        result.fumble = true;
        result.failure = true;
        result.text.push_str(" ＞ 大失敗");
        return;
    }

    get_check_result(total, result);
    result
        .text
        .push_str(&format!(" ／ ({}回目のアシストなら)大失敗", number - 1));
}

// ---------------------------------------------------------------------------
// デート表。Ruby `#getDateBothResult` 以下
// ---------------------------------------------------------------------------

/// Ruby `#getDateBothResult`。
fn get_date_both_result(
    date_type: Option<char>,
    pc1: &str,
    pc2: &str,
    rng: &mut Randomizer,
    ...
    // Ruby: 先に入れ替え前の並びで見出しを組む。
    let mut result = format!("{pc1}[{dice1}],{pc2}[{dice2}] ＞ ");

    let mut number = dice1 * 10 + dice2;
    let (pc1, pc2) = if dice1 > dice2 {
    ...
    result.push_str(&get_date_result(date_type, number, pc1, pc2));
    Ok(result)
}

/// Ruby `#getDateResult`。
fn get_date_result(date_type: Option<char>, number: i64, pc1: &str, pc2: &str) -> String {
    let Some((name, table)) = get_date_table_by_type(date_type) else {
        // Ruby: `return '', []` → get_table_by_number が default "1" を返し、
        // 呼び出し側は `"表(番号) ＞ 1"` を作る。実際には到達しない。
        return format!("表({number}) ＞ 1");
    };

    let text = get_table_by_number(number, table, "1");
    let text = change_pc_name(&text, "受け身キャラ", pc1);
    let text = change_pc_name(&text, "攻め気キャラ", pc2);

    format!("{name}表({number}) ＞ {text}")
}

/// Ruby `#getDateTableByType`。
fn get_date_table_by_type(
    date_type: Option<char>,
) -> Option<(&'static str, &'static [(i64, &'static str)])> {
    match date_type {
        None => Some(("デート", DATE_ITEMS)),
        Some('F') => Some(("友達デート", FDATE_ITEMS)),
        Some('O') => Some(("片思いデート", ODATE_ITEMS)),
        Some('M') => Some(("真夜中デート", MDATE_ITEMS)),
        _ => None,
    }
}

/// Ruby `#changePcName`。
fn change_pc_name(text: &str, base: &str, name: &str) -> String {
    if name.is_empty() {
        return text.to_string();
    }
    text.replace(base, &format!("{base}({name})"))
}

/// Ruby `Base#get_table_by_number`（先頭から `number >= index` の項目を返す）。
fn get_table_by_number(index: i64, table: &'static [(i64, &'static str)], default: &str) -> String {
...
// ---------------------------------------------------------------------------
// checkAnyCommand。Ruby `#checkAnyCommand`
// ---------------------------------------------------------------------------

/// Ruby `#checkAnyCommand`。`command.upcase` で分岐する。
fn check_any_command(command: &str, rng: &mut Randomizer) -> Result<String, EvalError> {
    let upcased = command.to_uppercase();

    if let Some((_, name, items)) = BREAK_TABLES.iter().find(|(key, _, _)| *key == upcased) {
        return get_break_table(name, items, rng);
    }
    if let Some((_, name, items)) = D6_TABLES.iter().find(|(key, _, _)| *key == upcased) {
        return get_d6_table(name, items, rng);
    }
    if let Some((_, name, items)) = NPC_TABLES.iter().find(|(key, _, _)| *key == upcased) {
        return get_random_npc(name, items, rng);
    }

    // Ruby: when /^NJ(\d+)$/i / when /^BS(\d+)$/i
    for (prefix, name, items) in ENCOUNT_TABLES {
        if let Some(rest) = upcased.strip_prefix(*prefix) {
            if !rest.is_empty() && rest.bytes().all(|b| b.is_ascii_digit()) {
                let level = rest.parse::<i64>().unwrap_or(0);
                return get_encount_table_result(name, items, level, rng);
            }
        }
    }
...
/// Ruby `#getBreakTable`。表名は `"#{name}休憩表(#{number}) #{text}"`（`＞` は挟まない）。
fn get_break_table(
    name: &str,
    ...
    match usize::try_from(index).ok().and_then(|i| table.get(i)) {
        None => Ok(String::new()),
        Some(text) => Ok(format!("{name}休憩表({number}) {text}")),
    }
}
...
fn get_d6_table(
    ...
    match usize::try_from(index).ok().and_then(|i| table.get(i)) {
        None => Ok(String::new()),
        Some(text) => Ok(format!("{name}({number}) {text}")),
    }
}

/// Ruby `#getRandomNpc`（`get_table_by_d66_swap` は D66 昇順）。
fn get_random_npc(
    name: &str,
    ...
    let result = get_table_by_number(number, table, "1");
    Ok(format!("{name}({number}) {result}"))
}

/// Ruby `#getEncountTableResult`（1D6 ＋ レベル。既定値は表の末尾）。
fn get_encount_table_result(
    name: &str,
    table: &'static [(i64, &'static str)],
    level: i64,
    rng: &mut Randomizer,
    ...
    let index = dice + level;

    // Ruby: get_table_by_number(index, table, table.last.last)
    let default = table.last().map(|(_, t)| *t).unwrap_or("1");
    let text = get_table_by_number(index, table, default);
    Ok(format!("{name}({index}) {text}"))
}

// ---------------------------------------------------------------------------
// 表データ（lib/bcdice/game_system/Elysion.rb から機械的に書き出したもの）
// ---------------------------------------------------------------------------

/// `(コマンド, 表示名, 項目)`。項目が番号つきの表（`get_table_by_number` 用）。
type NumberedTable = (&'static str, &'static str, &'static [(i64, &'static str)]);

/// Ruby `#checkAnyCommand` の休憩表分岐（`(コマンド, 表示名, 項目)`）。
static BREAK_TABLES: &[(&str, &str, &[&str])] = &[
    ("RBT", "教室", RBT_ITEMS),
    ("SBT", "購買", SBT_ITEMS),
    ("BBT", "部室", BBT_ITEMS),
    ("CBT", "生徒会室", CBT_ITEMS),
...
/// Ruby `#checkAnyCommand` の1D6表分岐。
static D6_TABLES: &[(&str, &str, &[&str])] = &[
    ("BFT", "戦場表", BFT_ITEMS),
    ("FWT", "致命傷表", FWT_ITEMS),
    ("FT", "ファンブル表", FT_ITEMS),
];

/// Ruby `#checkAnyCommand` のランダムNPC表分岐。
static NPC_TABLES: &[NumberedTable] = &[
    ("SRT", "学生生活関連ＮＰＣ表", SRT_ITEMS),
    ("ORT", "教師・その他ＮＰＣ表", ORT_ITEMS),
    ("DRT", "学生図鑑　下級学年表", DRT_ITEMS),
    ("URT", "学生図鑑　上級学年表", URT_ITEMS),
];

/// Ruby `#checkAnyCommand` の遭遇表分岐（`NJ<レベル>` / `BS<レベル>`）。
static ENCOUNT_TABLES: &[NumberedTable] = &[
    ("NJ", "日常遭遇表", NJ_ITEMS),
    ("BS", "ボスキャラクター遭遇表", BS_ITEMS),
];

/// Ruby `HELP_MESSAGE`。
static HELP_MESSAGE: &str = r"・判定（ELn+m）
...
/// Ruby `register_prefix`（生成スタブの値をそのまま保つ）。
static PREFIXES: &[&str] = &[
    "EL", "DATE", "FDATE", "ODATE", "MDATE", "RBT", "SBT", "BBT", "CBT", "DBT", "IBT", "FBT",
    "LBT", "PBT", "NBT", "ABT", "VBT", "GBT", "HBT", "BFT", "FWT", "FT", "SRT", "ORT", "DRT",
    "URT", "NJ", "BS", "EBT", "GIT", "HBT", "HT", "IT", "JH", "KT", "NA", "NT", "OJ1", "OJ2",
    "TBT", "UBT", "UT1", "UT2", "UT3", "UT4",
];

/// Ruby `#getClassRoomBreakTable`（教室休憩表・2D6）。
static RBT_ITEMS: &[&str] = &[
...
/// Ruby `#getSchoolStoreBrakeTable`（購買休憩表・2D6）。
...
/// Ruby `#getClubRoomBrakeTable`（部室休憩表・2D6）。
static BBT_ITEMS: &[&str] = &[
    "風紀委員の巡回！\n「そこ、何やってる！」風紀委員に見つかった。現在が授業パートであれば、こっぴどく叱られる。〔政治力〕で判定を行う。失敗すると、次のパートは行動できない。授業パート以外であれば、心身ともにリフレッシュ。《アウル》が1点回復する。",
...
/// Ruby `#getStudentCouncilBrakeTable`（生徒会室休憩表・2D6）。
static CBT_ITEMS: &[&str] = &[
    "風紀委員の巡回！\n「そこ、何やってる！」風紀委員に見つかった。現在が真夜中パートであれば、こっぴどく叱られる。〔政治力〕で判定を行う。失敗すると、次のパートは行動できない。真夜中パート以外であれば、心身ともにリフレッシュ。《アウル》が1点回復する。",
...
/// Ruby `#getDormitoryBrakeTable`（学生寮休憩表・2D6）。
static DBT_ITEMS: &[&str] = &[
    "風紀委員の巡回！\n「そこ、何やってる！」風紀委員に見つかった。現在が授業パートであれば、こっぴどく叱られる。〔政治力〕で判定を行う。失敗すると、次のパートは行動できない。授業パート以外であれば、心身ともにリフレッシュ。《アウル》が1点回復する。",
...
/// Ruby `#getLibraryBrakeTable`（図書館休憩表・2D6）。
...
/// Ruby `#getRoofBrakeTable`（屋上休憩表・2D6）。
static FBT_ITEMS: &[&str] = &[
    "風紀委員の巡回！\n「そこ、何やってる！」風紀委員に見つかった。現在が授業パートであれば、こっぴどく叱られる。〔政治力〕で判定を行う。失敗すると、次のパートは行動できない。授業パート以外であれば、心身ともにリフレッシュ。《アウル》が1点回復する。",
...
/// Ruby `#getLaboratoryBrakeTable`（研究室休憩表・2D6）。
static LBT_ITEMS: &[&str] = &[
    "風紀委員の巡回！\n「そこ、何やってる！」風紀委員に見つかった。現在が真夜中パートであれば、こっぴどく叱られる。〔政治力〕で判定を行う。失敗すると、次のパートは行動できない。真夜中パート以外であれば、心身ともにリフレッシュ。《アウル》が1点回復する。",
...
/// Ruby `#getPoolBrakeTable`（プール休憩表・2D6）。
static PBT_ITEMS: &[&str] = &[
    "風紀委員の巡回！\n「そこ、何やってる！」風紀委員に見つかった。現在が授業パートであれば、こっぴどく叱られる。〔政治力〕で判定を行う。失敗すると、次のパートは行動できない。授業パート以外であれば、心身ともにリフレッシュ。《アウル》が1点回復する。",
...
/// Ruby `#getInnerCourtBrakeTable`（中庭休憩表・2D6）。
static NBT_ITEMS: &[&str] = &[
    "風紀委員の巡回！\n「そこ、何やってる！」風紀委員に見つかった。現在が授業パートであれば、こっぴどく叱られる。〔政治力〕で判定を行う。失敗すると、次のパートは行動できない。授業パート以外であれば、心身ともにリフレッシュ。《アウル》が1点回復する。",
...
/// Ruby `#getShoppingAvenueBrakeTable`（商店街休憩表・2D6）。
static ABT_ITEMS: &[&str] = &[
    "風紀委員の巡回！\n「そこ、何やってる！」風紀委員に見つかった。現在が授業パートであれば、こっぴどく叱られる。〔政治力〕で判定を行う。失敗すると、次のパートは行動できない。授業パート以外であれば、心身ともにリフレッシュ。《アウル》が1点回復する。",
...
/// Ruby `#getDevastationBrakeTable`（廃墟休憩表・2D6）。
static VBT_ITEMS: &[&str] = &[
...
/// Ruby `#getGateBrakeTable`（ゲート休憩表・2D6）。
static GBT_ITEMS: &[&str] = &[
...
/// Ruby `#getBattleFieldTable`（戦場表・1D6）。
static BFT_ITEMS: &[&str] = &[
    "平地。\n特に特殊効果はない。",
...
/// Ruby `#getFatalWoundsTable`（致命傷表・1D6）。
static FWT_ITEMS: &[&str] = &[
...
/// Ruby `#getFumbleTable`（ファンブル表・1D6）。
static FT_ITEMS: &[&str] = &[
...
/// Ruby `#getDateTable`（デート）。
static DATE_ITEMS: &[(i64, &str)] = &[
...
/// Ruby `#getFrindDateTable`（友達デート）。
static FDATE_ITEMS: &[(i64, &str)] = &[
...
/// Ruby `#getOnewayDateTable`（片思いデート）。
static ODATE_ITEMS: &[(i64, &str)] = &[
...
/// Ruby `#getMidnightDateTable`（真夜中デート）。
static MDATE_ITEMS: &[(i64, &str)] = &[
    (11, "「こんなはずじゃなかったのにッ！」\n仲良くするつもりが、ひどい喧嘩になってしまう。この表の使用者のお互いに対する《感情値》が1点上昇し、属性が《敵意》になる。"),
...
/// Ruby `#getRandomNpcSchoolLife`（学生生活関連ＮＰＣ表）。
static SRT_ITEMS: &[(i64, &str)] = &[
    (11, "振り直し／任意"),
    (12, "大山恵（おおやま・めぐみ）：中等部3年0組：P84"),
    (
        13,
        "黒瀧辰馬（くろたき・たつま）：風紀委員長・大学部5年0組：P82",
    ),
    ...
    (
        22,
        "神楽坂茜（かぐらざか・あかね）：生徒会会長・高等部2年0組：P82",
    ),
    (23, "炎條忍（えんじょう・しのぶ）：高等部1年0組：P83"),
    (
        24,
        "中山寧々美（なかやま・ねねみ）：新聞同好会会長・高等部3年0組：P83",
    ),
    (25, "恵ヴィヴァルディ（めぐみ・−）：大学部2年0組：P83"),
    (26, "轟闘吾（とどろき・とうご）：高等部3年0組：P84"),
    (
        33,
        "鬼島武（きじま・たけし）：生徒会副会長・大学部1年0組：P82",
    ),
    ...
    (
        44,
        "大塔寺源九郎（だいとうじ・げんくろう）：生徒会書記・高等部3年0組：P82",
    ),
    (45, "ストローベレー：用務員：P83"),
    (46, "竜崎アリス（りゅうざき・−）：オペレーター：P84"),
    (
        55,
        "大鳥南（おおとり・みなみ）：生徒会会計・高等部3年0組：P82",
    ),
    (56, "筧鷹政（かけい・たかまさ）：OB：P83"),
    (66, "振り直し／任意"),
];

/// Ruby `#getRandomNpcOther`（教師・その他ＮＰＣ表）。
static ORT_ITEMS: &[(i64, &str)] = &[
    (11, "振り直し／任意"),
...
/// Ruby `#getRandomNpcDownClassmen`（学生図鑑　下級学年表）。
static DRT_ITEMS: &[(i64, &str)] = &[
    ...
    (
        46,
        "アイリス・L・橋場（−・るなくるす・はしば）：高等部1年11組：P79",
    ),
...
/// Ruby `#getRandomNpcUpperClassmen`（学生図鑑　上級学年表）。
static URT_ITEMS: &[(i64, &str)] = &[
    (11, "下妻笹緒（しもつま・ささお）：高等部2年2組：P117"),
    (
        12,
        "ファティナ・Ｖ・アイゼンブルク（−・フォン・−）：高等部2年3組：P110",
    ),
    ...
    (
        16,
        "大炊御門菫（おおいのみかど・すみれ）：高等部3年6組：P112",
    ),
    (
        22,
        "朱頼天山楓（しゅらい・てんざん・かえで）：高等部3年114組：P152",
    ),
    ...
    (
        45,
        "アウリーエ・Ｆ・ダッチマン（−・フライング・−）：大学部3年50組：P152",
    ),
...
/// Ruby `#getUsuallyEncount`（日常遭遇表）。
static NJ_ITEMS: &[(i64, &str)] = &[
    (2, "ブラックシープ（基本ｐ129）×2"),
    ...
    (
        15,
        "マッドマーマンティス（ｐ102）×2、バンパイアロード（ｐ103）",
    ),
    (16, "ドラゴン（ｐ99）×2"),
    (17, "風紀委員（ｐ107）、保安委員（ｐ107）×2"),
];

/// Ruby `#getBossEncount`（ボスキャラクター遭遇表）。
static BS_ITEMS: &[(i64, &str)] = &[
    (2, "剥奪天使（ｐ101）、イフリート（基本ｐ126）×PCと同じ数"),
    (
        3,
        "没落悪魔（ｐ105）、ブラックシープ（基本ｐ129）×PCと同じ数",
    ),
    ...
    (
        7,
        "美しきバンシー（ｐ103）、デビルキャリアー（ｐ102）×PCと同じ数",
    ),
    (8, "ドラゴン（ｐ99）、キラー（基本ｐ126）×PCと同じ数"),
    (
        9,
        "バンパイアロード（ｐ103）、戦闘員（基本ｐ132）×PCと同じ数",
    ),
    (10, "殺戮天使（ｐ101）、下級使徒（ｐ100）×PCと同じ数"),
    (11, "小悪魔（ｐ105）、デスストーカー（ｐ102）×PCと同じ数"),
    (
        12,
        "仙人（基本ｐ127）、サブラヒナイト（基本ｐ126）×PCと同じ数",
    ),
    ...
    (
        17,
        "悪魔貴族（基本ｐ131）、ブラッドウォーリア（ｐ103）×PCと同じ数",
    ),
];

// ==== TABLES ====

/// Ruby `TABLES["EBT"]`（お祭り室休憩表・2D6）。
static EBT_ITEMS: &[&str] = &[
...
static EBT: Table = Table::from_dice("お祭り室休憩表", 2, 6, EBT_ITEMS);

/// Ruby `TABLES["GIT"]`（陰謀表・2D6）。
static GIT_ITEMS: &[&str] = &[
...
static GIT: Table = Table::from_dice("陰謀表", 2, 6, GIT_ITEMS);

/// Ruby `TABLES["HBT"]`（温泉休憩表・2D6）。
static HBT_ITEMS: &[&str] = &[
...
static HBT: Table = Table::from_dice("温泉休憩表", 2, 6, HBT_ITEMS);

/// Ruby `TABLES["HT"]`（ロウ表・2D6）。
static HT_ITEMS: &[&str] = &[
...
static HT: Table = Table::from_dice("ロウ表", 2, 6, HT_ITEMS);

/// Ruby `TABLES["IT"]`（釣り人表・1D6）。
static IT_ITEMS: &[&str] = &[
...
static IT: Table = Table::from_dice("釣り人表", 1, 6, IT_ITEMS);

/// Ruby `TABLES["JH"]`（授業ハプニング表・2D6）。
static JH_ITEMS: &[&str] = &[
...
static JH: Table = Table::from_dice("授業ハプニング表", 2, 6, JH_ITEMS);

/// Ruby `TABLES["KT"]`（カオス表・2D6）。
static KT_ITEMS: &[&str] = &[
    "「たまには釣りにでも行くか」「釣り人表」を振る。",
...
static KT: Table = Table::from_dice("カオス表", 2, 6, KT_ITEMS);

/// Ruby `TABLES["NA"]`（新初期アイテム決定表・2D6）。
static NA_ITEMS: &[&str] = &[
    "従　者　ｐ89",
    "菓子折り　ｐ88",
    "ホイッスル　ｐ89",
...
static NA: Table = Table::from_dice("新初期アイテム決定表", 2, 6, NA_ITEMS);

/// Ruby `TABLES["NT"]`（ニュートラル表・1D6）。
static NT_ITEMS: &[&str] = &[
...
static NT: Table = Table::from_dice("ニュートラル表", 1, 6, NT_ITEMS);

/// Ruby `TABLES["OJ1"]`（音楽ジャンル決定表Ａ・1D6）。
static OJ1_ITEMS: &[&str] = &[
    "さわやかな",
    "ちょっと大人な",
    "正統派",
    "初恋色の",
...
static OJ1: Table = Table::from_dice("音楽ジャンル決定表Ａ", 1, 6, OJ1_ITEMS);

/// Ruby `TABLES["OJ2"]`（音楽ジャンル決定表Ｂ・1D6）。
static OJ2_ITEMS: &[&str] = &[
    "アイドルポップス",
    "ロックチューン",
    "ラブソング",
    "バラード",
...
static OJ2: Table = Table::from_dice("音楽ジャンル決定表Ｂ", 1, 6, OJ2_ITEMS);

/// Ruby `TABLES["TBT"]`（修学旅行休憩表・2D6）。
static TBT_ITEMS: &[&str] = &[
...
static TBT: Table = Table::from_dice("修学旅行休憩表", 2, 6, TBT_ITEMS);

/// Ruby `TABLES["UBT"]`（潜入調査休憩表・2D6）。
static UBT_ITEMS: &[&str] = &[
...
static UBT: Table = Table::from_dice("潜入調査休憩表", 2, 6, UBT_ITEMS);

/// Ruby `TABLES["UT1"]`（ユニット名決定表1A・D66）。
static UT1_ITEMS: &[(i64, TableItem)] = &[
    (11, TableItem::Text("白い")),
    (12, TableItem::Text("月の")),
    (13, TableItem::Text("私は")),
    (14, TableItem::Text("虹色の")),
    (15, TableItem::Text("放課後")),
    (16, TableItem::Text("はじめての")),
    (22, TableItem::Text("乙女の")),
    (23, TableItem::Text("ピンクの")),
    (24, TableItem::Text("魔法の")),
    (25, TableItem::Text("蒼い")),
    (26, TableItem::Text("恋の")),
    (33, TableItem::Text("ときめき")),
    (34, TableItem::Text("太陽の")),
    (35, TableItem::Text("燃えよ")),
    (36, TableItem::Text("雨の")),
    (44, TableItem::Text("星の")),
    (45, TableItem::Text("君に")),
    (46, TableItem::Text("思い出の")),
    (55, TableItem::Text("都会の")),
    (56, TableItem::Text("やさしい")),
    (66, TableItem::Text("黒い")),
];
static UT1: D66Table = D66Table::new("ユニット名決定表1A", D66SortType::Asc, UT1_ITEMS);

/// Ruby `TABLES["UT2"]`（ユニット名決定表1B・D66）。
static UT2_ITEMS: &[(i64, TableItem)] = &[
    (11, TableItem::Text("革命")),
    (12, TableItem::Text("エージェント")),
    (13, TableItem::Text("アリス")),
    (14, TableItem::Text("自転車")),
    (15, TableItem::Text("忍者")),
    (16, TableItem::Text("予知夢")),
    (22, TableItem::Text("リボン")),
    (23, TableItem::Text("メロディー")),
    (24, TableItem::Text("バースデイ")),
    (25, TableItem::Text("チョコレート")),
    (26, TableItem::Text("プレゼント")),
    (33, TableItem::Text("カオス")),
    (34, TableItem::Text("つばさ")),
    (35, TableItem::Text("夏")),
    (36, TableItem::Text("ココロ")),
    (44, TableItem::Text("ジェラシー")),
    (45, TableItem::Text("朝ご飯")),
    (46, TableItem::Text("保健室")),
    (55, TableItem::Text("祈り")),
    (56, TableItem::Text("迷宮")),
    (66, TableItem::Text("久遠ヶ原学園")),
];
static UT2: D66Table = D66Table::new("ユニット名決定表1B", D66SortType::Asc, UT2_ITEMS);

/// Ruby `TABLES["UT3"]`（ユニット名決定表2A・D66）。
static UT3_ITEMS: &[(i64, TableItem)] = &[
    (11, TableItem::Text("堕天使")),
    (12, TableItem::Text("ラヴクラフト")),
    (13, TableItem::Text("セレブリティ")),
    (14, TableItem::Text("スターダスト")),
    (15, TableItem::Text("ロリポップ")),
    (16, TableItem::Text("ファースト")),
    (22, TableItem::Text("ストロベリー")),
    (23, TableItem::Text("シャイニー")),
    (24, TableItem::Text("ドリーム")),
    (25, TableItem::Text("ヴァンパイア")),
    (26, TableItem::Text("シンデレラ")),
    (33, TableItem::Text("スウィート")),
    (34, TableItem::Text("ウィッチ")),
    (35, TableItem::Text("フラワー")),
    (36, TableItem::Text("オレンジ")),
    (44, TableItem::Text("ダイアモンド")),
    (45, TableItem::Text("モノクロ")),
    (46, TableItem::Text("バレンタイン")),
    (55, TableItem::Text("スタイリッシュ")),
    (56, TableItem::Text("コンクリート")),
    (66, TableItem::Text("ニンジャ")),
];
static UT3: D66Table = D66Table::new("ユニット名決定表2A", D66SortType::Asc, UT3_ITEMS);

/// Ruby `TABLES["UT4"]`（ユニット名決定表2B・D66）。
static UT4_ITEMS: &[(i64, TableItem)] = &[
    (11, TableItem::Text("スレイヤー")),
    (12, TableItem::Text("ファイター")),
    (13, TableItem::Text("セッション")),
    (14, TableItem::Text("ドーナツ")),
    (15, TableItem::Text("ワルツ")),
    (16, TableItem::Text("ティアラ")),
    (22, TableItem::Text("ガール")),
    (23, TableItem::Text("スカイ")),
    (24, TableItem::Text("キス")),
    (25, TableItem::Text("シティ")),
    (26, TableItem::Text("ステージ")),
    (33, TableItem::Text("ラブ")),
    (34, TableItem::Text("ロマンス")),
    (35, TableItem::Text("ノート")),
    (36, TableItem::Text("ショコラ")),
    (44, TableItem::Text("プリンセス")),
    (45, TableItem::Text("キャンディ")),
    (46, TableItem::Text("ハニー")),
    (55, TableItem::Text("ハンター")),
    (56, TableItem::Text("ディスコ")),
    (66, TableItem::Text("ブルー")),
];
static UT4: D66Table = D66Table::new("ユニット名決定表2B", D66SortType::Asc, UT4_ITEMS);
