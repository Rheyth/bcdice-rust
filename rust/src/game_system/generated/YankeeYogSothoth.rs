...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/YankeeYogSothoth.toml");
    ...
    /// `test/data/YankeeYogSothoth.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/YankeeYogSothoth.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("YankeeYogSothoth.toml must parse");
        assert_eq!(
            data.tests.len(),
            52,
            "case count in test/data/YankeeYogSothoth.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "YankeeYogSothoth",
                "unexpected game system in YankeeYogSothoth.toml"
            );
            ...
            match eval_command(&GameSystemId::new("YankeeYogSothoth"), &tc.input, &mut src) {
