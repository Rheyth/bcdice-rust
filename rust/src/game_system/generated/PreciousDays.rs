...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/PreciousDays.toml");
    ...
    /// `test/data/PreciousDays.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/PreciousDays.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("PreciousDays.toml must parse");
        assert_eq!(
            data.tests.len(),
            10,
            "case count in test/data/PreciousDays.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "PreciousDays",
                "unexpected game system in PreciousDays.toml"
            );
            ...
            match eval_command(&GameSystemId::new("PreciousDays"), &tc.input, &mut src) {
