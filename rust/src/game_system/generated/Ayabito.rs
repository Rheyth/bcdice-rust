...
mod tests {
    ...
    /// `test/data/Ayabito.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Ayabito.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Ayabito.toml must parse");
        assert_eq!(data.tests.len(), 41, "case count in test/data/Ayabito.toml");

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Ayabito",
                "unexpected game system in Ayabito.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Ayabito"), &tc.input, &mut src) {
