...
mod tests {
    ...
    /// `test/data/NSSQ.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/NSSQ.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("NSSQ.toml must parse");
        assert_eq!(data.tests.len(), 28, "case count in test/data/NSSQ.toml");

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "NSSQ",
                "unexpected game system in NSSQ.toml"
            );
            ...
            match eval_command(&GameSystemId::new("NSSQ"), &tc.input, &mut src) {
