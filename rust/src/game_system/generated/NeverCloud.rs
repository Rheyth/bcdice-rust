//! P4で手書き移植した `lib/bcdice/game_system/NeverCloud.rb`。
//!
...
//!
//! 移植したもの:
//! - `NeverCloud#eval_game_system_specific_command`
//!   （`check_action` = xNC/xD6/xD 判定、`TEXTS`、`TABLES`、`ALIAS_TEXTS`）
//! - `NeverCloud#check_action` → `check_action`
...
use crate::dice_table::{table::Table, RollableTable};
use crate::enums::RoundType;
use crate::eval::EvalError;
use crate::game_system::{GameSystem, SpecificCommandOutput};
use crate::randomizer::Randomizer;

/// Ruby `BCDice::GameSystem::NeverCloud`（ID: `NeverCloud`）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct NeverCloud;

impl GameSystem for NeverCloud {
    ...
    fn help_message(&self) -> &'static str {
        r"・LIST　のコマンドを入力して、ルールの解説・2D6表、一覧の表示
...
/// Ruby `/^(\d+)(?:NC|D6?)((?:[-+]\d+)*)(>=(\d+))?$/i`。
fn check_action_pattern() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| {
        Regex::new(r"(?i)^(\d+)(?:NC|D6?)((?:[-+]\d+)*)(>=(\d+))?$").expect("valid regex")
    })
}

/// Ruby `NeverCloud#eval_game_system_specific_command`。
fn eval_specific_command(
    command: &str,
    rng: &mut Randomizer,
) -> Result<Option<SpecificCommandOutput>, EvalError> {
    if check_action_pattern().is_match(command) {
        return check_action(command, rng).map(|text| Some(SpecificCommandOutput::text(text)));
    }
    if let Some(text) = text_for(command) {
        return Ok(Some(SpecificCommandOutput::text(text.to_string())));
    }
    ...
    if let Some(target) = ALIAS_TEXTS
        .iter()
        .find(|(key, _)| *key == command)
        .map(|(_, v)| *v)
    {
        if let Some(text) = text_for(target) {
            return Ok(Some(SpecificCommandOutput::text(text.to_string())));
        }
    }
    Ok(None)
...
/// Ruby `NeverCloud#check_action`。
fn check_action(command: &str, rng: &mut Randomizer) -> Result<String, EvalError> {
    let caps = check_action_pattern()
        .captures(command)
        .ok_or(EvalError::Internal("NeverCloud: check_action mismatch"))?;
    let dice_count: i64 = caps
        .get(1)
        .and_then(|m| m.as_str().parse().ok())
        .unwrap_or(0);
    let modify_str_raw = caps.get(2).map(|m| m.as_str()).unwrap_or("");
    let modify_number = arithmetic::eval(modify_str_raw, RoundType::Floor)?
        .as_ref()
        .map(crate::randomizer::sat_i64)
        .unwrap_or(0);
    let cmp_str = caps.get(3).map(|m| m.as_str()).unwrap_or("");
    let target: Option<i64> = caps.get(4).and_then(|m| m.as_str().parse().ok());

    // Ruby: modify_number == 0 なら modify_str を空文字列に。
    let modify_str = if modify_number == 0 {
        String::new()
    } else {
        modify_str_raw.to_string()
    };
    ...
    let mut total = dice_value + modify_number;

    let result = if dice_list.iter().filter(|&&d| d == 1).count() as i64 == dice_count {
        total = 0;
        Some("ファンブル")
    } else if dice_list.iter().filter(|&&d| d == 6).count() >= 2 {
        Some("クリティカル")
    ...
    let mut sequence = vec![
        format!("({dice_count}D6{modify_str}{cmp_str})"),
        format!("{dice_value}[{dice_str}]{modify_str}"),
        total.to_string(),
    ];
    if let Some(result) = result {
        sequence.push(result.to_string());
    }

    Ok(sequence.join(" ＞ "))
}

/// Ruby `TEXTS[command]`。`.chomp` 済みの文字列を返す。
fn text_for(command: &str) -> Option<&'static str> {
    TEXTS
        .iter()
        .find(|(key, _)| *key == command)
        .map(|(_, text)| *text)
...
/// Ruby `ALIAS_TEXTS`。
static ALIAS_TEXTS: &[(&str, &str)] = &[("ARTS", "ARTS1")];

/// Ruby `TABLES`。
static TABLES: &[(&str, Table)] = &[
    ("CHAR1", Table::from_dice("所属表", 2, 6, CHAR1_ITEMS)),
    ("CHAR2", Table::from_dice("趣味表", 2, 6, CHAR2_ITEMS)),
    (
        "CHAR3",
        Table::from_dice("リビド武装形状表", 2, 6, CHAR3_ITEMS),
    ),
    (
        "NAYA1",
        Table::from_dice("悩みの詳細表・愛/Love", 2, 6, NAYA1_ITEMS),
    ),
    (
        "NAYA2",
        Table::from_dice("悩みの詳細表・体/Figure", 2, 6, NAYA2_ITEMS),
    ),
    (
        "NAYA3",
        Table::from_dice("悩みの詳細表・才/Talent", 2, 6, NAYA3_ITEMS),
    ),
    (
        "NAYA4",
        Table::from_dice("悩みの詳細表・絆/Bonds", 2, 6, NAYA4_ITEMS),
    ),
    (
        "NAYA5",
        Table::from_dice("悩みの詳細表・住/Home", 2, 6, NAYA5_ITEMS),
    ),
];

const CHAR1_ITEMS: &[&str] = &[
    "無職/専業主婦/専業主夫",
    "パート・アルバイト",
    "中学生",
    "高校生",
...
const LIST_TEXT: &str = "※注記：このダイスボットデータは『ネバークラウドTRPG』公式『私立彩音学園』主導の下で制作されました。
...
const LIKE1_TEXT: &str = "LIKE(基本ルール書籍版P60)
...
const RESE1_TEXT: &str = "リサーチカード(基本ルール書籍版P63)
...
const RESE2_TEXT: &str = "インタビュー(基本ルール書籍版P62)
...
const RESE3_TEXT: &str = "パイルドライヴ(基本ルール書籍版P64)
...
const RESE4_TEXT: &str = "[決意]と[使命](基本ルール書籍版P61)
...
const ARTS1_TEXT: &str = "リビドアーツのデータ項目(基本ルール書籍版P40)
...
const STAT1_TEXT: &str = "SSリスト(基本ルール書籍版P41)
...
const STAT2_TEXT: &str = "LSリスト(基本ルール書籍版P41)
...
const ACTI4_TEXT: &str = "サーヴァント共通アクション(基本ルール書籍版P42)
...
const BATT5_TEXT: &str = "攻撃の手順(メインアクションで行う)04.ダメージ適用ステップ(基本ルール書籍版P74)
...
const BATT8_TEXT: &str = "パケット(基本ルール書籍版P69)
...
const BATT9_TEXT: &str = "リビドストーム(基本ルール書籍版P71)
〈ピーター〉のナナメにあるSqを【リビドストーム】と呼ぶ。
　このSqにいるキャラクターは攻撃の際に自身の{得意方向}の効果が無効になる。";

const PIET1_TEXT: &str = "ピーターの基本的な性質(基本ルール書籍版P72)
...
const MINI1_TEXT: &str = "基本ルールからねばくらミニへの変更点(ねばくらミニ書籍版P05)
...
const MINI2_TEXT: &str = "ねばくらミニでのリサーチのルール変更点(ねばくらミニ書籍版P11)
・[Wコトノハ]は宣言せず〈レゾナンス・パイルドライヴ〉は発生しない(プリプレイでも[Wコトノハ]を選択しない)
・「ピーターのバフ」は無効になる。
・《インタビュー》はPCごとにシーン1回10分とする。";

const MINI3_TEXT: &str = "ねばくらミニでのバトル演出のルール(ねばくらミニ書籍版P12)
...
const MINI4_TEXT: &str = "ねばくらミニでのアーツ(ねばくらミニ書籍版P13)
...
const MINI5_TEXT: &str = "ねばくらミニでのスペシャルアーツ(ねばくらミニ書籍版P13)
...
const MINI6_TEXT: &str = "ねばくらミニでのポータルとサーヴァント(ねばくらミニ書籍版P14)。
・「ポータル」なら威力ロールに+30してよい。
・「サーヴァント」は自身が攻撃の対象になったとき、対象を自身のサーヴァントに変更してよい。";

const CHAR1L_TEXT: &str = "・所属表(2D6またはRoC。基本ルール書籍版P33)
2, 無職/専業主婦/専業主夫
3, パート・アルバイト
4, 中学生
5, 高校生
...
const CHAR2L_TEXT: &str = "・趣味表(2D6またはRoC。基本ルール書籍版P38)
2, プログラミング/ハッキング
3, テレビゲーム/ボードゲーム
4, 楽器演奏/作曲/編曲/DTM
5, 工芸/アクセサリー/プラモデル制作
...
const CHAR3L_TEXT: &str = "・リビド武装形状表(2D6またはRoC。基本ルール書籍版P39)
2, 模様。輝く五芒星や、巨大な数字など
...
const NAYA1L_TEXT: &str = "●悩みの詳細表・愛/Love:得意方向(正面)(2D6またはRoC。基本ルール書籍版P49)
...
const NAYA5L_TEXT: &str = "●悩みの詳細表・住/Home:得意方向(側面)(2D6またはRoC。基本ルール書籍版P49)
2, 親がもううるさくない
...
mod tests {
    ...
    fn all_toml_cases_pass() {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .unwrap()
            .join("test/data/NeverCloud.toml");
        if !path.exists() {
            return;
        }
        let data = TestDataFile::load(&path).expect("NeverCloud.toml must parse");
        assert_eq!(
            data.tests.len(),
            119,
            "case count in test/data/NeverCloud.toml"
        );
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(tc.game_system, "NeverCloud");
            let mut src = SeededRandomizer::new(tc.rands.iter().map(|r| (r.value, r.sides)));
            let result = eval_command(&GameSystemId::new("NeverCloud"), &tc.input, &mut src)
