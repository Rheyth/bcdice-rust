...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/DungeonsAndDragons5.toml");
    ...
    /// `test/data/DungeonsAndDragons5.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/DungeonsAndDragons5.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("DungeonsAndDragons5.toml must parse");
        assert_eq!(
            data.tests.len(),
            74,
            "case count in test/data/DungeonsAndDragons5.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "DungeonsAndDragons5",
                "unexpected game system in DungeonsAndDragons5.toml"
            );
            ...
            match eval_command(
                &GameSystemId::new("DungeonsAndDragons5"),
                &tc.input,
                &mut src,
            ) {
        ...
        assert!(
            failures.is_empty(),
            "{}/{} DungeonsAndDragons5 cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
