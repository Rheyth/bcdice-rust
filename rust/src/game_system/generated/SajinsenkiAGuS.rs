...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/SajinsenkiAGuS.toml");
    ...
    /// `test/data/SajinsenkiAGuS.toml` の全ケースが通ること。
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/SajinsenkiAGuS.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("SajinsenkiAGuS.toml must parse");
        assert_eq!(
            data.tests.len(),
            26,
            "case count in test/data/SajinsenkiAGuS.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "SajinsenkiAGuS",
                "unexpected game system in SajinsenkiAGuS.toml"
            );
            ...
            match eval_command(&GameSystemId::new("SajinsenkiAGuS"), &tc.input, &mut src) {
