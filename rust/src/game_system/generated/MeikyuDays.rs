//! P4で手書き移植した `lib/bcdice/game_system/MeikyuDays.rb`。
...
fn replace_text(string: &str) -> String {
    static MD6: OnceLock<Regex> = OnceLock::new();
    static MD: OnceLock<Regex> = OnceLock::new();
    let md6 = MD6.get_or_init(|| Regex::new(r"(?i)(\d+)MD6").expect("valid regex"));
    let md = MD.get_or_init(|| Regex::new(r"(?i)(\d+)MD").expect("valid regex"));
    let string = md6.replace_all(string, "${1}R6");
    md.replace_all(&string, "${1}R6").into_owned()
}
...
fn result_2d6(
    ...
    Some(match (dice_total, target) {
        (..=2, _) => CheckOutcome::Result(Box::new(EvalResult::fumble("絶対失敗"))),
        (12.., _) => CheckOutcome::Result(Box::new(EvalResult::critical("絶対成功"))),
        (_, Target::Question) => CheckOutcome::Nothing,
        (_, Target::Number(target)) if total >= target => {
            CheckOutcome::Result(Box::new(EvalResult::success("成功")))
        }
        _ => CheckOutcome::Result(Box::new(EvalResult::failure("失敗"))),
    })
}

fn check_roll(command: &str, rng: &mut Randomizer) -> Result<Option<String>, EvalError> {
    let command = replace_text(command);
    let Some(m) = check_roll_pattern().captures(&command) else {
        return Ok(None);
    };

    let matched = &m[1];
    let dice_count = m[2].parse::<i64>().unwrap_or(i64::MAX);
    let bonus = arithmetic::eval(&m[3], RoundType::Floor)?
        .as_ref()
        .map(crate::randomizer::sat_i64)
        .unwrap_or(0);
    let sign = m.get(5).map_or("", |m| m.as_str());
    let target = m
        .get(6)
        .map_or(0, |m| m.as_str().parse::<i64>().unwrap_or(i64::MAX));

    let mut dice = rng.roll_barabara(dice_count, 6)?;
    dice.sort_unstable();
    let dice_now = match dice.as_slice() {
        [one] => one.wrapping_add(*one),
        [.., left, right] => left.wrapping_add(*right),
        [] => 0,
    };
    let total = dice_now.wrapping_add(bonus);
    let dice_text = dice
        .iter()
        .map(i64::to_string)
        .collect::<Vec<_>>()
        .join(",");
    let mut roll_text = format!("{dice_now}[{dice_text}]");
    if bonus > 0 {
        roll_text += &format!("+{bonus}");
    } else if bonus < 0 {
        roll_text += &bonus.to_string();
    }

    let detailed = roll_text
        .bytes()
        .any(|b| !(b.is_ascii_digit() || b == b'[' || b == b']'));
    let mut output = if detailed {
        format!("({matched}) ＞ {roll_text} ＞ {total}")
    } else {
        format!("({matched}) ＞ {total}")
    };

    if let Some(cmp_op) = normalize::comparison_operator(sign) {
        if let Some(CheckOutcome::Result(result)) = result_2d6(
...
fn eval_specific_command(
    ...
    Ok(check_roll(command, rng)?.map(SpecificCommandOutput::text))
}
...
pub struct MeikyuDays;

impl GameSystem for MeikyuDays {
...
"
    ...
    fn prefixes(&self) -> &'static [&'static str] {
        &[
            r"\d+MD6?", r"\d+R6", "DRT", "DBT", "DNT", "DHT", "KST", "CAT", "FWT", "CFT", "APT",
            "DCT", "MCT", "PCT", "LCT", "MPT", "T1T", "T2T", "T3T", "T4T",
        ]
    }
    ...
    fn result_2d6(
        ...
        result_2d6(total, dice_total, cmp_op, target)
    }
