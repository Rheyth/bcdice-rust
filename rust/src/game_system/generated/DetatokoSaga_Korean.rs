...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/DetatokoSaga_Korean.toml");
    ...
    /// `test/data/DetatokoSaga_Korean.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/DetatokoSaga_Korean.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("DetatokoSaga_Korean.toml must parse");
        assert_eq!(
            data.tests.len(),
            42,
            "case count in test/data/DetatokoSaga_Korean.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "DetatokoSaga:Korean",
                "unexpected game system in DetatokoSaga_Korean.toml"
            );
            ...
            match eval_command(
                &GameSystemId::new("DetatokoSaga:Korean"),
                &tc.input,
                &mut src,
            ) {
        ...
        assert!(
            failures.is_empty(),
            "{}/{} DetatokoSaga:Korean cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
