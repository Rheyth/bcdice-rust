...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/Irisbane_Korean.toml");
    ...
    /// `test/data/Irisbane_Korean.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Irisbane_Korean.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Irisbane_Korean.toml must parse");
        assert_eq!(
            data.tests.len(),
            41,
            "case count in test/data/Irisbane_Korean.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Irisbane:Korean",
                "unexpected game system in Irisbane_Korean.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Irisbane:Korean"), &tc.input, &mut src) {
