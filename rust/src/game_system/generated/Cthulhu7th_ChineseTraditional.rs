...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/Cthulhu7th_ChineseTraditional.toml");
        path.exists().then_some(path)
    }

    /// `test/data/Cthulhu7th_ChineseTraditional.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Cthulhu7th_ChineseTraditional.toml not found");
            return;
        };

        let data =
            TestDataFile::load(&path).expect("Cthulhu7th_ChineseTraditional.toml must parse");
        assert_eq!(
            data.tests.len(),
            151,
            "case count in test/data/Cthulhu7th_ChineseTraditional.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Cthulhu7th:ChineseTraditional",
                "unexpected game system in Cthulhu7th_ChineseTraditional.toml"
            );
            ...
            match eval_command(
                &GameSystemId::new("Cthulhu7th:ChineseTraditional"),
                &tc.input,
                &mut src,
            ) {
            ...
            if !reasons.is_empty() {
                failures.push(format!(
                    "FAIL Cthulhu7th_ChineseTraditional:{}:{}\n  - {}",
                    i + 1,
                    tc.input,
                    reasons.join("\n  - ")
                ));
        ...
        assert!(
            failures.is_empty(),
            "{}/{} Cthulhu7th_ChineseTraditional cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
