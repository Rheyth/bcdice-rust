//! P4で手書き移植した `lib/bcdice/game_system/ShinobiGami.rb`。
//!
...
//!
//! 移植したもの:
//! - `ShinobiGami#result_2d6`（ファンブル / スペシャル / 目標値比較）
//! - `#eval_game_system_specific_command` → `action_roll`（`nSG@s#f>=x`）と
//!   `repeat_table`（`n` + 表コマンド）
//! - `DemonSkillTableForMetamorphose`（妖魔化表 `MT` / `NMT` の連鎖項目）
//!
//! # 表データ
//!
//! Ruby側は `I18n.t("ShinobiGami.…", locale:)` で `i18n/ShinobiGami/ja_jp.yml` から表を作る。
//! Rust側は同じ値を `static` として直接持つ。データ部分（`JA_` 接頭辞の `static` 群）は
//! 同YAMLから機械的に書き出したもので、値は1文字も変えていない。
//!
//! ロケール差のあるデータは [`SystemTables`] に束ね、
//! `ShinobiGami_Korean`（`ko_kr`）が同じ関数群を使い回す。
...
/// 引くと文字列を返す表。
///
/// `roll_tables` が引く表は `DiceTable::Table`（`RollResult` を返す）と
/// 妖魔化表（`MT` / `NMT`）が混在し、Ruby側はどちらも `to_s` して使う。
/// 文字列で揃えたトレイトにしておくと1つのテーブル表に並べられる。
...
/// Ruby `ShinobiGami::DemonSkillTableForMetamorphose`。
///
/// 妖魔化表の項目のうち「前置きの説明文＋妖魔忍法表」になっているもの。
pub(crate) struct DemonSkillTableForMetamorphose {
    pretext: &'static str,
    table: &'static Table,
}

impl DemonSkillTableForMetamorphose {
    /// Ruby `DemonSkillTableForMetamorphose.new(pretext, table)`。
    pub(crate) const fn new(pretext: &'static str, table: &'static Table) -> Self {
        Self { pretext, table }
    }
}

impl RollText for DemonSkillTableForMetamorphose {
    /// Ruby `"#{@pretext} ＞ #{@table.roll(randomizer)}"`。
    fn roll_text(&self, rng: &mut Randomizer) -> Result<String, EvalError> {
        Ok(format!("{} ＞ {}", self.pretext, self.table.roll(rng)?))
    }
}

/// 妖魔化表（`MT` / `NMT`）の項目。Ruby `ChainTable` の `Array<String, #roll>`。
pub(crate) enum ChainItem {
    /// 文字列の項目。
    Text(&'static str),
    /// 連鎖先の表。
    Roll(&'static DemonSkillTableForMetamorphose),
}

/// 妖魔化表。Ruby `DiceTable::ChainTable` に `DemonSkillTableForMetamorphose` を並べたもの。
///
/// 共通の [`crate::dice_table::ChainTable`] は連鎖先に `RollResult`（`表名(値) ＞ 本体`）を
/// 要求するが、`DemonSkillTableForMetamorphose#roll` が返すのは表名を持たない
/// `前置き ＞ 表名(値) ＞ 本体` という文字列なので、ここに専用の実装を置く。
pub(crate) struct MetamorphoseChainTable {
    name: &'static str,
    times: i64,
    sides: i64,
    items: &'static [ChainItem],
}

impl MetamorphoseChainTable {
    /// ダイスの個数と面数を直接指定して構築する（Ruby の `'1D6'` 指定に対応）。
    pub(crate) const fn from_dice(
        name: &'static str,
        times: i64,
        sides: i64,
        items: &'static [ChainItem],
...
impl RollText for MetamorphoseChainTable {
    ...
    fn roll_text(&self, rng: &mut Randomizer) -> Result<String, EvalError> {
        ...
        let body = match usize::try_from(index).ok().and_then(|i| self.items.get(i)) {
            Some(ChainItem::Text(text)) => (*text).to_owned(),
            Some(ChainItem::Roll(table)) => table.roll_text(rng)?,
            // 範囲外は Ruby の `@items[index]` が nil になるのと同じく空文字列
            None => String::new(),
        };
...
/// 1ロケール分の表と定型文。`ShinobiGami` と `ShinobiGami_Korean` はこれだけが違う。
pub(crate) struct SystemTables {
    /// Ruby `TABLES`
    pub(crate) tables: &'static [(&'static str, &'static dyn RollText)],
    /// Ruby `SCENE_TABLES`
    pub(crate) scene_tables: &'static [(&'static str, &'static dyn RollText)],
    /// Ruby `DEMON_SKILL_TABLES`
    pub(crate) demon_skill_tables: &'static [(&'static str, &'static dyn RollText)],
    /// Ruby `DEMON_SKILL_TABLES_NEW`
    pub(crate) demon_skill_tables_new: &'static [(&'static str, &'static dyn RollText)],
    /// Ruby `RTT`
...
/// Ruby `ShinobiGami#eval_game_system_specific_command`。
///
/// `action_roll(command) || repeat_table(command)`。
pub(crate) fn eval_specific_command(
    ...
    if let Some(text) = repeat_table(sys, command, rng)? {
...
fn roll_tables(
    tables: &'static [(&'static str, &'static dyn RollText)],
    command: &str,
    rng: &mut Randomizer,
) -> Result<Option<String>, EvalError> {
    match tables.iter().find(|(key, _)| *key == command) {
        None => Ok(None),
        Some((_, table)) => Ok(Some(table.roll_text(rng)?)),
    }
}

/// 先頭の数字列を切り出す。Ruby `command.start_with?(/\d/)` と `command.to_i` に対応する。
///
/// Ruby の `to_i` は多倍長だが、Rustでは `i64` に飽和させる
/// （実際のコマンドで桁あふれするものは無く、あってもダイスを振り切れずエラーになる）。
fn leading_number(command: &str) -> (Option<i64>, &str) {
    let digits = command.len()
        - command
    ...
    let value = command[..digits].parse::<i64>().unwrap_or(i64::MAX);
    (Some(value), &command[digits..])
}

/// Ruby `ShinobiGami#repeat_table`。
///
/// 「回数+コマンド」で同じ表を複数回引き、`"\n"` で連結する。
fn repeat_table(
    sys: &SystemTables,
    ...
    // Ruby: times = command.start_with?(/\d/) ? command.to_i : 1
    //       key   = command.sub(/^\d+/, '')
    let (times, key) = leading_number(command);
    let times = times.unwrap_or(1);

    let mut results: Vec<String> = Vec::new();
    for _ in 0..times {
        let rolled = match roll_tables(sys.tables, key, rng)? {
            Some(text) => Some(text),
            None => match roll_tables(sys.scene_tables, key, rng)? {
                Some(text) => Some(text),
                None => match roll_tables(sys.demon_skill_tables, key, rng)? {
                    Some(text) => Some(text),
                    None => match roll_tables(sys.demon_skill_tables_new, key, rng)? {
                        Some(text) => Some(text),
                        None => sys.rtt.roll_command(rng, key)?,
                    },
                },
            },
        };
        // Ruby: `.compact` で nil を落とす
        if let Some(text) = rolled {
            results.push(text);
        }
    }
...
/// Ruby `ShinobiGami#action_roll`（`nSG@s#f>=x`）。
fn action_roll(
    sys: &SystemTables,
    command: &str,
    rng: &mut Randomizer,
    ...
    let parser = PARSER.get_or_init(|| {
        Parser::new(&[r"\d*SG"], RoundType::Floor)
    ...
    // Ruby: times = cmd.command.start_with?(/\d/) ? cmd.command.to_i : 2
    let times = leading_number(&cmd.command).0.unwrap_or(2);
    // ダイスを振る前に打ち切る（Ruby も roll_barabara より前で return する）
    if times <= 1 {
        return Ok(None);
    }

    // Ruby: cmd.critical ||= 12 / cmd.fumble ||= 2（`to_s` にも効くので先に代入する）
    let critical = sat_i64(cmd.critical.get_or_insert(I::from(12)));
    let fumble = sat_i64(cmd.fumble.get_or_insert(I::from(2)));
    ...
    let mut result = if dice_total <= fumble {
        ...
        EvalResult::critical(sys.special)
    } else if let (Some(_), Some(target_number)) = (cmd.cmp_op, cmd.target_number.clone()) {
        // 目標値は比較演算子と対でしか入らないので、ここが Ruby の
        // `total >= cmd.target_number` に対応する
        ...
        // Ruby: elsif cmd.cmp_op.nil? -> Result.new（テキストなし）
        EvalResult::new()
    };

    let mut sequence = vec![format!("({})", cmd.to_s(SuffixPosition::AfterModifyNumber))];
    // Ruby: dice_list_full_str は times <= 2 のとき nil なので `.compact` で落ちる
...
// ---------------------------------------------------------------------------
// ja_jp ロケールの表データ（i18n/ShinobiGami/ja_jp.yml から機械的に書き出したもの）
// ---------------------------------------------------------------------------

static JA_DSA_ITEMS: &[&str] = &[
    "【震々】(怪p.252／基本p.172)",
    "【神隠】(怪p.252／基本p.172)",
    "【夜雀】(怪p.252／基本p.172)",
    "【猟犬】(怪p.252／基本p.172)",
...
static JA_DSA: Table = Table::from_dice("妖魔忍法表A", 1, 6, JA_DSA_ITEMS);

static JA_DSB_ITEMS: &[&str] = &[
    "【野衾】(怪p.253／基本p.172)",
    "【付喪神】(怪p.253／基本p.172)",
    "【見越】(怪p.253／基本p.172)",
    "【木魂】(怪p.253／基本p.172)",
...
static JA_DSB: Table = Table::from_dice("妖魔忍法表B", 1, 6, JA_DSB_ITEMS);

static JA_DSC_ITEMS: &[&str] = &[
    "【百眼】(怪p.254／基本p.173)",
    "【呑口】(怪p.254／基本p.173)",
    "【荒吐】(怪p.254／基本p.173)",
    "【怨霊】(怪p.254／基本p.173)",
...
static JA_DSC: Table = Table::from_dice("妖魔忍法表C", 1, 6, JA_DSC_ITEMS);

static JA_DSN1_ITEMS: &[&str] = &[
...
static JA_DSN1: Table = Table::from_dice("妖魔忍法表・異霊", 1, 6, JA_DSN1_ITEMS);

static JA_DSN2_ITEMS: &[&str] = &[
...
static JA_DSN2: Table = Table::from_dice("妖魔忍法表・凶身", 1, 6, JA_DSN2_ITEMS);

static JA_DSN3_ITEMS: &[&str] = &[
    "【鵺】(基本 p172)／【朱盤】（隠忍 p29）",
...
static JA_DSN3: Table = Table::from_dice("妖魔忍法表・神化", 1, 6, JA_DSN3_ITEMS);

static JA_DSN4_ITEMS: &[&str] = &[
    "【黒手】(隠忍 p28)／【強威】（隠忍 p25）",
    "【業弓】(隠忍 p28)／【強威】（隠忍 p25）",
    "【怪病】(隠忍 p28)／【強威】（隠忍 p25）",
    "【鏖殺】(隠忍 p28)／【強威】（隠忍 p25）",
...
static JA_DSN4: Table = Table::from_dice("妖魔忍法表・攻激", 1, 6, JA_DSN4_ITEMS);

static JA_MT_DSA: DemonSkillTableForMetamorphose =
    DemonSkillTableForMetamorphose::new("1D6を振り、「妖魔忍法表A」で、ランダムに忍法の種類を決定する。妖魔化している間、その妖魔忍法を修得しているものとして扱う。この異形は、違う種類の妖魔忍法である限り、違う異形として扱う。", &JA_DSA);
static JA_MT_DSB: DemonSkillTableForMetamorphose =
    DemonSkillTableForMetamorphose::new("1D6を振り、「妖魔忍法表B」で、ランダムに忍法の種類を決定する。妖魔化している間、その妖魔忍法を修得しているものとして扱う。この異形は、違う種類の妖魔忍法である限り、違う異形として扱う。", &JA_DSB);
static JA_MT_DSC: DemonSkillTableForMetamorphose =
    DemonSkillTableForMetamorphose::new("1D6を振り、「妖魔忍法表C」で、ランダムに忍法の種類を決定する。妖魔化している間、その妖魔忍法を修得しているものとして扱う。この異形は、違う種類の妖魔忍法である限り、違う異形として扱う。", &JA_DSC);
static JA_MT_ITEMS: &[ChainItem] = &[
    ChainItem::Roll(&JA_MT_DSA),
    ChainItem::Roll(&JA_MT_DSB),
    ChainItem::Roll(&JA_MT_DSC),
    ChainItem::Text("妖魔化している間、戦闘中、1ラウンドに使用できる忍法のコストが、自分のプロット値+3点になり、装備忍法の【揺音】を修得する。"),
    ChainItem::Text("妖魔化している間、【接近戦攻撃】によって与える接近戦ダメージが2点になる。"),
    ChainItem::Text("妖魔化している間、このキャラクターの攻撃に対する回避判定と、このキャラクターの奥義に対する奥義破り判定にマイナス1の修正がつく。"),
];
static JA_MT: MetamorphoseChainTable =
    MetamorphoseChainTable::from_dice("異形表", 1, 6, JA_MT_ITEMS);

static JA_NMT_DSN1: DemonSkillTableForMetamorphose = DemonSkillTableForMetamorphose::new(
    "異霊態。「妖魔忍法表・異霊」を使用し、妖魔化している間、その妖魔忍法を修得する。",
    &JA_DSN1,
);
static JA_NMT_DSN2: DemonSkillTableForMetamorphose = DemonSkillTableForMetamorphose::new(
    "凶身態。「妖魔忍法表・凶身」を使用し、妖魔化している間、その妖魔忍法を修得する。",
    &JA_DSN2,
);
static JA_NMT_DSN3: DemonSkillTableForMetamorphose = DemonSkillTableForMetamorphose::new(
    "神化態。「妖魔忍法表・神化」を使用し、妖魔化している間、その妖魔忍法を修得する。",
    &JA_DSN3,
);
static JA_NMT_DSN4: DemonSkillTableForMetamorphose = DemonSkillTableForMetamorphose::new(
    "攻激態。「妖魔忍法表・攻激」を使用し、妖魔化している間、その妖魔忍法を修得する。",
    &JA_DSN4,
);
static JA_NMT_ITEMS: &[ChainItem] = &[
    ChainItem::Roll(&JA_NMT_DSN1),
    ChainItem::Roll(&JA_NMT_DSN2),
    ChainItem::Roll(&JA_NMT_DSN3),
    ChainItem::Roll(&JA_NMT_DSN4),
    ChainItem::Text("業魔態。妖魔化している間、戦闘中、各ラウンドに使用できる忍法のコストの合計が、自分のプロット値＋３点になり、装備忍法の【揺音】（基本 p85）を修得する。"),
    ChainItem::Text("不視態。妖魔化している間、このキャラクターの攻撃に対する回避判定と、このキャラクターの奥義に対する奥義破り判定にマイナス１の修正がつく。"),
];
static JA_NMT: MetamorphoseChainTable =
    MetamorphoseChainTable::from_dice("新異形表", 1, 6, JA_NMT_ITEMS);

static JA_MTR_ITEMS: &[&str] = &[
...
static JA_MTR: Table = Table::from_dice("異形表", 1, 6, JA_MTR_ITEMS);

static JA_NMTR_ITEMS: &[&str] = &[
    "異霊態。「妖魔忍法表・異霊」を使用し、妖魔化している間、その妖魔忍法を修得する。",
    "凶身態。「妖魔忍法表・凶身」を使用し、妖魔化している間、その妖魔忍法を修得する。",
    "神化態。「妖魔忍法表・神化」を使用し、妖魔化している間、その妖魔忍法を修得する。",
    "攻激態。「妖魔忍法表・攻激」を使用し、妖魔化している間、その妖魔忍法を修得する。",
...
static JA_NMTR: Table = Table::from_dice("新異形表", 1, 6, JA_NMTR_ITEMS);
...
static JA_OFT: Table = Table::from_dice("ファンブル表（旧文庫版）", 1, 6, JA_OFT_ITEMS);

static JA_KFT_ITEMS: &[&str] = &[
    "何か調子がおかしい。そのサイクルの間、すべての行為判定にマイナス１の修正がつく。",
    "しまった！　好きな忍具を１つ失ってしまう。",
    "情報が漏れる！　あなた以外のキャラクターは、あなたの持っている【秘密】か【居所】の中から、好きなものをそれぞれ一つ知ることができる。",
    "油断した！　術の制御に失敗し、好きな【生命力】を１点失う。",
...
static JA_KFT: Table = Table::from_dice("怪ファンブル表", 1, 6, JA_KFT_ITEMS);

static JA_ET_ITEMS: &[&str] = &[
    "共感（プラス）／不信（マイナス）",
    "友情（プラス）／怒り（マイナス）",
    "愛情（プラス）／妬み（マイナス）",
    "忠誠（プラス）／侮蔑（マイナス）",
...
static JA_ET: Table = Table::from_dice("感情表", 1, 6, JA_ET_ITEMS);

static JA_WT_ITEMS: &[&str] = &[
...
static JA_WT: Table = Table::from_dice("変調表", 1, 6, JA_WT_ITEMS);
...
static JA_OWT: Table = Table::from_dice("変調表（旧文庫版）", 1, 6, JA_OWT_ITEMS);

static JA_KWT_ITEMS: &[&str] = &[
    "故障:すべての忍具が使用不能になる。この効果は累積しない。各サイクルの終了時に、《絡繰術》で行為判定を行い、成功するとこの変調は無効化される。",
    "マヒ:修得している特技の中からランダムに一つを選び、その特技が使用不能になる。この効果は、修得している特技の数だけ累積する。各サイクルの終了時に、《身体操術》で行為判定を行い、成功するとこの変調はすべて無効化される。",
    "重傷:命中判定、情報判定、感情判定を行うたびに、接近戦ダメージを１点受ける。この効果は累積しない。各サイクルの終了時に、《生存術》で行為判定を行い、成功するとこの変調は無効化される。",
    "行方不明:メインフェイズ中、自分以外がシーンプレイヤーのシーンに登場することができなくなる。この効果は累積しない。各サイクルの終了時に、《経済力》で行為判定を行い、成功するとこの変調は無効化される。",
...
static JA_KWT: Table = Table::from_dice("怪変調表", 1, 6, JA_KWT_ITEMS);

static JA_GWT_ITEMS: &[&str] = &[
...
static JA_GWT: Table = Table::from_dice("戦国変調表", 1, 6, JA_GWT_ITEMS);

static JA_OBT_ITEMS: &[&str] = &[
    "平地:特になし。",
...
static JA_OBT: Table = Table::from_dice("戦場表（旧文庫版）", 1, 6, JA_OBT_ITEMS);
...
static JA_BT: Table = Table::from_dice("戦場表", 1, 6, JA_BT_ITEMS);

static JA_PT_ITEMS: &[&str] = &[
...
static JA_PT: Table = Table::from_dice("プライズ効果表", 2, 6, JA_PT_ITEMS);

static JA_BNT_ITEMS: &[&str] = &[
...
static JA_BNT: Table = Table::from_dice("戦場新効果表", 1, 6, JA_BNT_ITEMS);

static JA_YWT_ITEMS: &[&str] = &[
    "器術：故障（現代）／催眠（戦国）",
    "体術：マヒ（現代）／火達磨（戦国）",
    "忍術：重傷（現代）／猛毒（戦国）",
    "謀術：行方不明（現代）／飢餓（戦国）",
...
static JA_YWT: Table = Table::from_dice("妖術変調対応表（現代／戦国）", 1, 6, JA_YWT_ITEMS);

static JA_YWT1_ITEMS: &[&str] = &[
    "器術：故障",
    "体術：マヒ",
    "忍術：重傷",
    "謀術：行方不明",
...
static JA_YWT1: Table = Table::from_dice("妖術変調対応表（現代）", 1, 6, JA_YWT1_ITEMS);

static JA_YWT2_ITEMS: &[&str] = &[
    "器術：催眠",
    "体術：火達磨",
    "忍術：猛毒",
    "謀術：飢餓",
...
static JA_YWT2: Table = Table::from_dice("妖術変調対応表（戦国）", 1, 6, JA_YWT2_ITEMS);

static JA_OTS_ITEMS: &[&str] = &[
    "「兵糧丸」を一個獲得する。",
    "「神通丸」を一個獲得する。",
    "「遁甲符」を一個獲得する。",
    "戦場を好きな場所に変更する。",
...
static JA_OTS: Table = Table::from_dice("【数奇】（御斎 p33）", 1, 6, JA_OTS_ITEMS);

static JA_HRPT_ITEMS: &[&str] = &[
...
static JA_HRPT: Table = Table::from_dice("パニック表", 2, 6, JA_HRPT_ITEMS);

static JA_OTAT_ITEMS: &[&str] = &[
...
static JA_OTAT: Table = Table::from_dice("覚醒表", 1, 6, JA_OTAT_ITEMS);

static JA_ONDT_ITEMS: &[&str] = &[
...
static JA_ONDT: Table = Table::from_dice("出物表", 2, 6, JA_ONDT_ITEMS);

static JA_NCT1_ITEMS: &[&str] = &[
...
static JA_NCT1: Table = Table::from_dice("攻撃系忍法授業シーン表", 1, 6, JA_NCT1_ITEMS);

static JA_NCT2_ITEMS: &[&str] = &[
...
static JA_NCT2: Table = Table::from_dice("防御系忍法授業シーン表", 1, 6, JA_NCT2_ITEMS);

static JA_NCT3_ITEMS: &[&str] = &[
...
static JA_NCT3: Table = Table::from_dice("戦略系忍法授業シーン表", 1, 6, JA_NCT3_ITEMS);

static JA_S_ST_ITEMS: &[&str] = &[
...
static JA_S_ST: Table = Table::from_dice("シーン表", 2, 6, JA_S_ST_ITEMS);

static JA_S_CST_ITEMS: &[&str] = &[
...
static JA_S_CST: Table = Table::from_dice("都市シーン表", 2, 6, JA_S_CST_ITEMS);

static JA_S_MST_ITEMS: &[&str] = &[
...
static JA_S_MST: Table = Table::from_dice("館シーン表", 2, 6, JA_S_MST_ITEMS);

static JA_S_DST_ITEMS: &[&str] = &[
...
static JA_S_DST: Table = Table::from_dice("出島シーン表", 2, 6, JA_S_DST_ITEMS);

static JA_S_TST_ITEMS: &[&str] = &[
...
static JA_S_TST: Table = Table::from_dice("トラブルシーン表", 2, 6, JA_S_TST_ITEMS);

static JA_S_NST_ITEMS: &[&str] = &[
...
static JA_S_NST: Table = Table::from_dice("日常シーン表", 2, 6, JA_S_NST_ITEMS);

static JA_S_TKST_ITEMS: &[&str] = &[
...
static JA_S_TKST: Table = Table::from_dice("東京シーン表", 2, 6, JA_S_TKST_ITEMS);

static JA_S_KST_ITEMS: &[&str] = &[
...
static JA_S_KST: Table = Table::from_dice("回想シーン表", 2, 6, JA_S_KST_ITEMS);

static JA_S_GST_ITEMS: &[&str] = &[
...
static JA_S_GST: Table = Table::from_dice("戦国シーン表", 2, 6, JA_S_GST_ITEMS);

static JA_S_GAST_ITEMS: &[&str] = &[
...
static JA_S_GAST: Table = Table::from_dice("学校シーン表", 2, 6, JA_S_GAST_ITEMS);

static JA_S_KYST_ITEMS: &[&str] = &[
...
static JA_S_KYST: Table = Table::from_dice("京都シーン表", 2, 6, JA_S_KYST_ITEMS);

static JA_S_JBST_ITEMS: &[&str] = &[
...
static JA_S_JBST: Table = Table::from_dice("神社仏閣シーン表", 2, 6, JA_S_JBST_ITEMS);

static JA_S_AKST_ITEMS: &[&str] = &[
    ...
    "神秘的な神社。祭りの準備が進められているが、今は人がいないようだ。",
...
static JA_S_AKST: Table = Table::from_dice("秋空に雪舞えばシーン表", 2, 6, JA_S_AKST_ITEMS);

static JA_S_CLST_ITEMS: &[&str] = &[
...
static JA_S_CLST: Table = Table::from_dice("災厄シーン表", 1, 6, JA_S_CLST_ITEMS);

static JA_S_DXST_ITEMS: &[&str] = &[
    "迷宮街。いつから囚われてしまったのだろう？何重にも交差し、曲がりくねった道を歩き続ける。このシーンの登場人物は《記憶術》で判定を行わなければならない。成功すると、迷宮の果てで好きな忍具を一つ獲得する。失敗すると、行方不明の変調を受ける。",
    "幻影城。訪れた者の過去や未来の風景を見せる場所。このシーンの登場人物は、《意気》の判定を行うことができる。成功すると、自分の持っている【感情】を好きな何かに変更することができる。",
    "死者たちの行進。無念の死を遂げた者たちが、仲間を求めて彷徨らっている。このシーンの登場人物は《死霊術》で判定を行わなければならない。失敗すると、ランダムに変調を一つを受ける。",
    "スラム。かろうじて生き延びている人たちが肩を寄せ合い生きているようだ。ここなら辛うじて安心できるかも……。",
...
static JA_S_DXST: Table = Table::from_dice("出島EXシーン表", 2, 6, JA_S_DXST_ITEMS);

static JA_S_HC_ITEMS: &[&str] = &[
...
static JA_S_HC: Table = Table::from_dice("中忍試験シーン表", 2, 6, JA_S_HC_ITEMS);

static JA_S_HK_ITEMS: &[&str] = &[
    "血の臭いがあたりに充満している。何者かの戦いがあった気配。　いや？まだ戦いは続いているのだろうか？",
    "これは……夢か？　もう終わったはずの過去。しかし、それを忘れることはできない。",
    "眼下に広がる街並みを眺める。ここからなら街を一望できるが……。",
    "世界の終わりのような暗黒。暗闇の中、お前達は密やかに囁く。",
...
static JA_S_HK: Table = Table::from_dice("影の街でシーン表", 2, 6, JA_S_HK_ITEMS);

static JA_S_HLST_ITEMS: &[&str] = &[
...
static JA_S_HLST: Table = Table::from_dice("斜歯ラボシーン表", 2, 6, JA_S_HLST_ITEMS);

static JA_S_HM_ITEMS: &[&str] = &[
...
static JA_S_HM: Table = Table::from_dice("密室シーン表", 2, 6, JA_S_HM_ITEMS);

static JA_S_HO_ITEMS: &[&str] = &[
...
static JA_S_HO: Table = Table::from_dice("病院シーン表", 2, 6, JA_S_HO_ITEMS);

static JA_S_HR_ITEMS: &[&str] = &[
...
static JA_S_HR: Table = Table::from_dice("龍動シーン表", 2, 6, JA_S_HR_ITEMS);

static JA_S_HS_ITEMS: &[&str] = &[
...
static JA_S_HS: Table = Table::from_dice("催眠シーン表", 1, 6, JA_S_HS_ITEMS);

static JA_S_HT_ITEMS: &[&str] = &[
    "血の臭いがあたりに充満している。何者かの戦いがあった気配。　いや？まだ戦いは続いているのだろうか？",
    "これは……夢か？　もう終わったはずの過去。しかし、それを忘れることはできない。",
    "眼下に広がる街並みを眺める。ここからなら街を一望できるが……。",
    "世界の終わりのような暗黒。暗闇の中、お前達は密やかに囁く。",
...
static JA_S_HT: Table = Table::from_dice("滅びの塔シーン表", 2, 6, JA_S_HT_ITEMS);

static JA_S_HY_ITEMS: &[&str] = &[
...
static JA_S_HY: Table = Table::from_dice("夜行列車シーン表", 2, 6, JA_S_HY_ITEMS);

static JA_S_NTST_ITEMS: &[&str] = &[
...
static JA_S_NTST: Table = Table::from_dice("夏の終わりシーン表", 2, 6, JA_S_NTST_ITEMS);

static JA_S_PLST_ITEMS: &[&str] = &[
...
static JA_S_PLST: Table = Table::from_dice("培養プラントシーン表", 1, 6, JA_S_PLST_ITEMS);

static JA_S_TMT_ITEMS: &[&str] = &[
...
static JA_S_TMT: Table = Table::from_dice("魔都東京シーン表", 2, 6, JA_S_TMT_ITEMS);

static JA_S_TMG_ITEMS: &[&str] = &[
...
static JA_S_TMG: Table = Table::from_dice("魔都学園シーン表", 2, 6, JA_S_TMG_ITEMS);

static JA_S_TC_ITEMS: &[&str] = &[
...
static JA_S_TC: Table = Table::from_dice("カジノシーン表", 2, 6, JA_S_TC_ITEMS);

static JA_S_TGS_ITEMS: &[&str] = &[
...
static JA_S_TGS: Table = Table::from_dice("月天に死の咲くシーン表", 2, 6, JA_S_TGS_ITEMS);

static JA_S_TRM_ITEMS: &[&str] = &[
...
static JA_S_TRM: Table = Table::from_dice("ロードムービーシーン表", 2, 6, JA_S_TRM_ITEMS);

static JA_S_TMC_ITEMS: &[&str] = &[
...
static JA_S_TMC: Table = Table::from_dice("マスカレイド・キャッスルシーン表", 2, 6, JA_S_TMC_ITEMS);

static JA_S_TKG_ITEMS: &[&str] = &[
    "清廉な気配が漂う森の中。鳥のさえずりやそよ風が木々を通りすぎる音が聞こえる。",
    "学校のトイレ。……なんだか少しだけ怖い気がする。",
...
static JA_S_TKG: Table = Table::from_dice("学校シーン表（黒星祭）", 2, 6, JA_S_TKG_ITEMS);

static JA_S_TKH_ITEMS: &[&str] = &[
...
static JA_S_TKH: Table = Table::from_dice("恋人との日々シーン表", 2, 6, JA_S_TKH_ITEMS);

static JA_S_OTFK_ITEMS: &[&str] = &[
...
static JA_S_OTFK: Table = Table::from_dice("不良高校シーン表", 2, 6, JA_S_OTFK_ITEMS);
...
/// Ruby `SCENE_TABLES`。
static JA_SCENE_TABLES: &[(&str, &dyn RollText)] = &[
    ("ST", &JA_S_ST),
    ("CST", &JA_S_CST),
    ("MST", &JA_S_MST),
    ("DST", &JA_S_DST),
...
/// Ruby `DEMON_SKILL_TABLES`。
static JA_DEMON_SKILL_TABLES: &[(&str, &dyn RollText)] =
    &[("DSA", &JA_DSA), ("DSB", &JA_DSB), ("DSC", &JA_DSC)];

/// Ruby `DEMON_SKILL_TABLES_NEW`。
static JA_DEMON_SKILL_TABLES_NEW: &[(&str, &dyn RollText)] = &[
    ("DSN1", &JA_DSN1),
    ("DSN2", &JA_DSN2),
    ("DSN3", &JA_DSN3),
    ("DSN4", &JA_DSN4),
...
static JA_RTT_CATEGORIES: &[SaiFicCategory] = &[
    SaiFicCategory::new("器術", JA_RTT_SKILLS1),
    SaiFicCategory::new("体術", JA_RTT_SKILLS2),
    SaiFicCategory::new("忍術", JA_RTT_SKILLS3),
    SaiFicCategory::new("謀術", JA_RTT_SKILLS4),
    SaiFicCategory::new("戦術", JA_RTT_SKILLS5),
    SaiFicCategory::new("妖術", JA_RTT_SKILLS6),
];
static JA_RTT: SaiFicSkillTable =
    SaiFicSkillTable::new(JA_RTT_CATEGORIES).with_formats(SaiFicFormats {
        rtt: "ランダム指定特技表(%<category_dice>d,%<row_dice>d) ＞ %<text>s",
        rct: DEFAULT_RCT_FORMAT,
        rttn: DEFAULT_RTTN_FORMAT,
        skill: "『%<category_name>s』%<skill_name>s",
    });

/// `ja_jp` ロケールの表と定型文一式。
pub(crate) static JA_SYSTEM: SystemTables = SystemTables {
    tables: JA_TABLES,
    scene_tables: JA_SCENE_TABLES,
    demon_skill_tables: JA_DEMON_SKILL_TABLES,
    demon_skill_tables_new: JA_DEMON_SKILL_TABLES_NEW,
    rtt: &JA_RTT,
    success: "成功",
    failure: "失敗",
    fumble: "ファンブル",
...
/// Ruby `BCDice::GameSystem::ShinobiGami`（ID: `ShinobiGami`）。
///
/// 表とメッセージは `ja_jp` ロケール。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ShinobiGami;

impl GameSystem for ShinobiGami {
...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/ShinobiGami.toml");
    ...
    /// `test/data/ShinobiGami.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/ShinobiGami.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("ShinobiGami.toml must parse");
        assert_eq!(
            data.tests.len(),
            164,
            "case count in test/data/ShinobiGami.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "ShinobiGami",
                "unexpected game system in ShinobiGami.toml"
            );
            ...
            match eval_command(&GameSystemId::new("ShinobiGami"), &tc.input, &mut src) {
