...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/KutuluRevised.toml");
    ...
    /// `test/data/KutuluRevised.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/KutuluRevised.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("KutuluRevised.toml must parse");
        assert_eq!(
            data.tests.len(),
            13,
            "case count in test/data/KutuluRevised.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "KutuluRevised",
                "unexpected game system in KutuluRevised.toml"
            );
            ...
            match eval_command(&GameSystemId::new("KutuluRevised"), &tc.input, &mut src) {
