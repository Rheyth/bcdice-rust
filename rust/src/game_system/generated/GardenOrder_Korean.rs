...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/GardenOrder_Korean.toml");
    ...
    /// `test/data/GardenOrder_Korean.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/GardenOrder_Korean.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("GardenOrder_Korean.toml must parse");
        assert_eq!(
            data.tests.len(),
            52,
            "case count in test/data/GardenOrder_Korean.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "GardenOrder:Korean",
                "unexpected game system in GardenOrder_Korean.toml"
            );
            ...
            match eval_command(
                &GameSystemId::new("GardenOrder:Korean"),
                &tc.input,
                &mut src,
            ) {
        ...
        assert!(
            failures.is_empty(),
            "{}/{} GardenOrder_Korean cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
