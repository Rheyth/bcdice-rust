...
mod tests {
    ...
    /// `test/data/Garako.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Garako.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Garako.toml must parse");
        assert_eq!(data.tests.len(), 52, "case count in test/data/Garako.toml");

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Garako",
                "unexpected game system in Garako.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Garako"), &tc.input, &mut src) {
