...
mod tests {
    ...
    /// `test/data/Insane.toml` の全ケースが通ること。
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/Insane.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Insane.toml must parse");
        assert_eq!(data.tests.len(), 63, "case count in test/data/Insane.toml");

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Insane",
                "unexpected game system in Insane.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Insane"), &tc.input, &mut src) {
