...
mod tests {
    ...
    /// `test/data/Chill.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Chill.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Chill.toml must parse");
        assert_eq!(data.tests.len(), 203, "case count in test/data/Chill.toml");

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Chill",
                "unexpected game system in Chill.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Chill"), &tc.input, &mut src) {
