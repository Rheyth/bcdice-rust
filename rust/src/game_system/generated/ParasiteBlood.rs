//! P4で手書き移植した `lib/bcdice/game_system/ParasiteBlood.rb`。
//!
...
//!
//! 移植したもの:
//! - `DemonParasite#result_nd6`（1の目2個でファンブル / 6の目2個でクリティカル）
//!   — `ParasiteBlood < DemonParasite` なので継承分もここに書く
//! - `DemonParasite#initialize` の `@sort_add_dice = true`
//!   （`@d66_sort_type = D66SortType::NO_SORT` はトレイトの既定値と同じなので省略）
//! - `ParasiteBlood#get_urge`（衝動表 `URGEx` / 誤作動表 `AURGEx`）
//!
//! 表データ（`PB_NORMAL_URGE_TABLE` / `PB_AAS_URGE_TABLE`）は原典 rb から
...
/// Ruby `BCDice::GameSystem::ParasiteBlood`（ID: `ParasiteBlood`）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ParasiteBlood;

impl GameSystem for ParasiteBlood {
...
"#
    ...
    /// Ruby `DemonParasite#initialize` の `@sort_add_dice = true`。
    fn sort_add_dice(&self) -> bool {
        true
    }

    /// Ruby `DemonParasite#result_nd6`。
    ///
    /// どの枝にも当たらない場合、Ruby の `if` は `nil` を返す（＝判定なし）。
    fn result_nd6(
        &self,
        _total: crate::Int,
        _dice_total: i64,
        ...
        if value_list.iter().filter(|d| **d == 1).count() >= 2 {
            // １の目が２個以上ならファンブル
            Some(CheckOutcome::Result(Box::new(EvalResult::fumble(
                "致命的失敗",
            ))))
        } else if value_list.iter().filter(|d| **d == 6).count() >= 2 {
            // ６の目が２個以上あったらクリティカル
            Some(CheckOutcome::Result(Box::new(EvalResult::critical(
                "効果的成功",
            ))))
        } else if target == Target::Question {
    ...
    /// Ruby `DemonParasite#eval_game_system_specific_command` → `get_urge`
    /// （`get_urge` は `ParasiteBlood` 側の定義で上書きされている）。
    fn eval_game_system_specific_command(
        &self,
        command: &str,
        rng: &mut Randomizer,
    ) -> Result<Option<SpecificCommandOutput>, EvalError> {
        get_urge(command, rng)
    }
}

/// Ruby `ParasiteBlood#get_urge` の `/(\w*)URGE\s*(\d+)/i`。
///
/// Ruby の `\w` / `\s` は ASCII 限定なので、Unicode 既定の `regex` クレートでは
/// 文字クラスを明示する。`(\w*)` は貪欲で、`AURGE1` では `A` だけが残る。
fn urge_pattern() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| {
        Regex::new(r"(?i)([0-9A-Za-z_]*)URGE[\t\n\x0B\x0C\r ]*([0-9]+)").expect("valid regex")
    })
}

/// Ruby `ParasiteBlood#get_urge`。パラサイトブラッドの衝動表。
fn get_urge(
    string: &str,
    rng: &mut Randomizer,
) -> Result<Option<SpecificCommandOutput>, EvalError> {
    let Some(captures) = urge_pattern().captures(string) else {
        return Ok(Some(SpecificCommandOutput::text("1")));
    };

    let initial_word = &captures[1];
    // Ruby: `Regexp.last_match(2).to_i`。桁あふれは Bignum になるが、
    // どのみち 1..5 の範囲外なのでエラー文言に落ちる。
    let urge_lv: i64 = captures[2].parse().unwrap_or(i64::MAX);

    // Ruby: `when ""` → 衝動表、`when /A/i` → 誤作動表、それ以外（あり得ない文字）→ 衝動表
    let urge_type = if initial_word.is_empty() {
        UrgeType::Normal
    } else if initial_word.contains(['A', 'a']) {
        UrgeType::Aas
    } else {
        UrgeType::Normal
    };

    if !(1..=5).contains(&urge_lv) {
        return Ok(Some(SpecificCommandOutput::text("衝動段階は1から5です")));
    }

    let dice_now = rng.roll_sum(2, 6)?;
    let urge = get_pb_urge_table(urge_lv, dice_now, urge_type);
    let title = match urge_type {
        UrgeType::Normal => "衝動表",
        UrgeType::Aas => "誤作動表",
    };

    Ok(Some(SpecificCommandOutput::text(format!(
        "{title}{urge_lv}-{dice_now}:{urge}"
    ))))
}

/// Ruby `urge_type`（1: 衝動表、2: 誤作動表）。
///
/// Ruby には `0` と `3以上` の分岐もあるが、`urge_type` に入るのは 1 か 2 だけなので
/// 到達しない。到達しない値を持たない列挙で表す。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum UrgeType {
    Normal,
    Aas,
...
/// Ruby `ParasiteBlood#get_pb_urge_table`: `table[level - 1][dice - 2]`。
fn get_pb_urge_table(level: i64, dice: i64, urge_type: UrgeType) -> &'static str {
    let table = match urge_type {
        UrgeType::Normal => &PB_NORMAL_URGE_TABLE,
        UrgeType::Aas => &PB_AAS_URGE_TABLE,
    };
    // level は 1..=5、dice は 2..=12 に収まるので添字は必ず有効
    table[(level - 1) as usize][(dice - 2) as usize]
}

/// Ruby `ParasiteBlood#get_pb_normal_urge_table`（衝動表 / 2D6）。
///
/// 添字は `[衝動段階 - 1][2D6の出目 - 2]`。
static PB_NORMAL_URGE_TABLE: [[&str; 11]; 5] = [
    [
...
/// Ruby `ParasiteBlood#get_pb_aas_urge_table`（誤作動表 / 2D6）。
///
/// 添字は `[衝動段階 - 1][2D6の出目 - 2]`。
static PB_AAS_URGE_TABLE: [[&str; 11]; 5] = [
    [
...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/ParasiteBlood.toml");
    ...
    /// `test/data/ParasiteBlood.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/ParasiteBlood.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("ParasiteBlood.toml must parse");
        assert_eq!(
            data.tests.len(),
            150,
            "case count in test/data/ParasiteBlood.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "ParasiteBlood",
                "unexpected game system in ParasiteBlood.toml"
            );
            ...
            match eval_command(&GameSystemId::new("ParasiteBlood"), &tc.input, &mut src) {
