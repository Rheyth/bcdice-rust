...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/ConvictorDrive.toml");
    ...
    /// `test/data/ConvictorDrive.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/ConvictorDrive.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("ConvictorDrive.toml must parse");
        assert_eq!(
            data.tests.len(),
            10,
            "case count in test/data/ConvictorDrive.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "ConvictorDrive",
                "unexpected game system in ConvictorDrive.toml"
            );
            ...
            match eval_command(&GameSystemId::new("ConvictorDrive"), &tc.input, &mut src) {
