...
mod tests {
    ...
    /// `test/data/Revulture.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Revulture.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Revulture.toml must parse");
        assert_eq!(
            data.tests.len(),
            38,
            "case count in test/data/Revulture.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Revulture",
                "unexpected game system in Revulture.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Revulture"), &tc.input, &mut src) {
