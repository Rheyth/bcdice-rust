//! P4で手書き移植した `lib/bcdice/game_system/HuntersMoon.rb`。
//!
...
//!
//! 移植したもの:
//! - `HuntersMoon#result_2d6`（2D6判定のファンブル・スペシャル）
//! - `HuntersMoon#roll_strange_ability_table` と
//!   `StrangeAbilityTable1` / `StrangeAbilityTable2`（異形アビリティー表 `SATx` / `SA2Tx`）
//! - `HuntersMoon::TABLES` と `Base#roll_tables`（各種表）
//! - `HuntersMoon::RTT`（指定特技表）
...
use crate::dice_table::sai_fic_skill_table::{
    DEFAULT_RCT_FORMAT, DEFAULT_RTT_FORMAT, DEFAULT_SKILL_FORMAT,
};
...
/// Ruby `HuntersMoon#eval_game_system_specific_command`。
fn eval_specific_command(
    command: &str,
    rng: &mut Randomizer,
) -> Result<Option<SpecificCommandOutput>, EvalError> {
    // Ruby: `case command when /^SA/i` ＝ `SA` で始まるなら異形アビリティー表だけを見る。
    // 一致しなかった場合に `roll_tables` へ落ちることはない。
    if sa_prefix_pattern().is_match(command) {
        return Ok(roll_strange_ability_table(command, rng)?.map(SpecificCommandOutput::text));
    }
...
/// Ruby `case command when /^SA/i`。
fn sa_prefix_pattern() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r"(?i)^SA").expect("valid regex"))
}

/// Ruby `roll_strange_ability_table` の `/^SA(2)?T(\d+)?$/`。
fn strange_ability_pattern() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r"^SA(2)?T(\d+)?$").expect("valid regex"))
}

/// Ruby `HuntersMoon#roll_strange_ability_table`。
fn roll_strange_ability_table(
    command: &str,
    rng: &mut Randomizer,
) -> Result<Option<String>, EvalError> {
    let Some(m) = strange_ability_pattern().captures(command) else {
        return Ok(None);
    };

    // Ruby: `m[2]&.to_i || 1`。桁あふれする入力は Ruby だと Bignum になり
    // `count.times` が乱数の上限に当たるので、i64 に収まらない場合も飽和させて同じ経路へ落とす。
    let count = match m.get(2) {
        Some(c) => c.as_str().parse::<i64>().unwrap_or(i64::MAX),
        None => 1,
    };
    // Ruby: `type2 = !m[1].nil?`
    let type2 = m.get(1).is_some();

    let mut dice_list: Vec<String> = Vec::new();
    let mut chosen_list: Vec<String> = Vec::new();

    let mut remaining = count;
    while remaining > 0 {
        let rolled = if type2 {
            roll_strange_ability2(rng)?
        } else {
            roll_strange_ability1(rng)?
        };
        dice_list.push(rolled.0);
        chosen_list.push(rolled.1);
        remaining -= 1;
    }
...
/// Ruby `StrangeAbilityTable1#roll`。戻り値は `[dice, chosen]`。
///
/// D66表だが出目の入れ替えはせず、1個目を行・2個目を列として素直に引く。
fn roll_strange_ability1(rng: &mut Randomizer) -> Result<(String, String), EvalError> {
    let dice1 = rng.roll_once(6)?;
    let dice2 = rng.roll_once(6)?;
    let body = strange_ability_body(STRANGE_ABILITY_BODY1, dice1, dice2);
    Ok(((dice1 * 10 + dice2).to_string(), body.to_owned()))
}

/// Ruby `StrangeAbilityTable2#roll`。戻り値は `[dice, chosen]`。
///
/// 先に1D6を振り、奇数なら表1・偶数なら表2を引く。
fn roll_strange_ability2(rng: &mut Randomizer) -> Result<(String, String), EvalError> {
    let dice = rng.roll_once(6)?;

    // Ruby: `dice.odd? ? StrangeAbilityTable1::BODY : BODY`
    let odd = dice % 2 != 0;
    let table = if odd {
        STRANGE_ABILITY_BODY1
    } else {
        STRANGE_ABILITY_BODY2
    };
    let table_id = if odd { 1 } else { 2 };

    let dice1 = rng.roll_once(6)?;
    let dice2 = rng.roll_once(6)?;
    let body = strange_ability_body(table, dice1, dice2);

    Ok((
        format!("{dice}-{}", dice1 * 10 + dice2),
        format!("[表{table_id}]{body}"),
    ))
}

/// Ruby `table[(dice1 - 1) * 6 + (dice2 - 1)]`。
///
/// 出目は1〜6なので添字は0〜35に収まる。範囲外は Ruby の `nil` と同じく空文字列にする。
fn strange_ability_body(table: &'static [&'static str], dice1: i64, dice2: i64) -> &'static str {
    usize::try_from((dice1 - 1) * 6 + (dice2 - 1))
        .ok()
...
/// Ruby `HuntersMoon#result_2d6`。
fn check_result_2d6(dice_total: crate::Int, cmp_op: CmpOp) -> Option<CheckOutcome> {
    // Ruby: `return nil unless cmp_op == :>=`
    if cmp_op != CmpOp::Ge {
        return None;
    }

    // Ruby側は目標値を見ないので、目標値が `?` でもファンブル・スペシャルは判定される。
    if dice_total <= I::from(2) {
        ...
        // Ruby: `if/elsif` のどちらにも当たらない＝暗黙の nil。
        // `Base#check_result` が既定の `result_ndx`（成功／失敗）へ進む。
        None
    }
}

/// Ruby `RTT`（`SaiFicSkillTable.new(..., rttn:, rttn_format:)`）。
///
/// `rtt` / `rct` は指定が無いので既定コマンド（`RTT` / `RCT`）のみ、
/// `rtt_format` と `s_format` も既定のまま。
static RTT: SaiFicSkillTable = SaiFicSkillTable::new(RTT_CATEGORIES)
    .with_commands(None, None, &["TST", "THT", "TAT", "TBT", "TLT", "TET"])
    .with_formats(SaiFicFormats {
        rtt: DEFAULT_RTT_FORMAT,
        rct: DEFAULT_RCT_FORMAT,
        rttn: "指定特技(%<category_name>s)表(%<row_dice>d) ＞ %<skill_name>s",
...
/// Ruby `TABLES["CLT"]`。
static TABLE_CLT: Table = Table::from_dice(
    "都市ロケーション表",
    1,
    6,
    &[
...
/// Ruby `TABLES["SLT"]`。
static TABLE_SLT: Table = Table::from_dice(
    "閉所ロケーション表",
    1,
    6,
    &[
...
/// Ruby `TABLES["HLT"]`。
static TABLE_HLT: Table = Table::from_dice(
    "炎熱ロケーション表",
    1,
    6,
    &[
...
/// Ruby `TABLES["FLT"]`。
static TABLE_FLT: Table = Table::from_dice(
    "冷暗ロケーション表",
    1,
    6,
    &[
...
/// Ruby `TABLES["ET"]`。
static TABLE_ET: Table = Table::from_dice(
    "遭遇表",
    1,
    6,
    &[
...
/// Ruby `TABLES["MAT"]`。
static TABLE_MAT: Table = Table::from_dice(
    "モノビースト行動表",
    1,
    6,
    &[
...
/// Ruby `TABLES["DLT"]`。
static TABLE_DLT: Table = Table::from_dice(
    "部位ダメージ決定表",
    2,
    6,
    &[
...
/// Ruby `TABLES["DS1ET"]`。
static TABLE_DS1ET: Table = Table::from_dice(
    "ディフェンス遭遇表1st",
    1,
    6,
    &[
...
/// Ruby `TABLES["DS2ET"]`。
static TABLE_DS2ET: Table = Table::from_dice(
    "ディフェンス遭遇表2nd",
    1,
    6,
    &[
...
/// Ruby `TABLES["DS3ET"]`。
static TABLE_DS3ET: Table = Table::from_dice(
    "ディフェンス遭遇表3rd",
    1,
    6,
    &[
...
/// Ruby `TABLES["EE1ET"]`。
static TABLE_EE1ET: Table = Table::from_dice(
    "エスケープ遭遇表1st",
    1,
    6,
    &[
...
/// Ruby `TABLES["EE2ET"]`。
static TABLE_EE2ET: Table = Table::from_dice(
    "エスケープ遭遇表2nd",
    1,
    6,
    &[
...
/// Ruby `TABLES["EE3ET"]`。
static TABLE_EE3ET: Table = Table::from_dice(
    "エスケープ遭遇表3rd",
    1,
    6,
    &[
...
/// Ruby `TABLES["ERT"]`。
static TABLE_ERT: Table = Table::from_dice(
    "代償表",
    1,
    6,
    &[
...
/// Ruby `TABLES["ET1ET"]`。
static TABLE_ET1ET: Table = Table::from_dice(
    "エスコート遭遇表1st",
    1,
    6,
    &[
...
/// Ruby `TABLES["ET2ET"]`。
static TABLE_ET2ET: Table = Table::from_dice(
    "エスコート遭遇表2nd",
    1,
    6,
    &[
...
/// Ruby `TABLES["ET3ET"]`。
static TABLE_ET3ET: Table = Table::from_dice(
    "エスコート遭遇表3rd",
    1,
    6,
    &[
...
/// Ruby `TABLES["MST"]`。
static TABLE_MST: Table = Table::from_dice(
    "異形化表",
    1,
    6,
    &[
...
/// Ruby `TABLES["TK1ET"]`。
static TABLE_TK1ET: Table = Table::from_dice(
    "トラッキング遭遇表1st",
    1,
    6,
    &[
...
/// Ruby `TABLES["TK2ET"]`。
static TABLE_TK2ET: Table = Table::from_dice(
    "トラッキング遭遇表2nd",
    1,
    6,
    &[
...
/// Ruby `TABLES["TK3ET"]`。
static TABLE_TK3ET: Table = Table::from_dice(
    "トラッキング遭遇表3rd",
    1,
    6,
    &[
...
/// Ruby `StrangeAbilityTable1::BODY`（異形アビリティー表1、D66の36項目）。
static STRANGE_ABILITY_BODY1: &[&str] = &[
    "大牙",
    "大鎌",
    "針山",
    "大鋏",
...
/// Ruby `StrangeAbilityTable2::BODY`（異形アビリティー表2、D66の36項目）。
static STRANGE_ABILITY_BODY2: &[&str] = &[
    "電撃",
    "障壁",
    "追加肢",
    "破裂球",
...
/// Ruby `BCDice::GameSystem::HuntersMoon`（ID: `HuntersMoon`）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct HuntersMoon;

impl GameSystem for HuntersMoon {
...
"
    ...
    /// Ruby `HuntersMoon#initialize` の `@sort_add_dice = true`。
    fn sort_add_dice(&self) -> bool {
        true
    }

    /// Ruby `HuntersMoon#initialize` の `@d66_sort_type = D66SortType::ASC`。
    fn d66_sort_type(&self) -> D66SortType {
        D66SortType::Asc
    }

    /// Ruby `HuntersMoon#initialize` の `@round_type = RoundType::CEIL`。
    ...
    fn result_2d6(
        ...
        check_result_2d6(crate::Int::from(dice_total), cmp_op)
    }
...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/HuntersMoon.toml");
    ...
    /// `test/data/HuntersMoon.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/HuntersMoon.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("HuntersMoon.toml must parse");
        assert_eq!(
            data.tests.len(),
            175,
            "case count in test/data/HuntersMoon.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "HuntersMoon",
                "unexpected game system in HuntersMoon.toml"
            );
            ...
            match eval_command(&GameSystemId::new("HuntersMoon"), &tc.input, &mut src) {
    ...
    /// `test/data/HuntersMoon.toml` が通らない分岐を押さえる。
    ///
    /// - `RCT`（ランダム分野表）は指定特技表の別コマンドで、TOMLに事例が無い
    /// - `SAT0` は `count` が0になり、表を1度も引かずに空欄で出力される
    #[test]
    fn branches_not_covered_by_toml() {
        fn eval(input: &str, rands: Vec<(i64, i64)>) -> Option<String> {
            let mut src = SeededRandomizer::new(rands);
            let out = eval_command(&GameSystemId::new("HuntersMoon"), input, &mut src)
        ...
        assert_eq!(
            eval("RCT", vec![(1, 6)]).as_deref(),
            Some("ランダム分野表(1) ＞ 社会")
        );
        assert_eq!(
            eval("SAT0", vec![]).as_deref(),
            Some("異形アビリティー表() ＞ ")
        );
    }
}
