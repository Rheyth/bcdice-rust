//! P4で手書き移植した `lib/bcdice/game_system/JuinKansen_Korean.rb`。
//!
//! メタデータ（id/name/sort_key/help_message/prefixes/settings）は
//! `rust/tools/generate_game_systems.rb` が生成したスタブの値をそのまま保っている。
//! 生成スクリプトを再実行するとこのファイルはスタブへ戻るので注意。
//!
//! Ruby側は `JuinKansen` を継承し `@locale = :ko_kr` で表を組み直すだけなので、
//! コマンド解釈は [`super::JuinKansen`] の実装をそのまま使い、
//! ここには `ko_kr` ロケールの表データだけを置く。

use super::JuinKansen::eval_specific_command;
use crate::dice_table::{RollableTable, Table};
use crate::eval::EvalError;
use crate::game_system::{GameSystem, SpecificCommandOutput};
use crate::randomizer::Randomizer;

static KO_DAILY_ITEMS: &[&str] = &[
...
static KO_DAILY: Table = Table::from_dice("일상 표", 2, 6, KO_DAILY_ITEMS);

static KO_PLACECITY_ITEMS: &[&str] = &[
...
static KO_PLACECITY: Table = Table::from_dice("장소 표「도시」", 2, 6, KO_PLACECITY_ITEMS);

static KO_PLACECOUNTRYSIDE_ITEMS: &[&str] = &[
    "「사건현장」여기는 이전에 괴이사건이 일어났다는 소문이 있는 사건현장이다. 잘 보면 근처에 시든 꽃다발이 놓여있다.",
...
static KO_PLACECOUNTRYSIDE: Table =
    Table::from_dice("장소 표「시골」", 2, 6, KO_PLACECOUNTRYSIDE_ITEMS);

static KO_PLACEFACILITY_ITEMS: &[&str] = &[
...
static KO_PLACEFACILITY: Table =
    Table::from_dice("장소 표「시설 안」", 2, 6, KO_PLACEFACILITY_ITEMS);

static KO_FIRSTLOOK_ITEMS: &[&str] = &[
...
static KO_FIRSTLOOK: Table = Table::from_dice("초면 표", 2, 6, KO_FIRSTLOOK_ITEMS);

static KO_APPRECIATIVEFRIEND_ITEMS: &[&str] = &[
...
static KO_APPRECIATIVEFRIEND: Table =
    Table::from_dice("친구 표", 2, 6, KO_APPRECIATIVEFRIEND_ITEMS);

static KO_FORESHADOW_ITEMS: &[&str] = &[
...
static KO_FORESHADOW: Table = Table::from_dice("복선 표", 1, 6, KO_FORESHADOW_ITEMS);

static KO_EMOTION_ITEMS: &[&str] = &["존경", "호의", "우정", "비호", "신뢰", "안심"];
static KO_EMOTION: Table = Table::from_dice("감정 표", 1, 6, KO_EMOTION_ITEMS);

static KO_SITUATION_ITEMS: &[&str] = &[
...
static KO_SITUATION: Table = Table::from_dice("상황 표", 1, 6, KO_SITUATION_ITEMS);

static KO_TARGET_ITEMS: &[&str] = &[
    "【체력】이 가장 높은 대상",
    "【민첩】이 가장 높은 대상",
    "【지성】이 가장 높은 대상",
    "【정신】이 가장 높은 대상",
...
static KO_TARGET: Table = Table::from_dice("대상 표", 1, 6, KO_TARGET_ITEMS);

static KO_INSANITY_ITEMS: &[&str] = &[
...
static KO_INSANITY: Table = Table::from_dice("광기 표", 1, 6, KO_INSANITY_ITEMS);

static KO_DEATH_ITEMS: &[&str] = &[
...
static KO_DEATH: Table = Table::from_dice("종말 표", 1, 6, KO_DEATH_ITEMS);

static KO_FEAR_ITEMS: &[&str] = &[
...
static KO_FEAR: Table = Table::from_dice("공포 표", 2, 6, KO_FEAR_ITEMS);

pub(crate) static KO_TABLES: &[(&str, &dyn RollableTable)] = &[
    ("DAILY", &KO_DAILY),
    ("PLACECITY", &KO_PLACECITY),
    ("PLACECOUNTRYSIDE", &KO_PLACECOUNTRYSIDE),
    ("PLACEFACILITY", &KO_PLACEFACILITY),
    ("FIRSTLOOK", &KO_FIRSTLOOK),
    ("APPRECIATIVEFRIEND", &KO_APPRECIATIVEFRIEND),
    ("FORESHADOW", &KO_FORESHADOW),
    ("EMOTION", &KO_EMOTION),
    ("SITUATION", &KO_SITUATION),
    ("TARGET", &KO_TARGET),
    ("INSANITY", &KO_INSANITY),
    ("DEATH", &KO_DEATH),
    ("FEAR", &KO_FEAR),
];

/// Ruby `BCDice::GameSystem::JuinKansen_Korean`（ID: `JuinKansen:Korean`）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct JuinKansen_Korean;

impl GameSystem for JuinKansen_Korean {
...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/JuinKansen_Korean.toml");
    ...
    /// `test/data/JuinKansen_Korean.toml` の全ケースが通ること。
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/JuinKansen_Korean.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("JuinKansen_Korean.toml must parse");
        assert_eq!(
            data.tests.len(),
            52,
            "case count in test/data/JuinKansen_Korean.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "JuinKansen:Korean",
                "unexpected game system in JuinKansen_Korean.toml"
            );
            ...
            match eval_command(&GameSystemId::new("JuinKansen:Korean"), &tc.input, &mut src) {
        ...
        assert!(
            failures.is_empty(),
            "{}/{} JuinKansen_Korean cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
