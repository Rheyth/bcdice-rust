...
mod tests {
    ...
    /// `test/data/ZombiLine.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/ZombiLine.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("ZombiLine.toml must parse");
        assert_eq!(
            data.tests.len(),
            23,
            "case count in test/data/ZombiLine.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "ZombiLine",
                "unexpected game system in ZombiLine.toml"
            );
            ...
            match eval_command(&GameSystemId::new("ZombiLine"), &tc.input, &mut src) {
