...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/RogueLikeHalf.toml");
    ...
    /// `test/data/RogueLikeHalf.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/RogueLikeHalf.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("RogueLikeHalf.toml must parse");
        assert_eq!(
            data.tests.len(),
            16,
            "case count in test/data/RogueLikeHalf.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "RogueLikeHalf",
                "unexpected game system in RogueLikeHalf.toml"
            );
            ...
            match eval_command(&GameSystemId::new("RogueLikeHalf"), &tc.input, &mut src) {
