//! P4で手書き移植した `lib/bcdice/game_system/MeikyuKingdom.rb` と
//! `lib/bcdice/game_system/meikyu_kingdom/*.rb`。
//!
...
//!
//! 移植したもの:
//! - `MeikyuKingdom#result_nd6` / `#result_nd6_only` / `#result_2d6_text` / `#kiryoku_result`
//! - `MeikyuKingdom#replace_text` / `#mayokin_check`（`nMK+m` → `nR6+m` の判定）
//! - `MeikyuKingdom#eval_game_system_specific_command`（各種表）
//! - `meikyu_kingdom/*.rb` の表データ
//!
//! # 表データ
//!
//! Ruby側の表はメソッド内のリテラル配列。Rust側は同じ値を `static` として持つ。
//! `MK_` 接頭辞の `static` 群は原典rbから機械抽出したもので、値は1文字も変えていない。
//!
//! # 継承
//!
//! Ruby `MeikyuKingdomBasic < MeikyuKingdom` は `kiryoku_result` と一部の表メソッドを
//! 上書きし、自分のコマンドに当たらなかったときだけ `super` でこのクラスの
//! ディスパッチャへ落ちる。Rustには継承がないので、上書きされうるメソッドを
//! [`MkHooks`] にまとめ、[`eval_specific`] / [`result_nd6_impl`] へ渡す形にした。
...
/// Ruby `BCDice::GameSystem::MeikyuKingdom`（ID: `MeikyuKingdom`）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct MeikyuKingdom;

impl GameSystem for MeikyuKingdom {
    ...
    fn eval_game_system_specific_command(
        ...
        Ok(eval_specific(command, rng, &MK_HOOKS)?.map(SpecificCommandOutput::text))
    }
}
...
static PREFIXES: &[&str] = &[
    r"\d+MK", r"\d+R6", "LRT", "ORT", "CRT", "ART", "FRT", "TBT", "CBT", "SBT", "VBT", "FBT",
    "ABT", "WBT", "LBT", "THT", "CHT", "SHT", "VHT", "KDT", "KCT", "KMT", "CAT", "FWT", "CFT",
    "TT", "NT", "ET", "MPT", "T1T", "T2T", "T3T", "T4T", "T5T", "NAME", "DFT", "IDT", "WIT", "LIT",
    "RIT", "SIT", "RWIT", "RUIT", "IFT", r"\d+RET", "PNT", "MLT", "KNT", "WORD",
];
...
/// Ruby で `MeikyuKingdomBasic` が上書きし、`MeikyuKingdom` 側のコードから
/// 動的ディスパッチで呼ばれるメソッド群。
///
/// Rustには継承がないため、`MeikyuKingdom` のディスパッチャと判定処理は
/// この関数表を受け取る形にしてある。
pub(crate) struct MkHooks {
    /// Ruby `#kiryoku_result(total_n, dice_list, diff)`。
    pub kiryoku_result: fn(i64, &[i64], i64) -> String,
    /// Ruby `#mk_name_ex_table(num)`。
    pub name_ex_table: fn(i64) -> String,
    /// Ruby `#mk_word_1_table(num)` 〜 `#mk_word_4_table(num)`。
    pub word_tables: [fn(i64) -> String; 4],
    /// Ruby `#mk_kingdom_name_1_table(num)` 〜 `#mk_kingdom_name_3_table(num)`。
    pub kingdom_name_tables: [fn(i64) -> String; 3],
}

/// `MeikyuKingdom` 自身の実装。
pub(crate) static MK_HOOKS: MkHooks = MkHooks {
    kiryoku_result,
    name_ex_table: mk_name_ex_table,
    word_tables: [
        mk_word_1_table,
        mk_word_2_table,
        mk_word_3_table,
    ...
    kingdom_name_tables: [
        mk_kingdom_name_1_table,
        mk_kingdom_name_2_table,
        mk_kingdom_name_3_table,
    ],
};
...
///
/// 番号が `index` 以上になる最初の項目を返す。該当なしなら `default`。
pub(crate) fn n_number(index: i64, table: &[(i64, &'static str)]) -> &'static str {
    table
        .iter()
...
/// Ruby `Base#get_table_by_nDx(table, count, dice_type)`。戻り値は `[text, num]`。
///
/// 添字が範囲外なら Ruby は `["1", 0]` を返す。
pub(crate) fn table_by_ndx(
    table: &[&'static str],
    count: i64,
    sides: i64,
    rng: &mut Randomizer,
) -> Result<(String, i64), EvalError> {
    let num = rng.roll_sum(count, sides)?;
    match usize::try_from(num - count).ok().and_then(|i| table.get(i)) {
        None => Ok(("1".to_string(), 0)),
        Some(text) => Ok(((*text).to_string(), num)),
    }
}

/// Ruby `Base#get_table_by_2d6(table)`。
fn table_by_2d6(table: &[&'static str], rng: &mut Randomizer) -> Result<(String, i64), EvalError> {
    table_by_ndx(table, 2, 6, rng)
}

/// Ruby `Base#get_table_by_1d6(table)`。
fn table_by_1d6(table: &[&'static str], rng: &mut Randomizer) -> Result<(String, i64), EvalError> {
    table_by_ndx(table, 1, 6, rng)
}

/// Ruby `Base#get_table_by_1d3(table)`。
///
/// 1D6を振り、`(num - 1) / 2` 番目の項目を選ぶ（返す番号は1D6の出目そのもの）。
fn table_by_1d3(table: &[&'static str], rng: &mut Randomizer) -> Result<(String, i64), EvalError> {
...
/// Ruby `Base#getCount(countText)`。空文字なら1。
pub(crate) fn get_count(count_text: &str) -> i64 {
    if count_text.is_empty() {
        return 1;
    }
    // Ruby `String#to_i` は解釈できない部分を捨てる。ここは `\d*` の捕捉なので数字のみ。
    count_text.parse().unwrap_or(i64::MAX)
}

/// Ruby の `command =~ /^XXX/i`。接頭辞はすべてASCIIなので `eq_ignore_ascii_case` で足りる。
pub(crate) fn head(command: &str, prefix: &str) -> bool {
    command.len() >= prefix.len()
        && command.as_bytes()[..prefix.len()].eq_ignore_ascii_case(prefix.as_bytes())
}
...
pub(crate) fn result_nd6_impl(
    ...
    let mut result = result_nd6_only(total, dice_total, cmp_op, target.clone())?;
    result.text += &(hooks.kiryoku_result)(total, value_list, crate::randomizer::sat_i64(target_n));
    Some(CheckOutcome::Result(Box::new(result)))
}

/// Ruby `MeikyuKingdom#result_nd6_only`。
pub(crate) fn result_nd6_only(
    total: i64,
    dice_total: i64,
    cmp_op: CmpOp,
    target: Target,
    ...
    // Ruby: total >= target。目標値が "?" の場合 Ruby は Integer と String の比較で
    // 例外になるが、`result_nd6` が先に `Result.nothing` を返すため到達しない。
    let target_n = match target {
        Target::Number(t) => t,
        Target::Question => return None,
    };
    ...
    } else if total >= crate::randomizer::sat_i64(&target_n) {
...
/// Ruby `MeikyuKingdom#result_2d6_text`。
fn result_2d6_text(total: i64, dice_total: i64, cmp_op: CmpOp, target: i64) -> String {
    match result_nd6_only(
        total,
        dice_total,
        cmp_op,
        Target::Number(crate::Int::from(target)),
    ) {
        None => String::new(),
...
/// Ruby `MeikyuKingdom#kiryoku_result`。
///
/// Ruby側にある `none6Dice_n`（`maxDice1 + maxDice2`）は出力に使われないので省いた。
pub(crate) fn kiryoku_result(total_n: i64, dice_list: &[i64], diff: i64) -> String {
    let num_6 = dice_list.iter().filter(|d| **d == 6).count();

    if num_6 == 0 {
        return String::new();
    } else if num_6 >= 2 {
        return format!(" ＆ 《気力》{num_6}点獲得");
    }

    let mut none6_list: Vec<i64> = dice_list.iter().copied().filter(|d| *d != 6).collect();
    none6_list.sort_unstable();

    // Ruby: maxDice1 = none6_list.pop.to_i（結果に使われないが2回 pop する順序は保つ）
    //       maxDice2 = none6_list.pop.to_i（空配列の pop は nil → to_i で 0）
    none6_list.pop();
    let max_dice2 = none6_list.pop().unwrap_or(0);

    let none6_total_n = total_n - 6 + max_dice2;
    let none6_dice_result = if none6_total_n >= diff {
        " ＞ 成功"
    } else {
        " ＞ 失敗"
    };

    format!(" (もしくは) {none6_total_n}{none6_dice_result} ＆ 《気力》1点獲得")
}

/// Ruby `MeikyuKingdom#replace_text`。`nMK6` / `nMK` を `nR6` に置き換える。
fn replace_text(string: &str) -> String {
    static RE_MK6: OnceLock<Regex> = OnceLock::new();
    static RE_MK: OnceLock<Regex> = OnceLock::new();
    let re_mk6 = RE_MK6.get_or_init(|| Regex::new(r"(?i)(\d+)MK6").expect("valid regex"));
    let re_mk = RE_MK.get_or_init(|| Regex::new(r"(?i)(\d+)MK").expect("valid regex"));

    let s = re_mk6.replace_all(string, "${1}R6");
    re_mk.replace_all(&s, "${1}R6").into_owned()
}

/// Ruby `/^S?((\d+)R6([+\-\d]*)(([>=]+)(\d+))?)/i`。
fn mayokin_pattern() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| {
        Regex::new(r"(?i)^S?((\d+)R6([+\-\d]*)(([>=]+)(\d+))?)").expect("valid regex")
    })
}
...
/// Ruby `MeikyuKingdom#mayokin_check`。
pub(crate) fn mayokin_check(
    string: &str,
    rng: &mut Randomizer,
    hooks: &MkHooks,
    ...
    let Some(m) = mayokin_pattern().captures(&string) else {
        return Ok(None);
    };

    let matched = m[1].to_string();
    // Ruby `String#to_i`。桁が大きすぎる指定は roll_barabara 側で弾かれる。
    let dice_count: i64 = m[2].parse().unwrap_or(i64::MAX);
    let modify_text = m.get(3).map_or("", |c| c.as_str());
    let sign_of_inequality = m.get(5).map_or("", |c| c.as_str());
    let diff: i64 = m
        .get(6)
        .map_or(0, |c| c.as_str().parse().unwrap_or(i64::MAX));

    let bonus = arithmetic_evaluator_eval(modify_text)?;

    let mut dice_list = rng.roll_barabara(dice_count, 6)?;
    dice_list.sort_unstable();
    let dice_str: Vec<String> = dice_list.iter().map(|d| d.to_string()).collect();
    let dice_str = dice_str.join(",");

    // Ruby: dice1 = diceCount >= 2 ? dice_list[diceCount - 2] : 0
    //
    // ダイス数が上限（`UPPER_LIMIT_DICE_TIMES` = 200）を超えると `roll_barabara` が
    // 空配列を返すため、Ruby は `nil + nil` の NoMethodError で落ちる（`201MK` など）。
    // ここでは添字が範囲外なら0として扱い、パニックさせない。
    let nth = |i: i64| -> i64 {
        usize::try_from(i)
            .ok()
            .and_then(|i| dice_list.get(i))
            .copied()
            .unwrap_or(0)
    };
    let dice1 = if dice_count >= 2 {
        nth(dice_count - 2)
    } else {
        0
    };
    let dice2 = if dice_count >= 1 {
        nth(dice_count - 1)
    } else {
        0
    };
    let dice_now = dice1 + dice2;

    let total_n = dice_now + bonus;

    let mut output = format!("{dice_now}[{dice_str}]");
    if bonus > 0 {
        output += &format!("+{bonus}");
    } else if bonus < 0 {
        output += &bonus.to_string();
    }
    ...
    let mut output = if has_other {
        format!("({matched}) ＞ {output} ＞ {total_n}")
    } else {
        format!("({matched}) ＞ {total_n}")
    };

    if !sign_of_inequality.is_empty() {
        // Ruby: Normalize.comparison_operator は未知の記号に nil を返す。
        // `[>=]+` の捕捉なので `>`, `>=`, `=`, `==`, `=>` などが来る。
        if let Some(cmp_op) = normalize::comparison_operator(sign_of_inequality) {
            output += &result_2d6_text(total_n, dice_now, cmp_op, diff);
        }
        output += &(hooks.kiryoku_result)(total_n, &dice_list, diff);
    }
...
/// Ruby `MeikyuKingdom#mk_name_a_table(num)`。
fn mk_name_a_table(num: i64) -> String {
    n_number(num, MK_NAME_A_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_name_b_table(num)`。
fn mk_name_b_table(num: i64) -> String {
    n_number(num, MK_NAME_B_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_name_ex_table(num)`。
fn mk_name_ex_table(num: i64) -> String {
    n_number(num, MK_NAME_EX_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_name_fa_table(num)`。
fn mk_name_fa_table(num: i64) -> String {
    n_number(num, MK_NAME_FA_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_word_1_table(num)`。
fn mk_word_1_table(num: i64) -> String {
    n_number(num, MK_WORD_1_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_word_2_table(num)`。
fn mk_word_2_table(num: i64) -> String {
    n_number(num, MK_WORD_2_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_word_3_table(num)`。
fn mk_word_3_table(num: i64) -> String {
    n_number(num, MK_WORD_3_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_word_4_table(num)`。
fn mk_word_4_table(num: i64) -> String {
    n_number(num, MK_WORD_4_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_kingdom_name_1_table(num)`。
fn mk_kingdom_name_1_table(num: i64) -> String {
    n_number(num, MK_KINGDOM_NAME_1_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_kingdom_name_2_table(num)`。
fn mk_kingdom_name_2_table(num: i64) -> String {
    n_number(num, MK_KINGDOM_NAME_2_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_kingdom_name_3_table(num)`。
fn mk_kingdom_name_3_table(num: i64) -> String {
    n_number(num, MK_KINGDOM_NAME_3_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_name_table`。戻り値は `[output, dice]`。
fn mk_name_table(rng: &mut Randomizer, hooks: &MkHooks) -> Result<(String, String), EvalError> {
    let name_n = rng.roll_once(6)?;
    let d1 = rng.roll_d66(D66SortType::Asc)?;
    let d2 = rng.roll_d66(D66SortType::Asc)?;

    let output = if name_n <= 1 {
        mk_nick_a_table(mk_name_a_table(d1), d2, rng)?
    } else if name_n <= 2 {
        mk_nick_a_table(mk_name_b_table(d1), d2, rng)?
    } else if name_n <= 3 {
        mk_nick_a_table((hooks.name_ex_table)(d1), d2, rng)?
    } else if name_n <= 4 {
        mk_nick_b_table(mk_name_a_table(d1), d2)
    } else if name_n <= 5 {
        mk_nick_b_table(mk_name_b_table(d1), d2)
    } else {
        mk_nick_b_table(mk_name_fa_table(d1), d2)
    };

    Ok((output, format!("{name_n},{d1},{d2}")))
}

/// Ruby `MeikyuKingdom#mk_nick_a_table(output, num)`。
fn mk_nick_a_table(output: String, num: i64, rng: &mut Randomizer) -> Result<String, EvalError> {
    if num < 66 {
        Ok(format!("{}{output}", n_number(num, MK_NICK_A_TABLE)))
    } else {
        Ok(format!("{output}{}世", rng.roll_once(6)?))
    }
}

/// Ruby `MeikyuKingdom#mk_nick_b_table(output, num)`。
fn mk_nick_b_table(output: String, num: i64) -> String {
    format!("{}{output}", n_number(num, MK_NICK_B_TABLE))
}

/// Ruby `MeikyuKingdom#mk_pn_decide_table(num)`。
fn mk_pn_decide_table(num: i64, rng: &mut Randomizer) -> Result<String, EvalError> {
    let mut output = String::new();

    let d1 = rng.roll_once(6)?;
    let d2 = rng.roll_once(6)?;

    // Ruby: (d / 2.0).ceil.to_i
    let d1 = (d1 + 1) / 2;
    let d2 = (d2 + 1) / 2;

    for _ in 0..num.max(0) {
        output += "「";
        output += &mk_decoration_table(d1, rng)?;
        output += &mk_placename_table(d2, rng)?;
        output += "」";
    }
...
/// Ruby `MeikyuKingdom#mk_decoration_table(num)`（項目が `lambda` の表）。
fn mk_decoration_table(num: i64, rng: &mut Randomizer) -> Result<String, EvalError> {
    let table: [fn(i64) -> String; 3] = [
        mk_basic_decoration_table,
        mk_spooky_decoration_table,
        mk_katakana_decoration_table,
    ];
    // Ruby: get_table_by_number(num, [[1, ..], [2, ..], [3, ..]])
    match table
        .iter()
        .enumerate()
        .find(|(i, _)| (*i as i64) + 1 >= num)
    {
        None => Ok("1".to_string()),
        Some((_, f)) => {
            let dice = rng.roll_d66(D66SortType::Asc)?;
            Ok(f(dice))
        }
    }
}

/// Ruby `MeikyuKingdom#mk_placename_table(num)`（項目が `lambda` の表）。
fn mk_placename_table(num: i64, rng: &mut Randomizer) -> Result<String, EvalError> {
    let table: [fn(i64) -> String; 3] = [
        mk_passage_placename_table,
        mk_natural_placename_table,
        mk_artifact_placename_table,
    ];
    match table
        .iter()
        .enumerate()
...
/// Ruby `MeikyuKingdom#mk_basic_decoration_table(num)`。
fn mk_basic_decoration_table(num: i64) -> String {
    n_number(num, MK_BASIC_DECORATION_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_spooky_decoration_table(num)`。
fn mk_spooky_decoration_table(num: i64) -> String {
    n_number(num, MK_SPOOKY_DECORATION_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_katakana_decoration_table(num)`。
fn mk_katakana_decoration_table(num: i64) -> String {
    n_number(num, MK_KATAKANA_DECORATION_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_passage_placename_table(num)`。
fn mk_passage_placename_table(num: i64) -> String {
    n_number(num, MK_PASSAGE_PLACENAME_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_natural_placename_table(num)`。
fn mk_natural_placename_table(num: i64) -> String {
    n_number(num, MK_NATURAL_PLACENAME_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_artifact_placename_table(num)`。
fn mk_artifact_placename_table(num: i64) -> String {
    n_number(num, MK_ARTIFACT_PLACENAME_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_ls_decide_table(num)`。
fn mk_ls_decide_table(num: i64, rng: &mut Randomizer) -> Result<String, EvalError> {
    let mut output = String::new();
    for _ in 0..num.max(0) {
        let dice = rng.roll_once(6)?;
        output += "「";
        output += &mk_landscape_table(dice, rng)?;
        output += "」";
    }
    Ok(output)
}

/// Ruby `MeikyuKingdom#mk_landscape_table(num)`（項目が `lambda` の表）。
///
/// Ruby側は表を作る前にD66を振るので、`num` が範囲外でもダイスは消費される。
fn mk_landscape_table(num: i64, rng: &mut Randomizer) -> Result<String, EvalError> {
    let dice = rng.roll_d66(D66SortType::Asc)?;
    let table: [fn(i64) -> String; 6] = [
        mk_artifact_landscape_table,
        mk_cave_landscape_table,
        mk_natural_landscape_table,
        mk_waterside_landscape_table,
        mk_skyrealm_landscape_table,
...
/// Ruby `MeikyuKingdom#mk_artifact_landscape_table(num)`。
fn mk_artifact_landscape_table(num: i64) -> String {
    n_number(num, MK_ARTIFACT_LANDSCAPE_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_cave_landscape_table(num)`。
fn mk_cave_landscape_table(num: i64) -> String {
    n_number(num, MK_CAVE_LANDSCAPE_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_natural_landscape_table(num)`。
fn mk_natural_landscape_table(num: i64) -> String {
    n_number(num, MK_NATURAL_LANDSCAPE_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_waterside_landscape_table(num)`。
fn mk_waterside_landscape_table(num: i64) -> String {
    n_number(num, MK_WATERSIDE_LANDSCAPE_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_skyrealm_landscape_table(num)`。
fn mk_skyrealm_landscape_table(num: i64) -> String {
    n_number(num, MK_SKYREALM_LANDSCAPE_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_strange_place_landscape_table(num)`。
fn mk_strange_place_landscape_table(num: i64) -> String {
    n_number(num, MK_STRANGE_PLACE_LANDSCAPE_TABLE).to_string()
}

/// Ruby `MeikyuKingdom#mk_device_factory_table`。
fn mk_device_factory_table(rng: &mut Randomizer) -> Result<String, EvalError> {
    let n = rng.roll_once(6)?;
    let output = mk_item_decide_table(n, rng)?;

    let dice = rng.roll_sum(2, 6)?;
    Ok(format!("{output} / {}", mk_item_features_table(dice, rng)?))
}

/// Ruby `MeikyuKingdom#mk_item_decide_table(num)`（項目が `lambda` の表）。
fn mk_item_decide_table(num: i64, rng: &mut Randomizer) -> Result<String, EvalError> {
    // Ruby: [[1, 武具], [2, 生活], [3, 回復], [4, 探索], [5, レア武具], [6, レア一般]]
    let index = (1..=6).find(|i| *i >= num);
    match index {
        None => Ok("1".to_string()),
        Some(1) => {
            let d = rng.roll_d66(D66SortType::Asc)?;
            Ok(n_number(d, MK_WEAPON_ITEM_TABLE).to_string())
        }
        Some(2) => {
            let d = rng.roll_d66(D66SortType::Asc)?;
            Ok(n_number(d, MK_LIFE_ITEM_TABLE).to_string())
        }
        Some(3) => {
            let d = rng.roll_d66(D66SortType::Asc)?;
            Ok(n_number(d, MK_REST_ITEM_TABLE).to_string())
        }
        Some(4) => {
            let d = rng.roll_d66(D66SortType::Asc)?;
            Ok(n_number(d, MK_SEARCH_ITEM_TABLE).to_string())
        }
        Some(5) => {
            let d = rng.roll_d66(D66SortType::NoSort)?;
            Ok(n_number(d, MK_RARE_WEAPON_ITEM_TABLE).to_string())
        }
        _ => {
            let d = rng.roll_d66(D66SortType::NoSort)?;
            Ok(n_number(d, MK_RARE_ITEM_TABLE).to_string())
        }
    }
}

/// Ruby `MeikyuKingdom#mk_item_features_table(num)`。
///
/// `num <= 3` / `<= 7` / `<= 9` / `<= 11` の枝は自分自身を再帰的に引く。
fn mk_item_features_table(num: i64, rng: &mut Randomizer) -> Result<String, EvalError> {
    let dice = rng.roll_sum(2, 6)?;

    let output = if num <= 2 {
        format!("「{}」の神力を宿す", mk_item_power_table(rng.roll_once(6)?))
    } else if num <= 3 {
        format!(
            "寿命を持つ。寿命の値を決定する。\nさらに、{}",
            mk_item_features_table(dice, rng)?
        )
    } else if num <= 4 {
        "境界障壁を持つ。《HP》の値を決定する。".to_string()
    } else if num <= 5 {
        "銘を持つ。銘を決定する。".to_string()
    } else if num <= 6 {
        let n = rng.roll_once(6)?;
        format!(
            "合成具である。もう1つの機能は「{}」である。",
            mk_item_decide_table(n, rng)?
        )
    } else if num <= 7 {
        format!(
        ...
        format!("「{}」の呪紋を持つ。", mk_item_jyumon_table(dice))
    } else if num <= 9 {
        let n = rng.roll_once(6)?;
        format!(
            "「{}」の呪禍を持つ。\nさらに、{}",
            mk_item_jyuka_table(n),
            mk_item_features_table(dice, rng)?
        )
    } else if num <= 10 {
        "高価だ。価格を設定する。".to_string()
    } else if num <= 11 {
        let n = rng.roll_once(6)?;
        // Ruby は適性表を引ききってから再帰するので、順序を保つため一度束縛する。
        let aptitude = mk_item_aptitude_table(n, rng)?;
        let nested = mk_item_features_table(dice, rng)?;
        format!("「条件：{aptitude}」の適性を持つ。\nさらに、{nested}")
    } else {
        let n = rng.roll_once(6)?;
        format!("「{}」の属性を持つ。", mk_item_attribute_table(n))
    };

    Ok(format!("特性[{num}]：{output}"))
}

/// Ruby `MeikyuKingdom#mk_item_power_table(num)`。
fn mk_item_power_table(num: i64) -> String {
    format!("[{num}]{}", n_number(num, MK_ITEM_POWER_TABLE))
}

/// Ruby `MeikyuKingdom#mk_item_jyumon_table(num)`。
fn mk_item_jyumon_table(num: i64) -> String {
    format!("[{num}]{}", n_number(num, MK_ITEM_JYUMON_TABLE))
}

/// Ruby `MeikyuKingdom#mk_item_jyuka_table(num)`。
fn mk_item_jyuka_table(num: i64) -> String {
    format!("[{num}]{}", n_number(num, MK_ITEM_JYUKA_TABLE))
}

/// Ruby `MeikyuKingdom#mk_item_attribute_table(num)`。
fn mk_item_attribute_table(num: i64) -> String {
    format!("[{num}]{}", n_number(num, MK_ITEM_ATTRIBUTE_TABLE))
}

/// Ruby `MeikyuKingdom#mk_item_aptitude_table(num)`（項目に `lambda` を含む表）。
fn mk_item_aptitude_table(num: i64, rng: &mut Randomizer) -> Result<String, EvalError> {
    let body = match (1..=6).find(|i| *i >= num) {
        None => "1".to_string(),
        Some(1) => "ランダムなクラス1種".to_string(),
        Some(2) => {
            let d = rng.roll_d66(D66SortType::Asc)?;
            mk_family_business_table(d)
        }
        Some(3) => format!("{}性", mk_gender_table(rng.roll_once(6)?)),
        Some(4) => "上級ジョブ".to_string(),
        Some(5) => "モンスタースキルを修得".to_string(),
        _ => "童貞、もしくは処女".to_string(),
    };
    Ok(format!("[{num}]{body}"))
}

/// Ruby `MeikyuKingdom#mk_gender_table(num)`。
fn mk_gender_table(num: i64) -> &'static str {
    if num % 2 != 0 {
        "男"
    } else {
        "女"
...
/// Ruby `MeikyuKingdom#mk_family_business_table(num)`。
fn mk_family_business_table(num: i64) -> String {
    format!("[{num}]{}", n_number(num, MK_FAMILY_BUSINESS_TABLE))
}
...
/// Ruby `/^NAME(\d*)/i` などの「接頭辞＋個数」パターン。
fn count_pattern() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r"(?i)^(?:NAME|PNT|MLT|KNT|WORD)(\d*)").expect("valid regex"))
}

/// `command` の接頭辞に続く数字（`(\d*)` の捕捉）を取り出す。
...
/// `get_table_by_*` の戻り値 `[text, num]` を `(output, total_n)` の並びに直す。
fn text_and_num((text, num): (String, i64)) -> (String, String) {
    (text, num.to_string())
}

/// Ruby `MeikyuKingdom#eval_game_system_specific_command`。
///
/// 戻り値は Ruby の `String` / `nil`。`when` の並び順は原典どおりに保つ
/// （`NAMEA`〜`NAMEFA` が `NAME(\d*)` より前、`T1T`〜`T5T` が `TT` より前）。
pub(crate) fn eval_specific(
    command: &str,
    rng: &mut Randomizer,
    hooks: &MkHooks,
) -> Result<Option<String>, EvalError> {
    if let Some(result) = mayokin_check(command, rng, hooks)? {
        return Ok(Some(result));
    }

    let (type_name, output, total_n): (&str, String, String) = if head(command, "NAMEA") {
        let n = rng.roll_d66(D66SortType::Asc)?;
        ("名前Ａ", mk_name_a_table(n), n.to_string())
    } else if head(command, "NAMEB") {
        let n = rng.roll_d66(D66SortType::Asc)?;
        ("名前Ｂ", mk_name_b_table(n), n.to_string())
    } else if head(command, "NAMEEX") {
        let n = rng.roll_d66(D66SortType::Asc)?;
        ("エキゾチック名前", (hooks.name_ex_table)(n), n.to_string())
    } else if head(command, "NAMEFA") {
        let n = rng.roll_d66(D66SortType::Asc)?;
        ("ファンタジック名前", mk_name_fa_table(n), n.to_string())
    } else if head(command, "NAME") {
        let count = get_count(captured_count(command));
        let mut names = String::new();
        for _ in 0..count.max(0) {
            let (name, dice) = mk_name_table(rng, hooks)?;
            names += &format!("[{dice}]{name} ");
        }
        // Ruby `String#strip`。ここで落ちるのは末尾の半角空白だけ。
        let stripped = names
            .trim_matches(|c: char| c.is_ascii_whitespace())
            .to_string();
        ("名前", stripped, count.to_string())
    } else if head(command, "PNT") {
        let count = get_count(captured_count(command));
        let out = mk_pn_decide_table(count, rng)?;
        ("地名", out, count.to_string())
    } else if head(command, "MLT") {
        // Ruby側も type は '地名'（迷宮風景表だが表示名は地名）。
        let count = get_count(captured_count(command));
        let out = mk_ls_decide_table(count, rng)?;
        ("地名", out, count.to_string())
    } else if head(command, "DFT") {
        let out = mk_device_factory_table(rng)?;
        ("デバイスファクトリー", out, "1".to_string())
    } else if head(command, "LRT") {
        let (o, n) = text_and_num(table_by_2d6(MK_LIFE_RESEARCH_TABLE, rng)?);
        ("生活散策", o, n)
    } else if head(command, "ORT") {
        let (o, n) = text_and_num(table_by_2d6(MK_ORDER_RESEARCH_TABLE, rng)?);
        ("治安散策", o, n)
    } else if head(command, "CRT") {
        let (o, n) = text_and_num(table_by_2d6(MK_CALTURE_RESEARCH_TABLE, rng)?);
        ("文化散策", o, n)
    } else if head(command, "ART") {
        let (o, n) = text_and_num(table_by_2d6(MK_ARMY_RESEARCH_TABLE, rng)?);
        ("軍事散策", o, n)
    } else if head(command, "FRT") {
        let (o, n) = text_and_num(table_by_2d6(MK_FESTIVAL_TABLE, rng)?);
        ("お祭り", o, n)
    } else if head(command, "TBT") {
        let (o, n) = text_and_num(table_by_2d6(MK_TALENT_BREAK_TABLE, rng)?);
        ("才覚休憩", o, n)
    } else if head(command, "CBT") {
        let (o, n) = text_and_num(table_by_2d6(MK_CHARM_BREAK_TABLE, rng)?);
        ("魅力休憩", o, n)
    } else if head(command, "SBT") {
        let (o, n) = text_and_num(table_by_2d6(MK_SEARCH_BREAK_TABLE, rng)?);
        ("探索休憩", o, n)
    } else if head(command, "VBT") {
        let (o, n) = text_and_num(table_by_2d6(MK_VALOR_BREAK_TABLE, rng)?);
        ("武勇休憩", o, n)
    } else if head(command, "FBT") {
        let (o, n) = text_and_num(table_by_2d6(MK_FESTIVAL_BREAK_TABLE, rng)?);
        ("お祭り休憩", o, n)
    } else if head(command, "THT") {
        let (o, n) = text_and_num(table_by_2d6(MK_TALENT_HAPPENING_TABLE, rng)?);
        ("才覚ハプニング", o, n)
    } else if head(command, "CHT") {
        let (o, n) = text_and_num(table_by_2d6(MK_CHARM_HAPPENING_TABLE, rng)?);
        ("魅力ハプニング", o, n)
    } else if head(command, "SHT") {
        let (o, n) = text_and_num(table_by_2d6(MK_SEARCH_HAPPENING_TABLE, rng)?);
        ("探索ハプニング", o, n)
    } else if head(command, "VHT") {
        let (o, n) = text_and_num(table_by_2d6(MK_VALOR_HAPPENING_TABLE, rng)?);
        ("武勇ハプニング", o, n)
    } else if head(command, "MPT") {
        let (o, n) = text_and_num(table_by_2d6(MK_MARKET_PRICE_TABLE, rng)?);
        ("相場", o, n)
    } else if head(command, "T1T") {
        let (o, n) = text_and_num(table_by_1d6(MK_TREASURE1_TABLE, rng)?);
        ("お宝１", o, n)
    } else if head(command, "T2T") {
        let (o, n) = text_and_num(table_by_1d6(MK_TREASURE2_TABLE, rng)?);
        ("お宝２", o, n)
    } else if head(command, "T3T") {
        let (o, n) = text_and_num(table_by_1d6(MK_TREASURE3_TABLE, rng)?);
        ("お宝３", o, n)
    } else if head(command, "T4T") {
        let (o, n) = text_and_num(table_by_1d6(MK_TREASURE4_TABLE, rng)?);
        ("お宝４", o, n)
    } else if head(command, "T5T") {
        let (o, n) = text_and_num(table_by_1d6(MK_TREASURE5_TABLE, rng)?);
        ("お宝５", o, n)
    } else if head(command, "RWIT") {
        let n = rng.roll_d66(D66SortType::NoSort)?;
        (
            "レア武具アイテム",
            n_number(n, MK_RARE_WEAPON_ITEM_TABLE).to_string(),
            n.to_string(),
        )
    } else if head(command, "RUIT") {
        let n = rng.roll_d66(D66SortType::NoSort)?;
        (
            "レア一般アイテム",
            n_number(n, MK_RARE_ITEM_TABLE).to_string(),
            n.to_string(),
        )
    } else if head(command, "WIT") {
        let n = rng.roll_d66(D66SortType::Asc)?;
        (
            "武具アイテム",
            n_number(n, MK_WEAPON_ITEM_TABLE).to_string(),
            n.to_string(),
        )
    } else if head(command, "LIT") {
        let n = rng.roll_d66(D66SortType::Asc)?;
        (
            "生活アイテム",
            n_number(n, MK_LIFE_ITEM_TABLE).to_string(),
            n.to_string(),
        )
    } else if head(command, "RIT") {
        let n = rng.roll_d66(D66SortType::Asc)?;
        (
            "回復アイテム",
            n_number(n, MK_REST_ITEM_TABLE).to_string(),
            n.to_string(),
        )
    } else if head(command, "SIT") {
        let n = rng.roll_d66(D66SortType::Asc)?;
        (
            "探索アイテム",
            n_number(n, MK_SEARCH_ITEM_TABLE).to_string(),
            n.to_string(),
        )
    } else if head(command, "IFT") {
        let n = rng.roll_sum(2, 6)?;
        let out = mk_item_features_table(n, rng)?;
        ("アイテム特性", out, n.to_string())
    } else if head(command, "IDT") {
        let n = rng.roll_once(6)?;
        let out = mk_item_decide_table(n, rng)?;
        ("アイテムカテゴリ決定", out, n.to_string())
    } else if head(command, "1RET") {
        let n = rng.roll_once(6)?;
        (
            "1Lvランダムエンカウント",
            n_number(n, MK_RANDOM_ENCOUNT1_TABLE).to_string(),
            n.to_string(),
        )
    } else if head(command, "2RET") {
        let n = rng.roll_once(6)?;
        (
            "2Lvランダムエンカウント",
            n_number(n, MK_RANDOM_ENCOUNT2_TABLE).to_string(),
            n.to_string(),
        )
    } else if head(command, "3RET") {
        let n = rng.roll_once(6)?;
        (
            "3Lvランダムエンカウント",
            n_number(n, MK_RANDOM_ENCOUNT3_TABLE).to_string(),
            n.to_string(),
        )
    } else if head(command, "4RET") {
        let n = rng.roll_once(6)?;
        (
            "4Lvランダムエンカウント",
            n_number(n, MK_RANDOM_ENCOUNT4_TABLE).to_string(),
            n.to_string(),
        )
    } else if head(command, "5RET") {
        let n = rng.roll_once(6)?;
        (
            "5Lvランダムエンカウント",
            n_number(n, MK_RANDOM_ENCOUNT5_TABLE).to_string(),
            n.to_string(),
        )
    } else if head(command, "6RET") {
        let n = rng.roll_once(6)?;
        (
            "6Lvランダムエンカウント",
            n_number(n, MK_RANDOM_ENCOUNT6_TABLE).to_string(),
            n.to_string(),
        )
    } else if head(command, "KDT") {
        let (o, n) = text_and_num(table_by_2d6(MK_KINGDOM_DISASTER_TABLE, rng)?);
        ("王国災厄", o, n)
    } else if head(command, "KCT") {
        let (o, n) = text_and_num(table_by_2d6(MK_KINGDOM_CHANGE_TABLE, rng)?);
        ("王国変動", o, n)
    } else if head(command, "KMT") {
        let (o, n) = text_and_num(table_by_2d6(MK_KINGDOM_MISCHANGE_TABLE, rng)?);
        ("王国変動失敗", o, n)
    } else if head(command, "CAT") {
        let (o, n) = text_and_num(table_by_2d6(MK_CRITICAL_ATTACK_TABLE, rng)?);
        ("痛打", o, n)
    } else if head(command, "FWT") {
        let (o, n) = text_and_num(table_by_2d6(MK_FATAL_WOUNDS_TABLE, rng)?);
        ("致命傷", o, n)
    } else if head(command, "CFT") {
        let (o, n) = text_and_num(table_by_2d6(MK_COMBAT_FUMBLE_TABLE, rng)?);
        ("戦闘ファンブル", o, n)
    } else if head(command, "TT") {
        let (o, n) = text_and_num(table_by_2d6(MK_TRAVEL_TABLE, rng)?);
        ("道中", o, n)
    } else if head(command, "NT") {
        let (o, n) = text_and_num(table_by_2d6(MK_NEGOTIATION_TABLE, rng)?);
        ("交渉", o, n)
    } else if head(command, "ET") {
        let (o, n) = text_and_num(table_by_1d3(MK_EMOTION_TABLE, rng)?);
        ("感情", o, n)
        ...
        let n = rng.roll_d66(D66SortType::Asc)?;
        let out = match count {
            1..=3 => (hooks.kingdom_name_tables[(count - 1) as usize])(n),
            // Ruby: case count に一致する when がなく output は "" のまま
            _ => String::new(),
        };
        ("王国名", out, n.to_string())
    } else if head(command, "WORD") && !captured_count(command).is_empty() {
        // Ruby: /^WORD(\d+)/i（数字が必須）
        let count = get_count(captured_count(command));
        let n = rng.roll_d66(D66SortType::Asc)?;
        let out = match count {
            1..=4 => (hooks.word_tables[(count - 1) as usize])(n),
            _ => String::new(),
        };
        ("単語", out, n.to_string())
    } else if head(command, "ABT") {
        let (o, n) = text_and_num(table_by_2d6(MK_AFTERSEARCH_BREAK_TABLE, rng)?);
        ("捜索後休憩", o, n)
    } else if head(command, "WBT") {
        let (o, n) = text_and_num(table_by_2d6(MK_WHOLE_BREAK_TABLE, rng)?);
        ("全体休憩", o, n)
    } else if head(command, "LBT") {
        // Ruby `getLoversBreakTable`: D66を振ってから get_table_by_number
        let value = rng.roll_d66(D66SortType::Asc)?;
        (
            "カップル休憩",
            n_number(value, MK_LOVERS_BREAK_TABLE).to_string(),
            value.to_string(),
        )
    } else {
        return Ok(None);
...
/// Ruby `MeikyuKingdom#getAftersearchBreakTable` の表（`get_table_by_2d6`）。
static MK_AFTERSEARCH_BREAK_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#getLoversBreakTable` の表。
static MK_LOVERS_BREAK_TABLE: &[(i64, &str)] = &[
...
/// Ruby `MeikyuKingdom#getWholeBreakTable` の表（`get_table_by_2d6`）。
static MK_WHOLE_BREAK_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_army_research_table` の表（`get_table_by_2d6`）。
static MK_ARMY_RESEARCH_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_artifact_landscape_table` の表。
static MK_ARTIFACT_LANDSCAPE_TABLE: &[(i64, &str)] = &[
    (11, "石組みの部屋"),
    (12, "巨大な縦穴に刻まれた螺旋階段"),
    (13, "埃だらけの古い図書館"),
    (14, "古びた、素朴な祭壇"),
...
/// Ruby `MeikyuKingdom#mk_artifact_placename_table` の表。
static MK_ARTIFACT_PLACENAME_TABLE: &[(i64, &str)] = &[
    (11, "城（キャッスル）"),
    (12, "壁（ウォール）"),
    (13, "砦（フォート）"),
    (14, "地帯（ゾーン）"),
...
/// Ruby `MeikyuKingdom#mk_basic_decoration_table` の表。
static MK_BASIC_DECORATION_TABLE: &[(i64, &str)] = &[
    (11, "欲望（よくぼう）"),
    (12, "漂流（ひょうりゅう）"),
    (13, "黄金（おうごん）"),
    (14, "火達磨（ひだるま）"),
...
/// Ruby `MeikyuKingdom#mk_calture_research_table` の表（`get_table_by_2d6`）。
static MK_CALTURE_RESEARCH_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_cave_landscape_table` の表。
static MK_CAVE_LANDSCAPE_TABLE: &[(i64, &str)] = &[
    (11, "岩肌がむき出しの洞穴"),
    (12, "コウモリや羽蟲が飛び交う洞穴"),
    (13, "放置された坑道"),
    (14, "誰かのキャンプ跡"),
...
/// Ruby `MeikyuKingdom#mk_charm_break_table` の表（`get_table_by_2d6`）。
static MK_CHARM_BREAK_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_charm_happening_table` の表（`get_table_by_2d6`）。
static MK_CHARM_HAPPENING_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_combat_fumble_table` の表（`get_table_by_2d6`）。
static MK_COMBAT_FUMBLE_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_critical_attack_table` の表（`get_table_by_2d6`）。
static MK_CRITICAL_ATTACK_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_item_power_table` の表。
static MK_ITEM_POWER_TABLE: &[(i64, &str)] = &[
    (1, "〔才覚〕"),
    (2, "〔魅力〕"),
    (3, "〔探索〕"),
    (4, "〔武勇〕"),
...
/// Ruby `MeikyuKingdom#mk_emotion_table` の表（`get_table_by_1d3`）。
static MK_EMOTION_TABLE: &[&str] = &["忠誠／怒り", "友情／不信", "愛情／侮蔑"];

/// Ruby `MeikyuKingdom#mk_family_business_table` の表。
static MK_FAMILY_BUSINESS_TABLE: &[(i64, &str)] = &[
    (11, "星術師"),
    (12, "魔道師"),
    (13, "召喚師"),
    (14, "博士"),
...
/// Ruby `MeikyuKingdom#mk_fatal_wounds_table` の表（`get_table_by_2d6`）。
static MK_FATAL_WOUNDS_TABLE: &[&str] = &[
    "圧倒的一撃で急所を貫かれた。死亡する",
...
/// Ruby `MeikyuKingdom#mk_festival_break_table` の表（`get_table_by_2d6`）。
static MK_FESTIVAL_BREAK_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_festival_table` の表（`get_table_by_2d6`）。
static MK_FESTIVAL_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_item_attribute_table` の表。
static MK_ITEM_ATTRIBUTE_TABLE: &[(i64, &str)] = &[
    (1, "自然の力"),
    (2, "幻夢の力"),
    (3, "星炎の力"),
    (4, "暗黒の力"),
...
/// Ruby `MeikyuKingdom#mk_item_jyuka_table` の表。
static MK_ITEM_JYUKA_TABLE: &[(i64, &str)] = &[
    (1, "「呪い」のバッドステータス"),
    (2, "「肥満」のバッドステータス"),
    (3, "「愚か」のバッドステータス"),
...
/// Ruby `MeikyuKingdom#mk_item_jyumon_table` の表。
static MK_ITEM_JYUMON_TABLE: &[(i64, &str)] = &[
    (2, "モンスタースキル"),
    (3, "便利スキル"),
    (4, "芸能スキル"),
    (5, "迷宮スキル"),
...
/// Ruby `MeikyuKingdom#mk_katakana_decoration_table` の表。
static MK_KATAKANA_DECORATION_TABLE: &[(i64, &str)] = &[
    (11, "マヨネーズ"),
    (12, "ダイナマイト"),
    (13, "ドラゴン"),
    (14, "ボヨヨン"),
...
/// Ruby `MeikyuKingdom#mk_kingdom_change_table` の表（`get_table_by_2d6`）。
static MK_KINGDOM_CHANGE_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_kingdom_disaster_table` の表（`get_table_by_2d6`）。
static MK_KINGDOM_DISASTER_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_kingdom_mischange_table` の表（`get_table_by_2d6`）。
static MK_KINGDOM_MISCHANGE_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_kingdom_name_1_table` の表。
static MK_KINGDOM_NAME_1_TABLE: &[(i64, &str)] = &[
    (11, "暗黒"),
    (12, "王政"),
    (13, "超"),
    (14, "共和制"),
...
/// Ruby `MeikyuKingdom#mk_kingdom_name_2_table` の表。
static MK_KINGDOM_NAME_2_TABLE: &[(i64, &str)] = &[
    (11, "英雄"),
    (12, "連合"),
    (13, "グランドゼロ"),
    (14, "迷宮（ダンジョン）"),
...
/// Ruby `MeikyuKingdom#mk_kingdom_name_3_table` の表。
static MK_KINGDOM_NAME_3_TABLE: &[(i64, &str)] = &[
    (11, "王国（キングダム）"),
    (12, "王朝"),
    (13, "会社（公社）"),
    (14, "学園（学校）"),
...
/// Ruby `MeikyuKingdom#mk_life_item_table` の表。
static MK_LIFE_ITEM_TABLE: &[(i64, &str)] = &[
    (11, "バックパック"),
    (12, "バックパック"),
    (13, "鍋"),
    (14, "クラッカー"),
...
/// Ruby `MeikyuKingdom#mk_life_research_table` の表（`get_table_by_2d6`）。
static MK_LIFE_RESEARCH_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_market_price_table` の表（`get_table_by_2d6`）。
static MK_MARKET_PRICE_TABLE: &[&str] = &[
    "無し", "肉", "牙", "鉄", "魔素", "機械", "衣料", "木", "火薬", "情報", "革",
];

/// Ruby `MeikyuKingdom#mk_name_a_table` の表。
static MK_NAME_A_TABLE: &[(i64, &str)] = &[
    (11, "オレンジ／ジャスミン"),
    (12, "ホオズキ／アサガオ"),
    (13, "クローバー／ダチュラ"),
    (14, "ダフニ／キノコ"),
...
/// Ruby `MeikyuKingdom#mk_name_b_table` の表。
static MK_NAME_B_TABLE: &[(i64, &str)] = &[
    (11, "エイジ／ウェンズデイ"),
    (12, "ジョルノ／ノエル"),
    (13, "タスク／マニャーナ"),
    (14, "ウィンター／ジュノー"),
...
/// Ruby `MeikyuKingdom#mk_name_ex_table` の表。
static MK_NAME_EX_TABLE: &[(i64, &str)] = &[
    (11, "モアイ／スイショウドクロ"),
    (12, "チュパカブラ／ムベンベ"),
    (13, "カンフー／インヤン"),
    (14, "ブシドー／ミヤコ"),
...
/// Ruby `MeikyuKingdom#mk_name_fa_table` の表。
static MK_NAME_FA_TABLE: &[(i64, &str)] = &[
    (11, "アダム／イヴ"),
    (12, "ジャック／モモ"),
    (13, "オズ／アリス"),
    (14, "コナン／レダ"),
...
/// Ruby `MeikyuKingdom#mk_nick_a_table` の表。
static MK_NICK_A_TABLE: &[(i64, &str)] = &[
    (11, "“災い転じて福となす”"),
    (12, "“七転び八起きの”"),
    (13, "“冗談にも程がある”"),
    (14, "“虎の尾を踏む”"),
...
/// Ruby `MeikyuKingdom#mk_natural_landscape_table` の表。
static MK_NATURAL_LANDSCAPE_TABLE: &[(i64, &str)] = &[
    (11, "苔むした部屋"),
    (12, "動物の声が響き渡る密林"),
    (13, "つる草でできた通路"),
    (14, "空洞いっぱいのお花畑"),
...
/// Ruby `MeikyuKingdom#mk_natural_placename_table` の表。
static MK_NATURAL_PLACENAME_TABLE: &[(i64, &str)] = &[
    (11, "砂漠（デザート）"),
    (12, "丘陵（ヒル）"),
    (13, "海（オーシャン）"),
    (14, "森（フォレスト）"),
...
/// Ruby `MeikyuKingdom#mk_negotiation_table` の表（`get_table_by_2d6`）。
static MK_NEGOTIATION_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_nick_b_table` の表。
static MK_NICK_B_TABLE: &[(i64, &str)] = &[
    (11, "“身も蓋もない”"),
    (12, "“七人の敵がいる”"),
    (13, "“ドラゴンも裸足で逃げ出す”"),
    (14, "“われらが”"),
...
/// Ruby `MeikyuKingdom#mk_order_research_table` の表（`get_table_by_2d6`）。
static MK_ORDER_RESEARCH_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_passage_placename_table` の表。
static MK_PASSAGE_PLACENAME_TABLE: &[(i64, &str)] = &[
    (11, "門（ゲート）"),
    (12, "回廊（コリドー）"),
    (13, "通り（ストリート）"),
    (14, "小路（アレイ）"),
...
/// Ruby `MeikyuKingdom#mk_random_encount1_table` の表。
static MK_RANDOM_ENCOUNT1_TABLE: &[(i64, &str)] = &[
...
/// Ruby `MeikyuKingdom#mk_random_encount2_table` の表。
static MK_RANDOM_ENCOUNT2_TABLE: &[(i64, &str)] = &[
...
/// Ruby `MeikyuKingdom#mk_random_encount3_table` の表。
static MK_RANDOM_ENCOUNT3_TABLE: &[(i64, &str)] = &[
...
/// Ruby `MeikyuKingdom#mk_random_encount4_table` の表。
static MK_RANDOM_ENCOUNT4_TABLE: &[(i64, &str)] = &[
...
/// Ruby `MeikyuKingdom#mk_random_encount5_table` の表。
static MK_RANDOM_ENCOUNT5_TABLE: &[(i64, &str)] = &[
...
/// Ruby `MeikyuKingdom#mk_random_encount6_table` の表。
static MK_RANDOM_ENCOUNT6_TABLE: &[(i64, &str)] = &[
...
/// Ruby `MeikyuKingdom#mk_rare_item_table` の表。
static MK_RARE_ITEM_TABLE: &[(i64, &str)] = &[
    (11, "ブルーリボン"),
    (12, "聖痕"),
    (13, "剥製"),
    (14, "愚者の冠"),
...
/// Ruby `MeikyuKingdom#mk_rare_weapon_item_table` の表。
static MK_RARE_WEAPON_ITEM_TABLE: &[(i64, &str)] = &[
    (11, "虚弾"),
    (12, "怪物毒"),
    (13, "小鬼の襟巻"),
    (14, "喇叭銃"),
...
/// Ruby `MeikyuKingdom#mk_rest_item_table` の表。
static MK_REST_ITEM_TABLE: &[(i64, &str)] = &[
    (11, "お弁当"),
    (12, "お弁当"),
    (13, "特効薬"),
    (14, "保存食"),
...
/// Ruby `MeikyuKingdom#mk_search_break_table` の表（`get_table_by_2d6`）。
static MK_SEARCH_BREAK_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_search_happening_table` の表（`get_table_by_2d6`）。
static MK_SEARCH_HAPPENING_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_search_item_table` の表。
static MK_SEARCH_ITEM_TABLE: &[(i64, &str)] = &[
    (11, "星の欠片"),
    (12, "星の欠片"),
    (13, "旗"),
    (14, "お守り"),
...
/// Ruby `MeikyuKingdom#mk_skyrealm_landscape_table` の表。
static MK_SKYREALM_LANDSCAPE_TABLE: &[(i64, &str)] = &[
    (11, "雨が降る部屋"),
    (12, "チーズにうがたれた洞穴"),
    (13, "中空に何層にも重なる空中庭園"),
    (14, "無限に連なる真っ白な洗濯物"),
...
/// Ruby `MeikyuKingdom#mk_spooky_decoration_table` の表。
static MK_SPOOKY_DECORATION_TABLE: &[(i64, &str)] = &[
    (11, "赤錆（あかさび）"),
    (12, "串刺し（くしざし）"),
    (13, "鬼蜘蛛（おにぐも）"),
    (14, "蠍（さそり）"),
...
/// Ruby `MeikyuKingdom#mk_strange_place_landscape_table` の表。
static MK_STRANGE_PLACE_LANDSCAPE_TABLE: &[(i64, &str)] = &[
    (11, "古びた六畳間"),
    (12, "せせこましいカラオケボックス"),
    (13, "時の止まった街"),
    (14, "ボールが一個転がっている体育館"),
...
/// Ruby `MeikyuKingdom#mk_talent_break_table` の表（`get_table_by_2d6`）。
static MK_TALENT_BREAK_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_talent_happening_table` の表（`get_table_by_2d6`）。
static MK_TALENT_HAPPENING_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_travel_table` の表（`get_table_by_2d6`）。
static MK_TRAVEL_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_treasure1_table` の表（`get_table_by_1d6`）。
static MK_TREASURE1_TABLE: &[&str] = &[
    "何も無し",
    "何も無し",
...
/// Ruby `MeikyuKingdom#mk_treasure2_table` の表（`get_table_by_1d6`）。
static MK_TREASURE2_TABLE: &[&str] = &[
    "そのモンスターの素材欄の中から、好きな素材３個",
...
/// Ruby `MeikyuKingdom#mk_treasure3_table` の表（`get_table_by_1d6`）。
static MK_TREASURE3_TABLE: &[&str] = &[
    "そのモンスターの素材欄の中から、好きな素材５個",
...
/// Ruby `MeikyuKingdom#mk_treasure4_table` の表（`get_table_by_1d6`）。
static MK_TREASURE4_TABLE: &[&str] = &[
    "そのモンスターの素材欄の中から、好きな素材５個",
    "そのモンスターの素材欄の中から、好きな素材１０個",
...
/// Ruby `MeikyuKingdom#mk_treasure5_table` の表（`get_table_by_1d6`）。
static MK_TREASURE5_TABLE: &[&str] = &[
    "そのモンスターの素材欄の中から、好きな素材１０個",
...
/// Ruby `MeikyuKingdom#mk_valor_break_table` の表（`get_table_by_2d6`）。
static MK_VALOR_BREAK_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_valor_happening_table` の表（`get_table_by_2d6`）。
static MK_VALOR_HAPPENING_TABLE: &[&str] = &[
...
/// Ruby `MeikyuKingdom#mk_waterside_landscape_table` の表。
static MK_WATERSIDE_LANDSCAPE_TABLE: &[(i64, &str)] = &[
    (11, "轟々と流れる川にかかった橋"),
    (12, "色とりどりの珊瑚の中"),
    (13, "腰高まで水に浸かった部屋"),
    (14, "澄んだ水が流れる噴水と水飲み場"),
...
/// Ruby `MeikyuKingdom#mk_weapon_item_table` の表。
static MK_WEAPON_ITEM_TABLE: &[(i64, &str)] = &[
    (11, "だんびら"),
    (12, "だんびら"),
    (13, "ダガー"),
    (14, "戦斧"),
...
/// Ruby `MeikyuKingdom#mk_word_1_table` の表。
static MK_WORD_1_TABLE: &[(i64, &str)] = &[
    (11, "魔法"),
    (12, "おめかし"),
    (13, "狭いところ"),
    (14, "夜更かし"),
...
/// Ruby `MeikyuKingdom#mk_word_2_table` の表。
static MK_WORD_2_TABLE: &[(i64, &str)] = &[
    (11, "科学"),
    (12, "読書"),
    (13, "広いところ"),
    (14, "早起き"),
...
/// Ruby `MeikyuKingdom#mk_word_3_table` の表。
static MK_WORD_3_TABLE: &[(i64, &str)] = &[
    (11, "子供"),
    (12, "弱い人"),
    (13, "処刑"),
    (14, "叙事詩"),
...
/// Ruby `MeikyuKingdom#mk_word_4_table` の表。
static MK_WORD_4_TABLE: &[(i64, &str)] = &[
    (11, "年寄り"),
    (12, "強い人"),
    (13, "空想"),
    (14, "冗談"),
...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/MeikyuKingdom.toml");
    ...
    /// `test/data/MeikyuKingdom.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/MeikyuKingdom.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("MeikyuKingdom.toml must parse");
        assert_eq!(
            data.tests.len(),
            622,
            "case count in test/data/MeikyuKingdom.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "MeikyuKingdom",
                "unexpected game system in MeikyuKingdom.toml"
            );
            ...
            match eval_command(&GameSystemId::new("MeikyuKingdom"), &tc.input, &mut src) {
