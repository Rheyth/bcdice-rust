//! P4で手書き移植した `lib/bcdice/game_system/MetallicGuardian.rb`。
//!
...
//! - `MetallicGuardian` 自身の `set_aliases_for_srs_roll('MG')`（＝判定コマンドの表記に
//!   `2D6` の別名 `MG` を足す設定）
//!
//! # SRS判定の置き場所
//!
//! Ruby側は `SRS` クラスに判定本体があり、`MetallicGuardian` は別名を足すだけ。
//! Rust側の `SRS.rs` はまだ生成スタブのままで別バッチの担当なので、ここでは
//! 判定本体をこのファイルに持つ（`TenkaRyouran.rs` も同じ理由で同じ実装を持つ）。
//! `SRS` 本体が移植されたら、両者をそちらへ寄せて重複を畳める。
//!
//! ロケール差のある文言は [`SystemTables`] に束ね、
//! `MetallicGuardian_Korean`（`ko_kr`）が同じ関数群を使い回す。
...
/// 1ロケール分の設定と定型文。`MetallicGuardian` と `MetallicGuardian_Korean` はこれだけが違う。
...
/// Ruby `BCDice::GameSystem::MetallicGuardian`（ID: `MetallicGuardian`）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct MetallicGuardian;

impl GameSystem for MetallicGuardian {
...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/MetallicGuardian.toml");
    ...
    /// `test/data/MetallicGuardian.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/MetallicGuardian.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("MetallicGuardian.toml must parse");
        assert_eq!(
            data.tests.len(),
            19,
            "case count in test/data/MetallicGuardian.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "MetallicGuardian",
                "unexpected game system in MetallicGuardian.toml"
            );
            ...
            match eval_command(&GameSystemId::new("MetallicGuardian"), &tc.input, &mut src) {
