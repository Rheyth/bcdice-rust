...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/YuMyoKishi.toml");
    ...
    /// `test/data/YuMyoKishi.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/YuMyoKishi.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("YuMyoKishi.toml must parse");
        assert_eq!(
            data.tests.len(),
            28,
            "case count in test/data/YuMyoKishi.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "YuMyoKishi",
                "unexpected game system in YuMyoKishi.toml"
            );
            ...
            match eval_command(&GameSystemId::new("YuMyoKishi"), &tc.input, &mut src) {
