//! P4で手書き移植した `lib/bcdice/game_system/ShoujoTenrankai.rb`。
//!
...
//!
//! 移植したもの:
//! - `ShoujoTenrankai#eval_game_system_specific_command`（出来事表）
//! - `#getTensionText` / `#getEventTableResult`
//! - `EVENT_TABLES`（35表）
...
/// Ruby `BCDice::GameSystem::ShoujoTenrankai`（ID: `ShoujoTenrankai`）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ShoujoTenrankai;

impl GameSystem for ShoujoTenrankai {
...
"
    ...
    /// Ruby `ShoujoTenrankai#eval_game_system_specific_command`。
    fn eval_game_system_specific_command(
        &self,
        command: &str,
        rng: &mut Randomizer,
    ) -> Result<Option<SpecificCommandOutput>, EvalError> {
        let info = EVENT_TABLES
            .iter()
        ...
        let (text, number1, number2) = get_event_table_result(info.table, rng)?;
        let tension_text = get_tension_text(number1, number2);

        let result = format!(
            "出来事表：{}([{},{}]) ＞ {}{}",
            info.name, number1, number2, text, tension_text
        );
...
/// Ruby `ShoujoTenrankai#getTensionText`。
pub(crate) fn get_tension_text(number1: i64, number2: i64) -> String {
    let diff = (number1 - number2).abs();
    if diff == 0 {
        return String::new();
    }
...
/// Ruby `ShoujoTenrankai#getEventTableResult`。
///
/// 戻り値は `(text, number1, number2)`。Rubyは `text.nil?` で nil を返すが、
/// 表は常に12要素で index は 0..=11 に収まるため到達しない（防御的にエラー）。
pub(crate) fn get_event_table_result(
    table: &'static [&'static str],
    rng: &mut Randomizer,
) -> Result<(&'static str, i64, i64), EvalError> {
    let number1 = rng.roll_once(6)?;
    let number2 = rng.roll_once(6)?;

    // Ruby: isOdd = number1.even?（変数名に反して偶数のとき真）
    let is_even = number1 % 2 == 0;

    let mut index = (number2 - 1) as usize;
    if is_even {
        index += 6;
    }

    let text = table.get(index).copied();
    let Some(text) = text else {
        return Err(EvalError::Internal("event table index out of range"));
    };

    Ok((text, number1, number2))
}
...
/// Ruby `EVENT_TABLES`。
static EVENT_TABLES: &[(&str, EventTable)] = &[
    ("SPRING", EventTable { name: "春", table: &[
    ...
    ("SUMMER", EventTable { name: "夏", table: &[
    ...
    ("AUTUMN", EventTable { name: "秋", table: &[
        "【奇・1】たわわに実った果物があります。",
    ...
    ("WINTER", EventTable { name: "冬", table: &[
        "【奇・1】太陽（月）に暈がかかっています。",
        "【奇・2】風花が舞っています。",
        "【奇・3】雪が積もっています。",
    ...
    ("MORNING", EventTable { name: "朝", table: &[
        "【奇・1】起きたら涙を流していました。",
    ...
    ("AFTERNOON", EventTable { name: "昼下がり", table: &[
        "【奇・1】快い眠気に襲われています。",
        "【奇・2】まだ食べ足りない感じがします。",
    ...
    ("TWILIGHT", EventTable { name: "黄昏", table: &[
    ...
    ("NIGHT", EventTable { name: "夜中", table: &[
        "【奇・1】なかなか眠くなれません。",
    ...
    ("MIDNIGHT", EventTable { name: "夜更け", table: &[
    ...
    ("NORBLE", EventTable { name: "貴族趣味", table: &[
    ...
    ("MARCHEN", EventTable { name: "メルヒェン", table: &[
    ...
    ("COLONIAL", EventTable { name: "コロニアル", table: &[
        "【奇・1】重厚な調度が征服欲を刺激します。",
        "【奇・2】床に傷を見つけてしまいました。",
        "【奇・3】重厚な調度に頼もしさを感じます。",
    ...
    ("SHELF", EventTable { name: "本棚のある場所", table: &[
    ...
    ("LITTLE", EventTable { name: "小さな妹の部屋", table: &[
    ...
    ("ELDER", EventTable { name: "素敵なお姉さまの部屋", table: &[
    ...
    ("ATERIEL", EventTable { name: "アトリエ", table: &[
    ...
    ("OPEN", EventTable { name: "集まれる場所", table: &[
    ...
    ("HAUNTED", EventTable { name: "精神的瑕疵物件", table: &[
    ...
    ("SIMPLE", EventTable { name: "侘び住まい", table: &[
        "【奇・1】若干、風通しがよすぎるようです。",
        "【奇・2】いろいろすっきりしています。",
        "【奇・3】壁のひびを見つけました。",
        "【奇・4】隅っこに穴が開いています。",
    ...
    ("LARGE", EventTable { name: "広い場所", table: &[
    ...
    ("CORRIDOR", EventTable { name: "廊下", table: &[
    ...
    ("STAIRS", EventTable { name: "階段", table: &[
    ...
    ("COSY", EventTable { name: "居心地のいい場所", table: &[
        "【奇・1】一瞬、物音が全て途絶えました。",
    ...
    ("TERRACE", EventTable { name: "テラス・ベランダ", table: &[
        "【奇・1】木の枝や木の葉が積もっています。",
    ...
    ("GARRET", EventTable { name: "屋根裏", table: &[
    ...
    ("KITCHEN", EventTable { name: "台所", table: &[
    ...
    ("REST", EventTable { name: "化粧室", table: &[
    ...
    ("CELLER", EventTable { name: "地下倉庫", table: &[
    ...
    ("LUMBER", EventTable { name: "物置", table: &[
    ...
    ("GARDEN", EventTable { name: "庭園", table: &[
    ...
    ("WASTED", EventTable { name: "廃園", table: &[
    ...
    ("SHORE", EventTable { name: "水のほとり", table: &[
        "【奇・1】水面に波紋が広がっていきます。",
        "【奇・2】視界の外で水音がしました。",
        "【奇・3】生臭い匂いがします。",
...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path =
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../test/data/ShoujoTenrankai.toml");
        path.exists().then_some(path)
    }

    /// `test/data/ShoujoTenrankai.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/ShoujoTenrankai.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("ShoujoTenrankai.toml must parse");
        assert_eq!(
            data.tests.len(),
            72,
            "case count in test/data/ShoujoTenrankai.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "ShoujoTenrankai",
                "unexpected game system in ShoujoTenrankai.toml"
            );
            ...
            match eval_command(&GameSystemId::new("ShoujoTenrankai"), &tc.input, &mut src) {
