...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path =
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../test/data/DivineCharger.toml");
        path.exists().then_some(path)
    }

    /// `test/data/DivineCharger.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/DivineCharger.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("DivineCharger.toml must parse");
        assert_eq!(
            data.tests.len(),
            71,
            "case count in test/data/DivineCharger.toml"
        );
        ...
        for (i, tc) in data.tests.iter().enumerate() {
            ...
            match eval_command(&GameSystemId::new("DivineCharger"), &tc.input, &mut src) {
