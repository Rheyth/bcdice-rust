...
mod tests {
    ...
    ///
    /// upstream の TOML は 14件目 `(4-3)WW12` に4本の `rands` を持つが、
    /// 前処理でコマンドが `1WW12` になるため実際に振るのは1本だけ。
    /// Ruby の `RandomizerMock` は余りを許す（枯渇したときだけ raise する）ので
    /// 本家のテストは通る。ここだけ余りを許容するが、本数まで厳密に一致させる
    /// （ダイスを振る本数がずれれば必ず失敗する）。それ以外の全ケースでは
    /// `rust/tests/toml_harness.rs::run_case` と同じく余り0本を要求する。
    const SURPLUS_RANDS_ALLOWED: &[(usize, usize)] = &[(14, 3)];

    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/WoW.toml");
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/WoW.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("WoW.toml must parse");
        assert_eq!(data.tests.len(), 19, "case count in test/data/WoW.toml");

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(tc.game_system, "WoW", "unexpected game system in WoW.toml");

            let mut reasons: Vec<String> = Vec::new();
            let rands: Vec<(i64, i64)> = tc.rands.iter().map(|r| (r.value, r.sides)).collect();
            let mut src = SeededRandomizer::new(rands);

            match eval_command(&GameSystemId::new("WoW"), &tc.input, &mut src) {
