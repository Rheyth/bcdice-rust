...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/Nechronica.toml");
    ...
    /// `test/data/Nechronica.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Nechronica.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Nechronica.toml must parse");
        assert_eq!(
            data.tests.len(),
            37,
            "case count in test/data/Nechronica.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Nechronica",
                "unexpected game system in Nechronica.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Nechronica"), &tc.input, &mut src) {
