...
mod tests {
    ...
    /// `test/data/Emoklore.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Emoklore.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Emoklore.toml must parse");
        assert_eq!(
            data.tests.len(),
            33,
            "case count in test/data/Emoklore.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Emoklore",
                "unexpected game system in Emoklore.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Emoklore"), &tc.input, &mut src) {
