//! P4で手書き移植した `lib/bcdice/game_system/DemonParasite.rb`。
//!
...
//! - `DemonParasite#get_urge`（衝動表 `URGEx` / 新衝動表 `NURGEx` / 誤作動表 `AURGEx` /
//!   ミュータント衝動表 `MURGEx` / 鬼御魂(戦闘外)衝動表 `UURGEx` / 鬼御魂(戦闘中)衝動表 `CURGEx`）
//!
//! 表データ（`*_TABLE`）は原典 rb から機械的に書き出したもので、値は1文字も変えていない。
...
/// Ruby `BCDice::GameSystem::DemonParasite`（ID: `DemonParasite`）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct DemonParasite;

impl GameSystem for DemonParasite {
...
/// Ruby `DemonParasite#get_urge` の `/([NAMUC])?URGE\s*(\d+)/i`。
///
/// Ruby の `\d` / `\s` は ASCII 限定なので、Unicode 既定の `regex` クレートでは
/// 文字クラスを明示する。
fn urge_pattern() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| {
        Regex::new(r"(?i)([NAMUC])?URGE[\t\n\x0B\x0C\r ]*([0-9]+)").expect("valid regex")
    })
}

/// Ruby `DemonParasite#get_urge`。衝動表。
///
/// 入力は `Base#dice_command` が大文字化済み（`@enabled_upcase_input`）なので、
/// 識別文字は常に大文字で届く。`case initialWord` は大文字だけを受けるので、
/// 小文字が来た場合（到達しない）は Ruby と同じく `'1'`（＝出力なし）に落とす。
fn get_urge(
    string: &str,
    rng: &mut Randomizer,
) -> Result<Option<SpecificCommandOutput>, EvalError> {
    ...
    let initial_word = captures.get(1).map(|m| m.as_str());
    // Ruby: `m[2].to_i`。桁あふれは Bignum になるが、どのみち 1..5 の範囲外なので
    // エラー文言に落ちる。
    let urge_lv: i64 = captures[2].parse().unwrap_or(i64::MAX);

    let (title, urge): (&str, &[[&str; 11]; 5]) = match initial_word {
        None => ("衝動表", &URGE_TABLE),
        Some("N") => ("新衝動表", &NEW_URGE_TABLE),
        Some("A") => ("誤作動表", &MALFUNCTION_TABLE),
        Some("M") => ("ミュータント衝動表", &MUTANT_TABLE),
        Some("U") => ("鬼御魂(戦闘外)衝動表", &ONIMITAMA_OUT_OF_BATTLE_TABLE),
        Some("C") => ("鬼御魂(戦闘中)衝動表", &ONIMITAMA_BATTLE_TABLE),
        // Ruby: あり得ない文字
        Some(_) => return Ok(Some(SpecificCommandOutput::text("1"))),
    };
    ...
    // urge_lv は 1..=5、dice_now は 2..=12 に収まるので添字は必ず有効
    let result_text = urge[(urge_lv - 1) as usize][(dice_now - 2) as usize];

    Ok(Some(SpecificCommandOutput::text(format!(
        "{title}{urge_lv}-{dice_now}:{result_text}"
    ))))
}

/// Ruby `DemonParasite::URGE_TABLE`（衝動表 / 2D6）。
///
/// 添字は `[衝動段階 - 1][2D6の出目 - 2]`。
static URGE_TABLE: [[&str; 11]; 5] = [
    [
...
/// Ruby `DemonParasite::NEW_URGE_TABLE`（新衝動表 / 2D6）。
///
/// 添字は `[衝動段階 - 1][2D6の出目 - 2]`。
static NEW_URGE_TABLE: [[&str; 11]; 5] = [
    [
...
/// Ruby `DemonParasite::MALFUNCTION_TABLE`（誤作動表 / 2D6）。
///
/// 添字は `[衝動段階 - 1][2D6の出目 - 2]`。
static MALFUNCTION_TABLE: [[&str; 11]; 5] = [
    [
...
/// Ruby `DemonParasite::MUTANT_TABLE`（ミュータント衝動表 / 2D6）。
///
/// 添字は `[衝動段階 - 1][2D6の出目 - 2]`。
static MUTANT_TABLE: [[&str; 11]; 5] = [
    [
...
/// Ruby `DemonParasite::ONIMITAMA_OUT_OF_BATTLE_TABLE`（鬼御魂(戦闘外)衝動表 / 2D6）。
///
/// 添字は `[衝動段階 - 1][2D6の出目 - 2]`。
static ONIMITAMA_OUT_OF_BATTLE_TABLE: [[&str; 11]; 5] = [
    [
...
/// Ruby `DemonParasite::ONIMITAMA_BATTLE_TABLE`（鬼御魂(戦闘中)衝動表 / 2D6）。
///
/// 添字は `[衝動段階 - 1][2D6の出目 - 2]`。
static ONIMITAMA_BATTLE_TABLE: [[&str; 11]; 5] = [
    [
...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/DemonParasite.toml");
    ...
    /// `test/data/DemonParasite.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/DemonParasite.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("DemonParasite.toml must parse");
        assert_eq!(
            data.tests.len(),
            98,
            "case count in test/data/DemonParasite.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "DemonParasite",
                "unexpected game system in DemonParasite.toml"
            );
            ...
            match eval_command(&GameSystemId::new("DemonParasite"), &tc.input, &mut src) {
