...
mod tests {
    ...
    /// `test/data/Villaciel.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Villaciel.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Villaciel.toml must parse");
        assert_eq!(
            data.tests.len(),
            51,
            "case count in test/data/Villaciel.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Villaciel",
                "unexpected game system in Villaciel.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Villaciel"), &tc.input, &mut src) {
