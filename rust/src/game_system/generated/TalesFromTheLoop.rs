...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/TalesFromTheLoop.toml");
    ...
    /// `test/data/TalesFromTheLoop.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/TalesFromTheLoop.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("TalesFromTheLoop.toml must parse");
        assert_eq!(
            data.tests.len(),
            42,
            "case count in test/data/TalesFromTheLoop.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "TalesFromTheLoop",
                "unexpected game system in TalesFromTheLoop.toml"
            );
            ...
            match eval_command(&GameSystemId::new("TalesFromTheLoop"), &tc.input, &mut src) {
