...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/HarnMaster.toml");
    ...
    /// `test/data/HarnMaster.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/HarnMaster.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("HarnMaster.toml must parse");
        assert_eq!(
            data.tests.len(),
            18,
            "case count in test/data/HarnMaster.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "HarnMaster",
                "unexpected game system in HarnMaster.toml"
            );
            ...
            match eval_command(&GameSystemId::new("HarnMaster"), &tc.input, &mut src) {
