...
mod tests {
    ...
    /// `test/data/AFF2e.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/AFF2e.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("AFF2e.toml must parse");
        assert_eq!(data.tests.len(), 19, "case count in test/data/AFF2e.toml");

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "AFF2e",
                "unexpected game system in AFF2e.toml"
            );
            ...
            match eval_command(&GameSystemId::new("AFF2e"), &tc.input, &mut src) {
