...
mod tests {
    ...
    /// `test/data/AniMalus.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/AniMalus.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("AniMalus.toml must parse");
        assert_eq!(
            data.tests.len(),
            31,
            "case count in test/data/AniMalus.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "AniMalus",
                "unexpected game system in AniMalus.toml"
            );
            ...
            match eval_command(&GameSystemId::new("AniMalus"), &tc.input, &mut src) {
