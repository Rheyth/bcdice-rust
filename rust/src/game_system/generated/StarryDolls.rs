//! P4で手書き移植した `lib/bcdice/game_system/StarryDolls.rb`。
//!
//! メタデータ（id/name/sort_key/help_message/prefixes/settings）は生成スタブの値を保ち、
//! 表データは `i18n/StarryDolls/ja_jp.yml` から機械的に展開している。

use crate::command_parser::{Parser, SuffixPosition};
use crate::dice_table::sai_fic_skill_table::SaiFicSkillTable;
use crate::dice_table::{
    ChainTable, RollableTable, SaiFicCategory, SaiFicFormats, Table, TableItem,
};
...
pub(crate) static STAR_PREFIXES: &[&str] = &[
    "\\d+D6",
    "RTT[1-6]?",
    "RCT",
    "HORA",
...
// i18n/StarryDolls/ja_jp.yml から機械的に展開した表データ。
static JA_HORA_ITEMS: &[&str] = &[
    "おひつじ座",
    "おうし座",
    "ふたご座",
    "かに座",
...
static JA_HORA: Table = Table::from_dice("ランダム星座表A", 1, 6, JA_HORA_ITEMS);

static JA_HORB_ITEMS: &[&str] = &[
    "てんびん座",
    "さそり座",
    "いて座",
    "やぎ座",
...
static JA_HORB: Table = Table::from_dice("ランダム星座表B", 1, 6, JA_HORB_ITEMS);

static JA_HOR_ITEMS: &[TableItem] = &[
    TableItem::Table(&JA_HORA),
    TableItem::Table(&JA_HORA),
    TableItem::Table(&JA_HORA),
    TableItem::Table(&JA_HORB),
    TableItem::Table(&JA_HORB),
    TableItem::Table(&JA_HORB),
];
static JA_HOR: ChainTable = ChainTable::from_dice("ランダム星座表", 1, 6, JA_HOR_ITEMS);

static JA_MRT_ITEMS: &[&str] = &["親", "友人", "恩師", "相棒", "上司", "恋人"];
static JA_MRT: Table = Table::from_dice("主人関係表", 1, 6, JA_MRT_ITEMS);

static JA_RAT_ITEMS: &[&str] = &[
    "尊敬／優越感",
    "理解／反抗",
    "愉快／恐怖",
    "友情／競争",
...
static JA_RAT: Table = Table::from_dice("関係属性表", 1, 6, JA_RAT_ITEMS);

static JA_MIR_ITEMS: &[&str] = &[
...
static JA_MIR: Table = Table::from_dice("奇跡表", 1, 6, JA_MIR_ITEMS);

static JA_BRT_ITEMS: &[&str] = &[
...
static JA_BRT: Table = Table::from_dice("戦果表", 1, 6, JA_BRT_ITEMS);

static JA_TRO_ITEMS: &[&str] = &[
...
static JA_TRO: Table = Table::from_dice("事件表", 2, 6, JA_TRO_ITEMS);

static JA_ENC_ITEMS: &[&str] = &[
...
static JA_ENC: Table = Table::from_dice("遭遇表", 1, 6, JA_ENC_ITEMS);

static JA_FWT_ITEMS: &[&str] = &[
...
static JA_FWT: Table = Table::from_dice("致命傷表", 1, 6, JA_FWT_ITEMS);

static JA_CAT_ITEMS: &[&str] = &[
...
static JA_CAT: Table = Table::from_dice("カタストロフ表", 1, 6, JA_CAT_ITEMS);

static JA_JDSRT_ITEMS: &[&str] = &[
...
static JA_JDSRT: Table = Table::from_dice("〈魔術師の庭〉回想表", 1, 6, JA_JDSRT_ITEMS);

static JA_SHRT_ITEMS: &[&str] = &[
...
static JA_SHRT: Table = Table::from_dice("〈セブンス・ヘブン〉回想表", 1, 6, JA_SHRT_ITEMS);

static JA_BCRT_ITEMS: &[&str] = &[
...
static JA_BCRT: Table = Table::from_dice("〈祝福の鐘〉回想表", 1, 6, JA_BCRT_ITEMS);

static JA_ODRT_ITEMS: &[&str] = &[
...
static JA_ODRT: Table = Table::from_dice("〈オメガ探偵社〉回想表", 1, 6, JA_ODRT_ITEMS);

static JA_SRT_ITEMS: &[&str] = &[
    "悪友",
    "きょうだい",
    "先輩／後輩",
    "相談相手",
...
static JA_SRT: Table = Table::from_dice("従者関係表", 1, 6, JA_SRT_ITEMS);

static JA_TRS_ITEMS: &[&str] = &[
...
static JA_TRS: Table = Table::from_dice("従者トラブル表", 2, 6, JA_TRS_ITEMS);

static JA_RAL_ITEMS: &[&str] = &[
    "仰せの通りに",
    "いえ、そのようなことは……",
    "我が主の判断に誤りなどございません",
    "ここは私が",
...
static JA_RAL: Table = Table::from_dice("リアクション表　忠誠", 1, 6, JA_RAL_ITEMS);

static JA_RAC_ITEMS: &[&str] = &[
    "嫌な予感がするわね",
    "あなたがどうしたいかが大切よ",
    "私にはわからないわ",
    "気にする必要はないわ",
...
static JA_RAC: Table = Table::from_dice("リアクション表　冷静", 1, 6, JA_RAC_ITEMS);

static JA_RAM_ITEMS: &[&str] = &[
    "あらあら、どうしたのかしら",
    "頭をなでて差し上げましょう",
    "素晴らしいことですわ",
    "あなたの思う通りやっていいのですよ",
...
static JA_RAM: Table = Table::from_dice("リアクション表　母性", 1, 6, JA_RAM_ITEMS);

static JA_RAO_ITEMS: &[&str] = &[
    "大丈夫、心配しなくていいよ",
    "自分を信じるんだ",
    "フフッ、君は本当にかわいいね",
    "君の力はこんなものじゃないだろう？",
...
static JA_RAO: Table = Table::from_dice("リアクション表　年長者", 1, 6, JA_RAO_ITEMS);

static JA_RAI_ITEMS: &[&str] = &[
    "やっちゃう～？",
    "楽しそうだね！",
    "ボクにまかせて！",
    "ボク、難しいことはわかんな～い",
...
static JA_RAI: Table = Table::from_dice("リアクション表　無邪気", 1, 6, JA_RAI_ITEMS);

static JA_RAE_ITEMS: &[&str] = &[
    "ほっほ、元気がいいのう",
    "どれ、ワシがやってみよう",
    "こりゃあたまげた",
    "ム？　ウ、ウム、そうじゃな……",
...
static JA_RAE: Table = Table::from_dice("リアクション表　長老", 1, 6, JA_RAE_ITEMS);

static JA_JDSBT_ITEMS: &[&str] = &[
...
static JA_JDSBT: Table = Table::from_dice("〈魔術師の庭〉出張表", 1, 6, JA_JDSBT_ITEMS);

static JA_SHBT_ITEMS: &[&str] = &[
...
static JA_SHBT: Table = Table::from_dice("〈セブンス・ヘブン〉出張表", 1, 6, JA_SHBT_ITEMS);

static JA_BCBT_ITEMS: &[&str] = &[
...
static JA_BCBT: Table = Table::from_dice("〈祝福の鐘〉出張表", 1, 6, JA_BCBT_ITEMS);

static JA_ODBT_ITEMS: &[&str] = &[
...
static JA_ODBT: Table = Table::from_dice("〈オメガ探偵社〉出張表", 1, 6, JA_ODBT_ITEMS);

static JA_ASBT_ITEMS: &[&str] = &[
...
static JA_ASBT: Table = Table::from_dice("〈天の川商店街〉出張表", 1, 6, JA_ASBT_ITEMS);

static JA_PABT_ITEMS: &[&str] = &[
...
static JA_PABT: Table = Table::from_dice("〈ポラリス星学院〉出張表", 1, 6, JA_PABT_ITEMS);

static JA_SCBT_ITEMS: &[&str] = &[
...
static JA_SCBT: Table = Table::from_dice("〈人形騎士団〉出張表", 1, 6, JA_SCBT_ITEMS);

static JA_TOSCBT_ITEMS: &[&str] = &[
...
static JA_TOSCBT: Table = Table::from_dice("〈人形騎士団〉への出張表", 1, 6, JA_TOSCBT_ITEMS);

static JA_TROF_ITEMS: &[&str] = &[
...
static JA_TROF: Table = Table::from_dice("森事件表", 2, 6, JA_TROF_ITEMS);

static JA_TROG_ITEMS: &[&str] = &[
...
static JA_TROG: Table = Table::from_dice("庭園事件表", 2, 6, JA_TROG_ITEMS);

static JA_TROC_ITEMS: &[&str] = &[
...
static JA_TROC: Table = Table::from_dice("城内事件表", 2, 6, JA_TROC_ITEMS);

static JA_TROT_ITEMS: &[&str] = &[
...
static JA_TROT: Table = Table::from_dice("都市事件表", 2, 6, JA_TROT_ITEMS);

static JA_TROL_ITEMS: &[&str] = &[
...
static JA_TROL: Table = Table::from_dice("図書館事件表", 2, 6, JA_TROL_ITEMS);

static JA_TROS_ITEMS: &[&str] = &[
...
static JA_TROS: Table = Table::from_dice("駅事件表", 2, 6, JA_TROS_ITEMS);

static JA_RTT_CATEGORY1: &[&str] = &[
    "創造",
    "研究",
    "守護",
    "絆",
...
static JA_RTT_CATEGORIES: &[SaiFicCategory] = &[
    SaiFicCategory::new("願望", JA_RTT_CATEGORY1),
    SaiFicCategory::new("元素", JA_RTT_CATEGORY2),
    SaiFicCategory::new("星使い", JA_RTT_CATEGORY3),
    SaiFicCategory::new("動作", JA_RTT_CATEGORY4),
    SaiFicCategory::new("召喚", JA_RTT_CATEGORY5),
    SaiFicCategory::new("人間性", JA_RTT_CATEGORY6),
];
static JA_RTT: SaiFicSkillTable =
    SaiFicSkillTable::new(JA_RTT_CATEGORIES).with_formats(SaiFicFormats::DEFAULT);
...
pub(crate) fn check_result_nd6(
    ...
    let result = if cmp_op.apply(&total, &target) {
...
fn action_roll(
    ...
    let parser = Parser::new(&["D6"], RoundType::Floor)
        .has_prefix_number()
    ...
    if special <= fumble {
        special = fumble + 1;
        cmd.critical = Some(crate::Int::from(special));
    }

    let dice_list = rng.roll_barabara(times, 6)?;
    let dice_total = dice_list
        .iter()
        .fold(0_i64, |sum, dice| sum.wrapping_add(*dice));
    let total = dice_total.wrapping_add(crate::randomizer::sat_i64(&cmd.modify_number));

    let mut result = if dice_total <= fumble {
        EvalResult::fumble(sys.fumble)
    } else if dice_total >= special {
        EvalResult::critical(sys.special)
    } else if cmd.question_target {
        EvalResult::new()
    } else if total
        >= crate::randomizer::sat_i64(&cmd.target_number.clone().unwrap_or(crate::Int::from(0)))
    {
        EvalResult::success(sys.success)
    } else {
        EvalResult::failure(sys.failure)
    ...
    let mut sequence = vec![
        ...
        format!("{dice_text}{}", modifier(&cmd.modify_number)),
        total.to_string(),
    ];
...
pub struct StarryDolls;

impl GameSystem for StarryDolls {
