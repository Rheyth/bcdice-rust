...
mod tests {
    ...
    /// `test/data/FullFace.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/FullFace.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("FullFace.toml must parse");
        assert_eq!(
            data.tests.len(),
            14,
            "case count in test/data/FullFace.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "FullFace",
                "unexpected game system in FullFace.toml"
            );
            ...
            match eval_command(&GameSystemId::new("FullFace"), &tc.input, &mut src) {
