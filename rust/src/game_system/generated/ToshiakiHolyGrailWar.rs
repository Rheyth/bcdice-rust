...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/ToshiakiHolyGrailWar.toml");
    ...
    /// `test/data/ToshiakiHolyGrailWar.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/ToshiakiHolyGrailWar.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("ToshiakiHolyGrailWar.toml must parse");
        assert_eq!(
            data.tests.len(),
            20,
            "case count in test/data/ToshiakiHolyGrailWar.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "ToshiakiHolyGrailWar",
                "unexpected game system in ToshiakiHolyGrailWar.toml"
            );
            ...
            match eval_command(
                &GameSystemId::new("ToshiakiHolyGrailWar"),
                &tc.input,
                &mut src,
            ) {
        ...
        assert!(
            failures.is_empty(),
            "{}/{} ToshiakiHolyGrailWar cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
