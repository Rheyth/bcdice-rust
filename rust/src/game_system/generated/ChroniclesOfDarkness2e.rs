...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/ChroniclesOfDarkness2e.toml");
    ...
    /// `test/data/ChroniclesOfDarkness2e.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/ChroniclesOfDarkness2e.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("ChroniclesOfDarkness2e.toml must parse");
        assert_eq!(
            data.tests.len(),
            43,
            "case count in test/data/ChroniclesOfDarkness2e.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "ChroniclesOfDarkness2e",
                "unexpected game system in ChroniclesOfDarkness2e.toml"
            );
        ...
        assert!(
            failures.is_empty(),
            "{}/{} ChroniclesOfDarkness2e cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
