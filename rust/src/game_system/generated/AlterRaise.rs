...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/AlterRaise.toml");
    ...
    /// `test/data/AlterRaise.toml` の全ケースが通ること。
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/AlterRaise.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("AlterRaise.toml must parse");
        assert_eq!(
            data.tests.len(),
            23,
            "case count in test/data/AlterRaise.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "AlterRaise",
                "unexpected game system in AlterRaise.toml"
            );
            ...
            match eval_command(&GameSystemId::new("AlterRaise"), &tc.input, &mut src) {
