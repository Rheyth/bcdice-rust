...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/EmbryoMachine.toml");
    ...
    /// `test/data/EmbryoMachine.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/EmbryoMachine.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("EmbryoMachine.toml must parse");
        assert_eq!(
            data.tests.len(),
            133,
            "case count in test/data/EmbryoMachine.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "EmbryoMachine",
                "unexpected game system in EmbryoMachine.toml"
            );
            ...
            match eval_command(&GameSystemId::new("EmbryoMachine"), &tc.input, &mut src) {
