...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/RecordOfSteam.toml");
    ...
    /// `test/data/RecordOfSteam.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/RecordOfSteam.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("RecordOfSteam.toml must parse");
        assert_eq!(
            data.tests.len(),
            8,
            "case count in test/data/RecordOfSteam.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "RecordOfSteam",
                "unexpected game system in RecordOfSteam.toml"
            );
            ...
            match eval_command(&GameSystemId::new("RecordOfSteam"), &tc.input, &mut src) {
