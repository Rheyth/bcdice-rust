...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/AnimaAnimus.toml");
    ...
    /// `test/data/AnimaAnimus.toml` の全ケースが通ること。
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/AnimaAnimus.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("AnimaAnimus.toml must parse");
        assert_eq!(
            data.tests.len(),
            23,
            "case count in test/data/AnimaAnimus.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "AnimaAnimus",
                "unexpected game system in AnimaAnimus.toml"
            );
            ...
            match eval_command(&GameSystemId::new("AnimaAnimus"), &tc.input, &mut src) {
