//! P4で手書き移植した `lib/bcdice/game_system/Insane_Korean.rb`。
//!
//! メタデータ（id/name/sort_key/help_message/prefixes/settings）は
//! `rust/tools/generate_game_systems.rb` が生成したスタブの値をそのまま保っている。
//! 生成スクリプトを再実行するとこのファイルはスタブへ戻るので注意。
//!
//! Ruby側は `Insane` を継承し `@locale = :ko_kr` で表を組み直すだけなので、
//! 表の引き方・コマンド解釈は [`super::Insane`] の実装をそのまま使い、
//! ここには `ko_kr` ロケールの表データだけを置く。
//! ko_kr に無いキー（`Insane.IRN.name`）は i18n gem の fallback と同じく `ja_jp` の値になる。

use super::Insane::{check_result_2d6, eval_specific_command, SystemTables, JA_IRN_NAME};
use crate::dice_table::sai_fic_skill_table::DEFAULT_SKILL_FORMAT;
use crate::dice_table::{
    D66Table, RollableTable, SaiFicCategory, SaiFicFormats, SaiFicSkillTable, Table, TableItem,
};
use crate::enums::D66SortType;
use crate::eval::EvalError;
use crate::game_system::{GameSystem, SpecificCommandOutput, Target};
use crate::normalize::CmpOp;
use crate::randomizer::Randomizer;
use crate::result::CheckOutcome;

static KO_ST_ITEMS: &[&str] = &[
    "주위가 피 냄새로 가득하다. 사건인가? 사고인가? 혹시 그것은 지금도 계속되고 있는 걸까?",
    "이것은…… 꿈인가? 이미 지나갔을 과거가 기억 속에서 되살아난다.",
    "눈 앞에 펼쳐진 거리의 풍경을 내려다본다. 왜 이렇게 높은 곳에……?",
    "세상의 끝처럼 느껴지는 어둠. 어둠 속에서 누군가가 움직이고 있다…….",
    "평화로운 시간이 흘러간다. 마치 그런 일이 없었던 것처럼.",
    "축축한 흙냄새. 농밀한 기척이 풍기는 숲속. 새나 벌레의 소리, 바람에 나무가 살랑거리는 소리가 들려온다.",
    "인적이 드문 주택가. 낯선 사람들이 사는 집 안에서는 불분명한 목소리나 소음이 새어 나온다…….",
    "갑자기 구름이 하늘을 뒤덮더니 세찬 비가 내린다. 사람들은 처마를 찾아 황급히 달려간다.",
    "황폐한 폐허. 쇠퇴한 생활의 흔적. 희미하게 들려오는 것은 바람 소리인가? 파도 소리인가? 귀울림인가?",
    "사람들. 떠들썩한 소리. 요란한 가게 내부의 BGM에, 이질적인 웃음소리. 소란스러운 번화가의 한구석인데……?",
    "밝은 빛을 받으며 안도의 한숨. 하지만 빛이 강할수록 그림자도 더 짙어진다…….",
];
static KO_ST: Table = Table::from_dice("장면표", 2, 6, KO_ST_ITEMS);

static KO_HJST_ITEMS: &[&str] = &[
    "갑자기 주위가 어두워진다. 정전인가? 어둠 속에서 누군가가 당신을 부르는 소리가 들려온다.",
    "똑. 똑. 똑. 어딘가에서 물방울 떨어지는 소리가 들린다.",
    "유리창 앞을 지나갈 때, 기분 나쁜 뭔가가 비친다. 눈의 착각인가……?",
    "TV에서 뉴스가 들린다. 아무래도 근처에서 끔찍한 사건이 있었던 모양인데…….",
    "어두운 길을 홀로 걷고 있다. 등 뒤에서 기분 나쁜 발소리가 다가오는 것 같은 기분이 드는데…….",
    "누구지? 계속 시선을 느낀다. 돌아봐도 일상적인 광경이 보일 뿐인데…….",
    "갑자기 핸드폰이 울린다. 매너 모드로 해뒀을 텐데……. 도대체 누구지?",
    "어두운 적색의 석양. 태양은 저물어가고, 하늘은 피처럼 붉다. 불안한 기분이 커져 간다…….",
    "어딘가에서 풍겨오는 맛있는 냄새에 갑자기 배가 고파졌다. 오늘은 뭘 먹을까?",
    "날카로운 울음소리가 울려 퍼진다. 고양이나 어린애가 어딘가에서 울고 있는 걸까? 아니면…….",
    "자다가 괴로워져서 눈을 뜬다. 뭔가 악몽을 꾼 것 같은데……. 어라, 의식은 있는데 몸이 안 움직여!",
];
static KO_HJST: Table = Table::from_dice("사실은 무서운 현대 일본 장면표", 2, 6, KO_HJST_ITEMS);

static KO_MTST_ITEMS: &[&str] = &[
    "이끼가 낀 거대한 바위가 늘어선, 강 위의 섬. 뭘 모시고 있는지도 알 수 없는 제단이 있고, 형언하기 힘든 분위기가 감돈다.",
    "무허가 술집. 간판도 없는 지하의 가게에서는 거리의 사내들과 작부들이 시끄럽게 떠들고 있다.",
    "유적 안. 누가 지었는지도 모를 비(非)유클리스 기하학적인 건물은 안을 걷는 자의 이성을 서서히 좀먹는다.",
    "대학도서관. 40만 권이 넘는 장서 중에는 모독적인 마도서도 포함되어 있다고 한다.",
    "세찬 바람을 타고 어딘가에서 바다 내음이 풍겨온다. 바다는 멀 텐데…….",
    "수많은 사람들로 붐비는 거리. 여기라면 누군가가 섞여들더라도 들킬 걱정은 없다.",
    "깊은 어둠 속. 그 너머에 형언하기 어려운 무언가가 있는 것만 같다.",
    "역사 깊은 신문사. 쉬지 않고 발행해온 백 년분에 달하는 신문이 보관되어 있다.",
    "오래된 묘지. 구부러진 나무 사이로 낡아서 묘비명도 읽을 수 없는 묘비가 줄지어 섰다. 몇몇 묘비는 왠지 기울어져 있다.",
    "강가에 있는 공장 터. 한참 전에 버려졌는지 건물은 무너져가고 있다. 아무래도 부랑자들이 거주하고 있는 모양이다.",
    "조용한 실내. 왠지 불온한 기척이 느껴지는데……. 저건 뭐지? 창문에, 창문에!",
];
static KO_MTST: Table = Table::from_dice("광란의 20년대 장면표", 2, 6, KO_MTST_ITEMS);

static KO_DVST_ITEMS: &[&str] = &[
    "영매사를 중심으로 원탁을 둘러싼 사람들이 강령회를 한다. 어두운 방 안에 기괴한 엑토플라즘이 떠돌기 시작한다.",
    "노동자들이 모인 펍. 불그스름한 얼굴의 남자들은 여직원의 가져다준 에일이나 진을 마시고 있다.",
    "피 냄새가 떠도는 장소. 여기에서 무슨 일이 있었던 걸까…….",
    "창관이 늘어선 빈민가. 건물 앞에서는 창부들이 손님을 기다린다.",
    "사람들로 북적거리는 소란스러운 거리. 다양한 소문이 이리저리 퍼지고 있다. 동양인을 비롯한 외국인의 모습도 보인다.",
    "안개가 짙게 깔린 거리. 가스등의 빛만이 돌바닥을 비춘다.",
    "조용한 방 안. 여기라면 뭘 해도 쓸데없는 추궁은 받지 않겠지.",
    "기적이 울리는 부두. 저 배는 외국으로 떠나는 걸까?",
    "서적이 잔뜩 있는 장소. 조사하기에는 좋긴 하지만…….",
    "귀족이나 자산계급의 사람들이 모이는 파티. 기품 있는 미소 아래로 어떤 꿍꿍이를 꾸미고 있을까?",
    "고요한 호숫가. 풀숲에서는 야생 토끼가 뛰어다니고 있다.",
];
static KO_DVST: Table = Table::from_dice("빅토리아의 어둠 장면표", 2, 6, KO_DVST_ITEMS);

static KO_DT_ITEMS: &[(i64, TableItem)] = &[
    (11, TableItem::Text("창백한")),
    (12, TableItem::Text("피를 흘리는")),
    (13, TableItem::Text("비늘이 있는")),
    (14, TableItem::Text("모독적인")),
    (15, TableItem::Text("원통형의")),
    (16, TableItem::Text("이상하게 증식하는")),
    (22, TableItem::Text("불규칙한")),
    (23, TableItem::Text("까악 까악 우는")),
    (24, TableItem::Text("무수한")),
    (25, TableItem::Text("털이 많은")),
    (26, TableItem::Text("색채가 없는")),
    (33, TableItem::Text("신축하는")),
    (34, TableItem::Text("외설적인")),
    (35, TableItem::Text("부풀어 오른")),
    (36, TableItem::Text("거대한")),
    (44, TableItem::Text("점액투성이의")),
    (45, TableItem::Text("끊임없이 변화하는")),
    (46, TableItem::Text("벌레 투성이의")),
    (55, TableItem::Text("키틴질의")),
    (56, TableItem::Text("(「본체표」를 사용) 같은")),
    (66, TableItem::Text("무지갯빛으로 빛나는")),
];
static KO_DT: D66Table = D66Table::new("형용표", D66SortType::Asc, KO_DT_ITEMS);

static KO_BT_ITEMS: &[(i64, TableItem)] = &[
    (11, TableItem::Text("인간")),
    (12, TableItem::Text("개")),
    (13, TableItem::Text("쥐")),
    (14, TableItem::Text("유령")),
    (15, TableItem::Text("민달팽이")),
    (16, TableItem::Text("벌레")),
    (22, TableItem::Text("얼굴")),
    (23, TableItem::Text("고양이")),
    (24, TableItem::Text("지렁이")),
    (25, TableItem::Text("소")),
    (26, TableItem::Text("새")),
    (33, TableItem::Text("반어인(半魚人)")),
    (34, TableItem::Text("인조인간")),
    (35, TableItem::Text("뱀")),
    (36, TableItem::Text("노인")),
    (44, TableItem::Text("아메바")),
    (45, TableItem::Text("여성")),
    (46, TableItem::Text("기계")),
    (55, TableItem::Text("문어")),
    (56, TableItem::Text("「부위표」를 사용")),
    (66, TableItem::Text("소인")),
];
static KO_BT: D66Table = D66Table::new("본체표", D66SortType::Asc, KO_BT_ITEMS);

static KO_PT_ITEMS: &[(i64, TableItem)] = &[
    (11, TableItem::Text("몸통")),
    (12, TableItem::Text("다리")),
    (13, TableItem::Text("팔")),
    (14, TableItem::Text("머리카락/갈기")),
    (15, TableItem::Text("입")),
    (16, TableItem::Text("유방")),
    (22, TableItem::Text("얼굴")),
    (23, TableItem::Text("피부")),
    (24, TableItem::Text("눈동자")),
    (25, TableItem::Text("꼬리")),
    (26, TableItem::Text("촉수")),
    (33, TableItem::Text("코")),
    (34, TableItem::Text("그림자")),
    (35, TableItem::Text("이빨")),
    (36, TableItem::Text("뼈")),
    (44, TableItem::Text("보석")),
    (45, TableItem::Text("날개")),
    (46, TableItem::Text("뇌")),
    (55, TableItem::Text("혀")),
    (56, TableItem::Text("가지나 나뭇잎")),
    (66, TableItem::Text("내장")),
];
static KO_PT: D66Table = D66Table::new("부위표", D66SortType::Asc, KO_PT_ITEMS);

static KO_FT_ITEMS: &[&str] = &[
    "공감（플러스）／불신（마이너스）",
    "우정（플러스）／분노（마이너스）",
    "애정（플러스）／질투（마이너스）",
    "충성（플러스）／모멸（마이너스）",
    "동경（플러스）／열등감（마이너스）",
    "광신（플러스）／살의（마이너스）",
];
static KO_FT: Table = Table::from_dice("감정표", 1, 6, KO_FT_ITEMS);

static KO_JT_ITEMS: &[(i64, TableItem)] = &[
    (11, TableItem::Text("고고학자　≪풍경≫≪고고학≫")),
    (12, TableItem::Text("갱　≪고문≫≪노여움≫")),
    (13, TableItem::Text("탐정　≪제육감≫≪수학≫")),
    (14, TableItem::Text("경찰　≪사격≫≪추적≫")),
    (15, TableItem::Text("호사가　≪예술≫≪인류학≫")),
    (16, TableItem::Text("의사　≪절단≫≪의학≫")),
    (22, TableItem::Text("교수　지식 분야에서 아무거나 둘")),
    (23, TableItem::Text("성직자　≪부끄러움≫≪친애≫")),
    (24, TableItem::Text("심리학자　정서 분야에서 아무거나 둘")),
    (
        25,
        TableItem::Text("학생　정서 분야와 지식 분야에서 아무거나 하나씩"),
    ),
    (26, TableItem::Text("기자　≪놀람≫≪미디어≫")),
    (33, TableItem::Text("기술자　≪전자기기≫≪기계≫")),
    (34, TableItem::Text("도둑　≪그늘≫≪함정≫")),
    (35, TableItem::Text("예능인　≪기쁨≫≪예술≫")),
    (36, TableItem::Text("작가　≪걱정≫≪교양≫")),
    (44, TableItem::Text("모험가　≪구타≫≪탈것≫")),
    (45, TableItem::Text("사서　≪정리≫≪미디어≫")),
    (46, TableItem::Text("요리사　≪소각≫≪맛≫")),
    (55, TableItem::Text("사업가　≪인내≫≪효율≫")),
    (56, TableItem::Text("호스티스　≪웃음≫≪관능≫")),
    (66, TableItem::Text("경호원　폭력 분야에서 아무거나 둘")),
];
static KO_JT: D66Table = D66Table::new("직업표", D66SortType::Asc, KO_JT_ITEMS);

static KO_CHT_ITEMS: &[&str] = &[
    "지정특기：죽음\n당신은 이야기를 나누다가 문득 상대의 어깨 너머를 본다.\n아무런 전조도 없이, 멀리 떨어진 건물의 옥상에서 여자가 뛰어내렸다.\n그녀는 소리를 지를 틈도 없이 빨려 들어가듯 지면으로 떨어진다. 거리가 있음에도 불구하고, 여자와 눈이 마주치고 말았다.\nーー그 이후, 여자의 얼굴이 뇌리에 달라붙어 떨어지지 않는다…….",
    "지정특기：구타\n지면에 쓰러진 상대를 보며, 당신은 식은땀이 등을 타고 흐르는 것을 느꼈다.\n상대는 부자연스럽게 몸을 뒤튼 채로 쓰러져서 꿈쩍도 하지 않는다. 조금씩 피 웅덩이가 넓어진다…….\nーー죽어버렸다.\n당황해서 눈을 깜빡인 순간, 상대의 몸은 사라졌다. 피 웅덩이도 없어졌다.\n당신은 멍하니 그 자리에 서 있다. 환각이었나……?",
    "지정특기：전자기기\n당신이 전화로 상대와 이야기하는데 갑자기 상대가 입을 다문다.\n「……저게 뭐야?」\n상대가 혼잣말처럼 중얼거리더니, 그때부터 당황한 듯한 목소리를 낸다.\n「오지 마……. 오지 마! 우왁! 우와아아아! 살려줘! 살려줘!!」\n그것을 끝으로 전화는 뚝 끊긴다. 다시 걸어보니 통화 중이다. 계속.",
    "지정특기：소리\n당신이 전화로 상대와 이야기하는데 중얼거리는 소리가 들린다.\n혼선일까? 이상하다 싶어서 듣고 있는 사이에 머리가 멍해진다.\n중얼중얼, 중얼중얼, 중얼중얼, 중얼중얼…… 목소리가 들린다.\n정신을 차리고 보니 전화를 든 채로 멍하니 서 있었다.\n통화는 끊겼다. 무슨 이야기를 했는지 기억이 안 난다.\n단지 엄청나게 무서운 이야기를 들은 것 같은 기분이 든다.\n애초에 당신은 누구와 이야기하고 있었던 걸까?",
    "지정특기：고문\n이야기를 나누다가 피 맛을 느꼈다.\n동시에 입안에서 뭔가가 굴러다니는 위화감을 느낀다.\n상대가 새파랗게 질리면서 당신의 얼굴을 가리킨다.\n무슨 일이냐고 물으려고 입을 열자 뭔가가 지면에 툭 떨어졌다.\n내려다보니 피 웅덩이 속에 당신의 하얀 이빨이 하나 떨어져 있다.",
    "지정특기：인류학\n당신은 이야기를 나누다가 시야에 위화감을 느끼고 눈을 깜빡인다.\n상대의 얼굴이, 이상해졌다.\n잡아 늘여서 휘저은 것처럼 그로테스크하게 찌그러졌다.\n깜짝 놀라서 다시 바라봤지만, 여전히 일그러져있다.\n상대는 전혀 눈치채지 못한 것 같다. 눈을 힘껏 감았다가 다시 보니 겨우 원래대로 돌아왔다.\n당신의 마음에 한 가지 의심이 생긴다. 눈앞의 상대는 정말로 인간일까?",
];
static KO_CHT: Table = Table::from_dice("회화 중에 생겨나는 공포표", 1, 6, KO_CHT_ITEMS);

static KO_VHT_ITEMS: &[&str] = &[
    "지정특기：탈것\n끼이이ーー익!! 세찬 브레이크 소리, 그리고 둔탁한 소리가 들렸다.\n깜짝 놀라 돌아보니 멈춘 차와 그 앞에 쓰러진 사람이 보였다. 교통사고다!\n서둘러 달려가 피해자의 얼굴을 본 순간, 당신은 얼어붙었다.\n쓰러져 있는 것은, 당신이었다.\nーー어!? 놀라서 눈을 깜빡이자 길 위의 당신도, 차도 이미 없었다.",
    "지정특기：풍경\n길가에 있는 집 지붕 위에 누군가가 서 있다.\n저런 곳에서 뭘……?\n그 사람은 춤을 추고 있는 것처럼 보였다. 손발을 휘두르고 머리를 세차게 움직이며 춤추고 있다. 정상적인 모습이 아니다. 미친 듯이 춤을 추고 있다.\n보고 있는 사이에 엄청나게 불길한 기분이 들었다. 보고 싶지 않은데 왠지 눈을 돌릴 수 없다. 불길한 예감이 점점 강해진다…….",
    "지정특기：종말\n우우우우우우우우우………….\n거리에 사이렌이 울린다.\n어디에서 울리고 있는 걸까? 언제까지 울리는 걸까?\n이렇게 소리가 큰데 어째서 아무 소란도 일어나지 않는 걸까.\n이상하게 여기며 걷고 있는데 길 건너편에서 걸어오는 누군가가 보인다. 다치기라도 했는지 비틀거리며, 지금이라도 쓰러질 것처럼 흐느적 흐느적 부자연스러운 걸음걸이로 걸어온다.\n저건 도대체……?",
    "지정특기：협박\n걷고 있는데 갑자기 조용해졌다. 주위를 둘러보니 사람도 차도, 아무것도 없다.\n아무도 없는 거리가 끝없이 펼쳐져 있다. 아까까지 사람이 많이 있었는데……!?\n「야! 뭐 하는 거야!」\n갑자기 누가 화를 내서 움찔했다.\n돌아보니 작업복을 입은 남성이 이쪽으로 달려오고 있었다.\n「바보 자식! 이런 곳에 오면ーー」 말이 끝나기도 전에, 갑자기 소리가 돌아온다.\n사람과 차가 오가는 원래의 거리다. 지금 그건 뭐였지……?",
    "지정특기：혼돈\n전봇대 아래에 여성이 웅크리고 앉아 있다.\n배를 부여잡고 괴로운 듯이 고개를 숙이고 있다.\n「괜찮으세요?」\n다가가서 말을 건 당신에게 여성은 끄덕였다.\n「네ーー 감사합니다.」\n그렇게 말하며 고개를 든 여성의 얼굴에는 아무것도 없었다. 껍질을 깐 반들반들한 달걀 같은 피부가 있을 뿐이었다.\n우왁!? 몸을 젖힌 순간 의식이 멀어지더니, 정신을 차리자 당신은 전봇대 아래에 웅크리고 앉아 있었다.",
    "지정특기：웃음\n역에 도착하니 묘하게 혼잡했다. 인명사고로 전철이 멈춘 모양이다. 재수 없구먼. 그렇게 생각하던 참에 개표구 근처의 인파 속에서 전통복 차림의 여성이 빠른 걸음으로 당신 쪽으로 다가왔다.\n여성은 만면에 웃음을 띄고 있었다. 혼잣말을 하는 건지 입이 움직이고 있다.\n스쳐 지나가면서 여성의 목소리가 귀에 들렸다.\n「해냈다, 해냈어. 해냈다, 해냈어. 해냈다, 해냈어. 꼴 좋다.」\n어!? 놀라서 돌아보는 당신을 남겨두고, 여성은 인파 속으로 사라졌다.",
];
static KO_VHT: Table = Table::from_dice("거리에서 마주치는 공포표", 1, 6, KO_VHT_ITEMS);

static KO_IHT_ITEMS: &[&str] = &[
    "지정특기：놀람\n다다다다다! 갑작스러운 소리에 당신은 흠칫 놀라서 올려다본다. 천장 위에서 뭔가가 돌아다니고 있는 모양이다. 동물이라도 기어들어 왔나? 그런 것치고는 소리가 크다.\nーー마치 어린아이가 제멋대로 달려 다니는 듯한.\n소리는 한순간 멈췄다가, 곧 다시 들린다.\n쿵! 쿵! 쿵! 쿵!\n뜀박질 소리가 들리는 곳은 정확하게 당신의 머리 위다…….",
    "지정특기：우주\n창문에서 빛이 들어온다.\n하늘로 눈을 돌리니 하얗게 빛나는 거대한 비행물체가 떠 있었다. 매료되어 바라보는 사이에 새나 비행기라고는 생각할 수 없는, 격렬하고 불규칙한 움직임으로 날아다니기 시작한다. 뭐지, 저건? 이상하게 여기고 있는데 뒤에서 누군가가 속삭였다.\n「저건 ……야.」\n퍼뜩 정신을 차리니 어느새 전혀 다른 장소에 있었다.\n손바닥에서 뭔가가 박혀있는 듯한 단단한 감촉이 느껴진다…….",
    "지정특기：냄새\n기묘한 물건이 배송됐다.\n테이프로 둘둘 말린 커다란 골판지 상자다. 보낸 사람의 이름이 적힌 종이가 붙어있지만, 글씨가 번져서 읽을 수 없다.\n상자의 내용물은 흙이었다. 도자기의 파편이나 돌멩이가 섞인, 이상한 냄새가 나는 흙이 들어있었다.\n정체를 알 수 없어서 내용물은 버렸는데, 그 이후 왠지 재수가 없는 듯한 기분이 든다…….",
    "지정특기：인내\n벽 너머에서 누군가가 이야기를 하는 소리가 들린다.\n「……니까. 이 녀석은……해야지.」 「그래……. ……하다고. ……해야지.」\n속닥속닥, 속닥속닥……. 음침한 분위기의 대화가 계속된다. 무슨 이야기를 하는 건지 내용은 잘 모르겠지만, 왠지 자신에 대해 이야기하는 것 같아서 기분이 나쁘다.\n신경이 쓰여서 벽에 귀를 가져다 댔을 때, 또렷한 목소리가 벽 너머에서 이렇게 말했다.\n「……저기, 잘 듣고 있어?」",
    "지정특기：촉감\n똑, 똑.\n목덜미에 떨어진 미지근한 물방울의 감촉에 당신은 이맛살을 찌푸렸다.\n어느새 책상 위에 빨간 물방울이 떨어지고 있다. 비린내가 코를 찌른다.\n똑, 똑, 똑. 물방울은 기세를 더하여 연달아 떨어져서 책상 위에 퍼진다.\n천천히 올려다보니, 천장에는 검붉은 얼룩이 크게 번져 있었다.\n똑. 똑똑. 똑.\nーー투두둑!\n점점 커지는 물소리에 당신은 꼼짝도 못 한다.\n천장 위에서, 도대체, 뭐가……?",
    "지정특기：지저\n잘 아는 장소에서 낯선 문을 발견했다.\n열어보니 아래로 내려가는 긴 계단이 어둠 속으로 이어져 있다.\n수상하게 여기며 내려가 보니…… 그곳은 지하실이었다.\n이런 장소가 있었다니.\n조명을 한 손에 들고 들어가니 뭔가가 다가오는 기척이 느껴진다.\n어둠 속에서, 누군가가, 당신의 이름을 불렀다.",
];
static KO_IHT: Table = Table::from_dice("갑자기 찾아오는 공포표", 1, 6, KO_IHT_ITEMS);

static KO_RHT_ITEMS: &[&str] = &[
    "지정특기：암흑\n무겁고 단단해 보이는 문을 연다.\n방 안은 어두웠다. 조명을 켜보니 다른 방으로 이어지는 길을 몇 개 찾았다.\n특별히 눈에 띄는 것도 없었다. 일단 입구로 돌아가려고 들어온 문 쪽을 돌아본다.\n거기에는 벽밖에 없었다. 그 중후한 문이 없어졌다.\n이런 말도 안 되는 일이…….\n하지만 아무리 찾아봐도 어느 벽에도 문 같은 것은 보이지 않는다.\n어쩔 수 없이 통로를 걷기로 했는데, 점점 불길한 기분이 치밀어 오른다.\n……그 문은, 열어서는 안 되는 것이 아니었을까?",
    "지정특기：정리\n당신은 폐허 속에서 부ー웅…… 하는 낮은 소리를 들었다.\n냉장고다. 어딘가에서 전기가 들어오고 있는지 폐허 구석에 하얀 냉장고가 쓸쓸히 놓여있다.\n딸깍, 문을 당겨서 열어본 당신은, 안에 있던 뭔가와, 눈이 마주쳤다.\n……정신을 차리고 보니 당신은 어둡고 차가운 곳에 몸을 웅크리고 있다.\n부ー웅…… 하는 소리가 들린다. 여기는 시원하고, 좁고ーー 매우 기분이 좋다.",
    "지정특기：추적\n폐허의 장지문을 열었을 때 당신은 강한 위화감을 느꼈다.\n두껍게 먼지가 쌓인 그 방에서는 방금까지 사람이 있었던 기척을 느낄 수 있었다.\n밥상 위에는 내용물이 남아있는 찻잔. 아까까지 누군가가 앉아 있었던 것처럼 움푹 팬 방석.\n왜 이 거실은 이렇게 생활감이 느껴지지……?",
    "지정특기：친애\n폐허를 걷고 있는데 갑자기 당신의 핸드폰에 전화가 걸렸다.\n조용한 폐허에 울리는 착신음에 깜짝 놀라서 받아보니, 전화 너머에서 당신의 할머니가 갑자기 호통을 쳤다.\n「너, 뭘 하는 거니! 그런 곳에 가면 안 되잖니!」\n어? 왜 할머니가……?\n「빨리 거기에서 나와! 그러다가 큰일 나!」\n뭐가 뭔지 몰라서 우물거리고 있는 사이에 전화는 끊어졌다.\n디스플레이에는 「통화권 이탈」이라고 표시되어 있다.",
    "지정특기：함정\n폐허를 걷고 있는데 갑자기 발에서 격통이 느껴졌다.\n비명을 삼키며 아래를 보니, 짐승을 잡을 때 쓰는 덫이 당신의 발목을 꽉 물고 있다.\n왜 이런 곳에 이런 함정이……?\n겨우 덫을 해제하고 다시 주위를 봤을 때, 당신은 깜짝 놀랐다.\n함정은 하나가 아니었다. 건물의 잔해에 가려지도록 설치한 수없이 많은 덫이 그곳에 있었다.",
    "지정특기：약품\n갑자기 시너 냄새가 코를 찔렀다. 폐허의 벽에 빨간 페인트로 끈적끈적한 문자가 적혀 있다.\n무슨 내용인지는 잘 모르겠지만 한결같은 악의와 증오를 그대로 칠해낸 듯한 터치에 오싹한 기분이 든다.\n그러다가 어떤 사실을 깨닫고, 당신은 소름이 돋았다.\n페인트가 아직 새것이다. ……방금 칠한 것처럼 새것이다.",
];
static KO_RHT: Table = Table::from_dice("폐허에서 마주치는 공포표", 1, 6, KO_RHT_ITEMS);

static KO_MHT_ITEMS: &[&str] = &[
    "지정특기：고통\n부ーーーーー웅.\n귓가에 날카로운 날갯소리가 들린다.\n본 적도 없는 새빨간 날벌레 떼가 날아왔다.\n부ーーーーー웅.\n날벌레를 쫓으려고 팔을 휘두른다.\n아얏!\n팔에서 무언가에 찔린 통증이 느껴진다.\n부ーーーーー웅.\n벌레들은 어딘가로 떠났다.\n순식간에 팔의 표면에서 기분 나쁜 물집이 우둘투둘 부어올랐다.",
    "지정특기：꿈\n나무 사위로 뭔가 커다란 것이 움직이는 모습이 보인다.\n살이 썩는 듯한 악취가 코를 찌른다.\n얼룩무늬의 저것은…… 모피? 아니면 갈기갈기 찢긴 옷 조각?\n그 녀석이 당신을 봤다.\n나뭇잎 사이로 이쪽을 보는 눈은, 완전히 사람과 같은ーー\n눈이 마주친 순간부터 기억이 없다. 정신을 차리니 인간의 머리카락과 흡사한 검은 털이 당신의 전신에 붙어 있었다.",
    "지정특기：원한\n숲속에서 맞닥뜨린 거목의 줄기에는 수많은 짚인형이 못 박혀 있었다.\n우와아…… 하고 놀라며 올려다보다가 우울한 사실을 알아차리고 말았다. 새로 못박힌 짚인형 중 하나에 이름표가 붙어 있었다.\nーー굉장히, 낯익은 이름이었다.",
    "지정특기：심해\n……어~이.\n……어~~~이.\n멀리서 누군가가 부르는 기분이 들어 파도 사이를 바라본다.\n반들반들한 검은 그림자가 떠오르고 가라앉기를 반복하며 당신을 부르고 있다.\n하나, 둘, 셋, 넷…… 일곱 명.\n일곱 명의 검은 그림자가 파도 속에서 당신에게 손을 흔들고 있다.\nーー왠지, 머리가 멍해졌다.",
    "지정특기：그늘\n덤불 속에 폐차가 파묻혀 있다.\n별다른 특징이 없는 하얀 밴이다.\n창문은 새까만 검댕으로 더럽혀져서 아무것도 보이지 않는다.\n차체는 녹이 슬었고, 염료도 벗겨졌다. 누가 봐도 버려진 지 오래된 폐차였다.\nーー그런데, 폐차 안으로부터 찌르는 듯한 시선이 느껴진다.\n록을 해제하는 소리가 들리고, 천천히 뒷좌석의 문이 열리기 시작한다…….",
    "지정특기：소각\n타닥…… 타닥…….\n불이 타오르는 소리가 난다.\n빈터에서 모닥불이 타오르고 있었다. 곁에는 아무도 없다.\n훈훈한 기분에 멈춰 서서 나뭇가지로 모닥불을 휘젓는데, 불 속에서 발랄한 웃음소리가 들렸다.\n흠칫 놀란 당신의 발치를 고양이만 한 크기의 무언가가 스치고 지나간다.\n다시 모닥불을 바라보니, 그곳에는 그저 그을린 뼈들이 남아있을 뿐이었다.",
];
static KO_MHT: Table = Table::from_dice("야외에서 마주치는 공포표", 1, 6, KO_MHT_ITEMS);

static KO_LHT_ITEMS: &[&str] = &[
    "지정특기：맛\n목이 말랐다.\n조사를 시작한 지 제법 시간이 지났다. 아무래도 너무 몰두한 모양이다.\n다시 화면을 보며 페트병의 물을 입에 머금는다. 그러자 입안에서 위화감이 퍼졌다.\n견디지 못하고 물을 뱉어냈더니 새까만 액체가 책상 위의 자료를 더럽혔다.\n입안에서 시궁창 같은 악취가 떠나지 않는다.\n다시 페트병의 물을 확인해보니 그냥 투명한 물인데……?",
    "지정특기：카메라\n자료 중에서 누렇게 변한 봉투에 들어있는 사진 다발이 나왔다.\n피사체는…… 당신이다.\n모르는 사이에 찍힌 당신의 사진이 수집 장이나 한 묶음이 되어 들어있다.\n색이 바랜 사진을 아무렇게나 동여맨 고무줄이 낡아서 끈적거렸다.\nーー누가 이런 사진을? 무슨 목적으로?",
    "지정특기：미디어\nTV의 뉴스를 보고 있는데 그때까지 쉬지 않고 떠들던 아나운서가 갑자기 입을 다물었다.\n의아해하고 있는데 아나운서가 기묘한 말을 하기 시작했다.\n「죽을 수도 있다고 했습니다. 또, 높은 확률로 재앙이 일어납니다. 오늘부터 내일까지 철저하게 경계하세요. 경계하세요. 경계하세요.」\n아나운서는 화면 너머에서 당신의 눈을 지그시 보고 있다.\n당신이 당황하자 저절로 TV의 전원이 꺼졌다.",
    "지정특기：민속학\n자료를 뒤지다가 어느 외진 마을에서 전해지는 끔찍한 풍습을 찾아냈다.\n폭력, 의식, 산 제물, 도저히 정상적인 인간이 할 짓이라고는 생각되지 않는 소행에 몸이 떨렸다.\n그 풍습에서 왠지 기시감이 느껴졌다.\n이런 걸 어디에서 읽었지? 생각하는 사이에 어떤 기억이 되살아났다.\n당신이 어렸을 적의 기억이었다.\n아니……. 그런, 말도 안 돼. 우리 고향이, 이런 마을일 리가 없어…….",
    "지정특기：마술\n자료 중에 기묘한 고서가 있었다. 가죽 표지가 호화로운 책으로, 묘한 냄새가 난다. 문장은 지리멸렬해서 제정신으로 썼다고는 생각되지 않는다.\n하지만 당장에라도 찢어질 것 같은 페이지를 한 장, 한 장 넘기고 있으려니 작가가 무엇을 말하는지 이해가 갔다.\n점점, 점점, 이해가 된다.\n아아, 이제, 알겠다.\n전부 알겠다.\n이제 괜찮다.\n이 책은 분명 당신이 읽을 때를 위해 쓰인 것이다.",
    "지정특기：역사\n표지에 없는 리포트를 발견했다.\n훌훌 넘겨보니 바로 당신이 지금 조사하고 있는 건에 대한 조사 보고서였다.\n안타깝게도 이곳저곳이 검게 칠해져서 중요한 부분을 알 수 없다. 추측하건대 아무래도 군이 조사했던 모양이다.\n군대? 왜 군대가 이 일을 조사한 거지……?",
];
static KO_LHT: Table = Table::from_dice("정보 속에 숨어있는 공포표", 1, 6, KO_LHT_ITEMS);

static KO_ECT_ITEMS: &[&str] = &[
    "실패작×3　기본ｐ250",
    "노려보는 자×1　데드 루프ｐ184　개×1　기본ｐ247",
    "신봉자×2　기본ｐ247",
    "얼굴을 가린 여자×1　데드 루프ｐ186",
    "유령 자동차×1　데드 루프ｐ187",
    "원령×1　기본ｐ249",
];
static KO_ECT: Table = Table::from_dice("조우표・도시", 1, 6, KO_ECT_ITEMS);

static KO_EMT_ITEMS: &[&str] = &[
    "달걀귀신×3　데드 루프ｐ184",
    "독충 떼×2　데드 루프ｐ185",
    "곰×1　데드 루프ｐ185",
    "거대곤충×1　데드 루프ｐ186",
    "늑대인간×1　기본ｐ272",
    "구불구불×1　데드 루프ｐ187",
];
static KO_EMT: Table = Table::from_dice("조우표・산림", 1, 6, KO_EMT_ITEMS);

static KO_EAT_ITEMS: &[&str] = &[
    "인혼×3　데드 루프ｐ184",
    "심해의 주민×2　기본ｐ267",
    "별을 건너는 자×1　기본ｐ267",
    "우주인×1　기본ｐ262",
    "마녀×1　기본ｐ249",
    "기어 다니는 것×1　기본ｐ267",
];
static KO_EAT: Table = Table::from_dice("조우표・해변", 1, 6, KO_EAT_ITEMS);

static KO_OHT_ITEMS: &[&str] = &[
    "지정특기：죽음\n창문을 쳐다본 순간, 창밖에서 낙하하는 사람과 눈이 마주쳤다! 서둘러서 창가로 뛰어갔지만, 아래에는 아무것도 없다. 환각이었나……?",
    "지정특기：기계\n갑자기 복사기가 소리를 내며 종이를 뱉어내기 시작한다. 바닥에 흩어진 복사용지에는 일그러진 사람의 얼굴 같은 것이 인쇄되어 있다. 기분 나빠…….",
    "지정특기：그늘\n창백한 어린애가 책상 밑에서 당신을 올려다보고 있다. 우악! 하고 소리치며 물러났더니 아이의 모습이 사라졌다.",
    "지정특기：촉감\n일을 하고 있는데 뒤에서 길고 검은 머리카락이 드리워진다. 여자의 검은 장발이다. ……뒤에서 엿보고 있는 건 도대체 누구지?",
    "지정특기：걱정\n어두운 얼굴의 남자가 시야 구석에서 지나가는 것이 보였다. 돌아봐도 아무도 없다. 누구야? 난 그런 녀석 몰라.",
    "지정특기：암흑\n팍! 갑작스러운 정전으로 플로어가 깜깜해졌다. 놀라서 얼굴을 드니 어둠 속에 서 있는 무수한 이들이 가만히 당신을 바라보고 있다…….",
];
static KO_OHT: Table = Table::from_dice("야근 호러 스케이프", 1, 6, KO_OHT_ITEMS);

static KO_OPT_ITEMS: &[&str] = &[
    "「어디까지 되었나요?」 의뢰인의 확인 전화. 지금 하고 있어! 전화할 시간도 아깝다고! 스트레스로 위가 아프다. 【생명력】-1.",
    "「기준이 바뀌어서……」 의뢰인이 제작 기준 변경을 통보. 지금!? 죽을래!? 모처럼 해둔 작업이 무의미해졌다. PP-1.",
    "「최근 전화를 안 받던데…… 괜찮아? 연인이나 가족 등 소중한 사람의 전화. 바빠 죽겠는데 웬 전화냐고 생각하면서도 기분이 조금 나아졌다. 【이성치】 1 회복.",
    "「특상 초밥 5인분 배달해주세요!」 잘못 걸린 전화였다. 사람 놀라게…….",
    "「이봐! 지난번 그 일 말인데, 어떻게 된 거야!」 다른 건의 클레임 전화다! 지친 정신에 대미지를 받아 【이성치】-1.",
    "「……하면 좋을 텐데.」 전화 너머에서 지옥에서 들려오는 듯한 목소리가 속삭였다. 오싹한 기분에 반사적으로 전화를 끊었다. 착신 이력은 남아 있지 않다. 지금 그거…… 뭐야!? 《전자기기》로 공포판정.",
];
static KO_OPT: Table = Table::from_dice("야근 시 전화표", 1, 6, KO_OPT_ITEMS);

static KO_OWT_ITEMS: &[&str] = &[
    "지직……. 갑자기 형광등이 깜빡인다. 전기계통에 이상이 생긴 걸까? 정전은 곤란한데…….",
    "똑. 똑. 어딘가에서 물이 떨어지는 소리가 들린다. 비가 새나? 아니면 수도꼭지를 제대로 안 잠갔나?",
    "쿠르르…… 촤아아아. 화장실에서 물을 내리는 소리가 들린다. 누가 화장실에 있나? 아니면 다른 층?",
    "사이렌 소리가 점점 커지더니 창밖에서 빨간 불빛이 번쩍거린다. 근처에서 무슨 일이라도 생긴 것 같은데…….",
    "등 뒤에서 누군가의 말소리가 들린 것 같다. 즉시 돌아봤지만…… 환청이었나?",
    "유리창 너머로 보이는 야경을 애달픈 기분으로 바라본다. 빨리 집에 가고 싶어…….",
    "갑자기 핸드폰이 울린다. 매너 모드로 해뒀을 텐데……. 도대체 누구야?",
    "갑작스러운 기계음에 깜짝 놀라 돌아보니 팩스가 종이를 뱉어내고 있다. 이런 시간에 뭐지?",
    "입맛을 당기는 냄새가 풍겨서 갑자기 배가 고파졌다. 이 냄새는 어디에서 나는 거야?",
    "기분전환으로 시작한 웹 서핑에 몰두하다가 정신을 차리고 보니 몇 분이나 경과……. 이러면 안 되지.",
    "꾸벅꾸벅 졸다가 눈을 번뜩 떴다. 황급히 시계를 봤더니…… 으악! 벌써 시간이 이렇게!?",
];
static KO_OWT: Table = Table::from_dice("야근 장면표", 2, 6, KO_OWT_ITEMS);

static KO_CNT1_ITEMS: &[&str] = &["플라잉", "트러블", "블러드", "프리티", "크림슨", "봄버"];
static KO_CNT1: Table = Table::from_dice("회사명 결정표1", 1, 6, KO_CNT1_ITEMS);

static KO_CNT2_ITEMS: &[&str] = &["위치즈", "인텔리전스", "캣츠", "버드", "호러", "인세인"];
static KO_CNT2: Table = Table::from_dice("회사명 결정표2", 1, 6, KO_CNT2_ITEMS);

static KO_CNT3_ITEMS: &[&str] = &[
    "(株) 주식회사",
    "(株) 주식회사",
    "(株) 주식회사",
    "(有) 유한회사",
    "(有) 유한회사",
    "(有) 유한회사",
];
static KO_CNT3: Table = Table::from_dice("회사명 결정표3", 1, 6, KO_CNT3_ITEMS);

static KO_RET_ITEMS: &[&str] = &[
    "「잠깐 이쪽으로 와주시겠습니까?」 갑자기 체포되어 구속된다. 「반응표」를 사용한 캐릭터는 이 장면이 끝나고 두 장면 동안 자신이 장면 플레이어가 아닌 장면에 등장할 수 없다(마스터 장면에는 등장 가능).",
    "「내가 협력할 수 있는 건 여기까지다.」 협력을 요청받은 인물은 두려움에 떨며 손에 든 꾸러미를 당신에게 떠넘긴다. 「반응표」를 사용한 캐릭터는 아무거나 아이템을 1개 획득한다.",
    "「그 건은 조사 중입니다. 정보 제공에 감사드립니다.」 협력을 요청받은 인물은 싱글벙글 웃으면서 그렇게 대답한다. 무슨 말을 해도 같은 대답밖에 돌아오지 않는다. 「반응표」를 사용한 캐릭터는 【이성치】를 1점 감소한다.",
    "「혹시 너, 그 사건의 관계자냐……?」 아무래도 협력을 요청받은 인물도 마침 같은 사건을 조사하던 모양이다. 이것저것 정보를 제공해줄 것 같다. 「반응표」를 사용한 캐릭터는 이후 조사판정을 할 때 +1의 수정을 적용한다.",
    "「꿈이라도 꾼 거 아니에요?」 아무리 열심히 호소해도 믿어주지 않는다. ……혹시 이상한 건 나인가? 정서 특기분야에서 무작위로 특기 하나를 선택하여 공포판정을 한다.",
    "「아, 됐고, 우리도 바쁘다고.」 이것저것 이야기해봐도 상대해주지 않는다. 문전박대를 당한다.",
    "「잠시 몸수색을 해봐도 될까요?」 수상한 인물로 여겨진 듯하다. 「반응표」를 사용한 캐릭터가 아이템이나 불법적으로 보이는 프라이즈를 가지고 있다면, 이 장면이 끝나고 두 장면 동안 자신이 장면 플레이어가 아닌 장면에 등장할 수 없다(마스터 장면에는 등장할 수 있다).",
    "「마음에 걸리는 이야기로군요. 저도 조사해보죠.」 정성껏 상담을 들어준다. 뭔가 알아내면 연락해준다고 하는데……. 1D6을 굴린다. 홀수라면 두 장면 후에 정보를 준다. 「반응표」를 사용한 캐릭터는 임의의 【비밀】을 하나 획득한다. 짝수라면 조사를 하던 NPC가 수수께끼의 죽임을 당한다. 「반응표」를 사용한 캐릭터는 지식 특기분야에서 무작위로 특기 하나를 선택하여 공포판정을 한다.",
    "「목숨이 아깝다면 이 이상 관여하지 마.」 당신은 인기척이 없는 장소에 끌려가서 얻어맞았다. 협력을 요청받은 인물은 협력을 요청한 당신을 거칠게 거절한다. 「반응표」를 사용한 캐릭터는 【생명력】을 1점 감소한다.",
    "「알겠습니다. 만약을 위해서 순찰을 강화하겠습니다.」 신변 보호를 약속받았다. 「반응표」를 사용한 캐릭터는 이 세션 동안 한 번만 자신이 받은 대미지를 무효로 할 수 있다. 대미지를 무효로 했다면 「반응표」를 사용한 캐릭터는 폭력 특기분야에서 무작위로 특기 하나를 선택하여 공포판정을 한다.",
    "「……이게 뭐야?」 협력을 요청받은 상대가 갑자기 죽는다. 녀석들의 손은 이런 곳까지 닿고 있었단 말인가? 「반응표」를 사용한 캐릭터는 폭력 특기분야에서 무작위로 특기 하나를 선택하여 공포판정을 한다.",
];
static KO_RET: Table = Table::from_dice("반응표", 2, 6, KO_RET_ITEMS);

pub(crate) static KO_TABLES: &[(&str, &dyn RollableTable)] = &[
    ("ST", &KO_ST),
    ("HJST", &KO_HJST),
    ("MTST", &KO_MTST),
    ("DVST", &KO_DVST),
    ("DT", &KO_DT),
    ("BT", &KO_BT),
    ("PT", &KO_PT),
    ("FT", &KO_FT),
    ("JT", &KO_JT),
    ("CHT", &KO_CHT),
    ("VHT", &KO_VHT),
    ("IHT", &KO_IHT),
    ("RHT", &KO_RHT),
    ("MHT", &KO_MHT),
    ("LHT", &KO_LHT),
    ("ECT", &KO_ECT),
    ("EMT", &KO_EMT),
    ("EAT", &KO_EAT),
    ("OHT", &KO_OHT),
    ("OPT", &KO_OPT),
    ("OWT", &KO_OWT),
    ("CNT1", &KO_CNT1),
    ("CNT2", &KO_CNT2),
    ("CNT3", &KO_CNT3),
    ("RET", &KO_RET),
];

static KO_RTT_SKILLS1: &[&str] = &[
    "소각",
    "고문",
    "포박",
    "협박",
    "파괴",
    "구타",
    "절단",
    "찌르기",
    "사격",
    "전쟁",
    "매장",
];
static KO_RTT_SKILLS2: &[&str] = &[
    "연심",
    "기쁨",
    "걱정",
    "부끄러움",
    "웃음",
    "인내",
    "놀람",
    "노여움",
    "원한",
    "슬픔",
    "친애",
];
static KO_RTT_SKILLS3: &[&str] = &[
    "고통",
    "관능",
    "촉감",
    "냄새",
    "맛",
    "소리",
    "풍경",
    "추적",
    "예술",
    "제육감",
    "그늘",
];
static KO_RTT_SKILLS4: &[&str] = &[
    "분해",
    "전자기기",
    "정리",
    "약품",
    "효율",
    "미디어",
    "카메라",
    "탈것",
    "기계",
    "함정",
    "병기",
];
static KO_RTT_SKILLS5: &[&str] = &[
    "물리학",
    "수학",
    "화학",
    "생물학",
    "의학",
    "교양",
    "인류학",
    "역사",
    "민속학",
    "고고학",
    "천문학",
];
static KO_RTT_SKILLS6: &[&str] = &[
    "시간", "혼돈", "심해", "죽음", "영혼", "마술", "암흑", "종말", "꿈", "지저", "우주",
];
pub(crate) static KO_RTT_CATEGORIES: &[SaiFicCategory] = &[
    SaiFicCategory::new("폭력", KO_RTT_SKILLS1),
    SaiFicCategory::new("정서", KO_RTT_SKILLS2),
    SaiFicCategory::new("지각", KO_RTT_SKILLS3),
    SaiFicCategory::new("기술", KO_RTT_SKILLS4),
    SaiFicCategory::new("지식", KO_RTT_SKILLS5),
    SaiFicCategory::new("괴이", KO_RTT_SKILLS6),
];

pub(crate) const KO_BET_NAME: &str = "배드엔드표";
pub(crate) static KO_BET_ITEMS: &[&str] = &[
    "당신의 주위에 칠흑 같은 어둠이, 이형의 앞다리가, 무수한 촉수가 모여든다. 그들은 새로운 동료의 탄생을 축복하고 있다. 당신은 이제 괴이를 두려워하지 않는다. 왜냐하면, 당신 자신이 괴이가 되었기 때문이다. 이후, 당신은 괴이 NPC가 되어 등장한다.",
    "감옥인지 수술실인지 모를 어두운 방에 감금된다. 그리고 매일같이 지독한 고문을 받았다. 어떻게든 틈을 노려서 도망칠 수는 있었지만……. 무작위로 특기 하나를 선택한다. 그 특기가 【공포심】이 된다.",
    "간발의 차이로 수수께끼의 조직에 소속된 에이전트에게 구조된다. 「너는 재능이 있다. 어때? 우리와 함께 일해보지 않겠어?」 당신은 원한다면 닌자/마법사/헌터가 되어 괴이와 싸울 수 있다. 그 경우, 당신은 다른 사이코로 픽션의 캐릭터로 다시 태어난다.",
    "병원의 침대에서 눈을 뜬다. 오랫동안 지독한 악몽을 꾼 것 같은 기분이 든다. 그 세션의 후유증 판정에 -1의 수정을 적용하며, 펌블치가 1 증가한다.",
    "어딘가의 민가에서 눈을 뜬다. 멋진 인물에게 구조되어 후한 간호를 받은 모양이다. 따로 페널티는 없다.",
    "「위험해!」 거대한 절망이 당신을 덮쳐온다. 1D6-1점의 대미지를 입는다. 이것으로 【생명력】이 0점이 되었다면 당신은 사망한다. 단, 당신에게 플러스 【감정】을 가진 NPC가 있다면 그 NPC가 해당 대미지를 무효로 해준다.",
    "다른 새로운 괴사건에 휘말린다. 고생 끝에 그쪽은 어떻게든 무사 해결! 따로 페널티는 없다.",
    "큰 상처를 입고 생사의 갈림길에서 헤맨다. 임의의 특기로 판정한다. 실패하면 사망한다. 이때 감소한 【생명력】만큼 마이너스 수정을 적용한다.",
    "눈을 뜨자 낯선 장소에 있었다. 여기는 어디야? 나는 누구지? 아무래도 공포에 질린 나머지 기억을 잃어버린 모양이다. 공적점이 있다면 그것을 1점 잃는다.",
    "눈을 뜨자, 그곳은 평소 지내던 장소였다. 하지만 어딘지 위화감이 느껴진다. 당신들 이외에는 아무도 사건에 대해 모르는 모양이다. 분명히 죽었던 그 사람도 살아있다. 시간을 여행한 것인가? 아니면 여기는 다른 세계선인가……? 무작위로 특기를 하나 선택한다. 그 특기가 【공포심】이 된다.",
    "뒤를 돌아보니 압도적인 「그것」이 기다리고 있었다. 무자비한 일격이 당신을 덮치고, 당신은 사망한다.",
];

pub(crate) const KO_FUMBLE: &str = "펌블(판정실패。 덱에서 【광기】를 1장 획득)";
pub(crate) const KO_SPECIAL: &str = "스페셜(판정성공。 【생명력】 1점이나 【이성치】 1점 회복)";

const RTTN_ALIASES: &[&str] = &["TVT", "TET", "TPT", "TST", "TKT", "TMT"];

static KO_RTT: SaiFicSkillTable = SaiFicSkillTable::new(KO_RTT_CATEGORIES)
    .with_commands(None, None, RTTN_ALIASES)
    .with_formats(SaiFicFormats {
        rtt: "랜덤 특기 결정표(%<category_dice>d,%<row_dice>d) ＞ %<text>s",
        rct: "랜덤 분야표(%<category_dice>d) ＞ %<category_name>s",
        rttn: "지정특기(%<category_name>s)표(%<row_dice>d) ＞ %<text>s",
        skill: DEFAULT_SKILL_FORMAT,
    });

static KO_SYSTEM: SystemTables = SystemTables {
    tables: KO_TABLES,
    rtt: &KO_RTT,
    bet_name: KO_BET_NAME,
    bet_items: KO_BET_ITEMS,
    irn_name: JA_IRN_NAME,
    fumble: KO_FUMBLE,
    special: KO_SPECIAL,
    success: "성공",
    failure: "실패",
};

/// Ruby `BCDice::GameSystem::Insane_Korean`（ID: `Insane:Korean`）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Insane_Korean;

impl GameSystem for Insane_Korean {
    fn id(&self) -> &'static str {
        "Insane:Korean"
    }

    fn name(&self) -> &'static str {
        "인세인"
    }

    fn sort_key(&self) -> &'static str {
        "国際化:Korean:인세인"
    }

    fn help_message(&self) -> &'static str {
        r"・판정
스페셜／펌블／성공／실패를 판정
・각종표
장면표　　　ST
　사실은 무서운 현대 일본 장면표 HJST／광란의 20년대 장면표 MTST
　빅토리아의 어둠 장면표 DVST
형용표　　　　DT
　본체표 BT／부위표 PT
감정표　　　　　　FT
직업표　　　　　　JT
배드엔드표　　BET
랜덤 특기 결정표　RTT
지정특기(폭력)표　　(TVT)
지정특기(정서)표　　(TET)
지정특기(지각)표　　(TPT)
지정특기(기술)표　　(TST)
지정특기(지식)표　　(TKT)
지정특기(괴이)표　　(TMT)
회화 중에 생겨나는 공포표(CHT)
거리에서 마주치는 공포표(VHT)
갑자기 찾아오는 공포표(IHT)
폐허에서 마주치는 공포표(RHT)
야외에서 마주치는 공포표(MHT)
정보 속에 숨어있는 공포표(LHT)
조우표　도시　(ECT)　산림　(EMT)　해변　(EAT)/반응표　RET
야근 호러 스케이프　OHT/야근 전화표　OPT/야근 장면표　OWT
회사명 결정표1　CNT1/회사명 결정표2　CNT2/회사명 결정표3　CNT3
・D66 다이스 있음.
"
    }

    fn prefixes(&self) -> &'static [&'static str] {
        &[
            "BET",
            "IRN",
            "RTT[1-6]?",
            "RCT",
            "TVT",
            "TET",
            "TPT",
            "TST",
            "TKT",
            "TMT",
            "ST",
            "HJST",
            "MTST",
            "DVST",
            "DT",
            "BT",
            "PT",
            "FT",
            "JT",
            "CHT",
            "VHT",
            "IHT",
            "RHT",
            "MHT",
            "LHT",
            "ECT",
            "EMT",
            "EAT",
            "OHT",
            "OPT",
            "OWT",
            "CNT1",
            "CNT2",
            "CNT3",
            "RET",
        ]
    }

    crate::impl_prefixes_pattern!();

    fn sort_add_dice(&self) -> bool {
        true
    }

    fn sort_barabara_dice(&self) -> bool {
        true
    }

    fn d66_sort_type(&self) -> D66SortType {
        D66SortType::Asc
    }

    fn result_2d6(
        &self,
        total: crate::Int,
        dice_total: i64,
        _value_list: &[i64],
        cmp_op: CmpOp,
        target: Target,
    ) -> Option<CheckOutcome> {
        check_result_2d6(&KO_SYSTEM, total, dice_total, cmp_op, target)
    }

    fn eval_game_system_specific_command(
        &self,
        command: &str,
        rng: &mut Randomizer,
    ) -> Result<Option<SpecificCommandOutput>, EvalError> {
        eval_specific_command(&KO_SYSTEM, command, rng)
    }
}

#[cfg(test)]
mod tests {
    use std::path::{Path, PathBuf};

    use crate::eval::eval_command;
    use crate::game_system::GameSystemId;
    use crate::randomizer::SeededRandomizer;
    use crate::toml_test::TestDataFile;

    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/Insane_Korean.toml");
        path.exists().then_some(path)
    }

    fn check_flag(reasons: &mut Vec<String>, name: &str, expected: bool, actual: bool) {
        if expected != actual {
            reasons.push(format!(
                "{name} flag mismatch: expected {expected}, actual {actual}"
            ));
        }
    }

    /// `test/data/Insane_Korean.toml` の全ケースが通ること。
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/Insane_Korean.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Insane_Korean.toml must parse");
        assert_eq!(
            data.tests.len(),
            57,
            "case count in test/data/Insane_Korean.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Insane:Korean",
                "unexpected game system in Insane_Korean.toml"
            );

            let mut reasons: Vec<String> = Vec::new();
            let rands: Vec<(i64, i64)> = tc.rands.iter().map(|r| (r.value, r.sides)).collect();
            let mut src = SeededRandomizer::new(rands);

            match eval_command(&GameSystemId::new("Insane:Korean"), &tc.input, &mut src) {
                Err(e) => reasons.push(format!("eval error: {e}")),
                Ok(None) => {
                    if !tc.expects_nil() {
                        reasons.push(format!(
                            "eval returned nil, but output was expected: {:?}",
                            tc.output
                        ));
                    }
                }
                Ok(Some(result)) => {
                    if tc.expects_nil() {
                        reasons.push(format!("expected nil output, got {:?}", result.text));
                    } else if result.text != tc.output {
                        reasons.push(format!(
                            "output mismatch\n    expected: {:?}\n    actual:   {:?}",
                            tc.output, result.text
                        ));
                    }
                    check_flag(&mut reasons, "secret", tc.secret, result.secret);
                    check_flag(&mut reasons, "success", tc.success, result.success);
                    check_flag(&mut reasons, "failure", tc.failure, result.failure);
                    check_flag(&mut reasons, "critical", tc.critical, result.critical);
                    check_flag(&mut reasons, "fumble", tc.fumble, result.fumble);
                }
            }

            if !src.is_empty() {
                reasons.push(format!("unconsumed rands remain ({})", src.remaining()));
            }

            if !reasons.is_empty() {
                failures.push(format!(
                    "FAIL Insane_Korean:{}:{}\n  - {}",
                    i + 1,
                    tc.input,
                    reasons.join("\n  - ")
                ));
            }
        }

        assert!(
            failures.is_empty(),
            "{}/{} Insane_Korean cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
    }
}
