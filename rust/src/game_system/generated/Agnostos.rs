...
mod tests {
    ...
    /// `test/data/Agnostos.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Agnostos.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Agnostos.toml must parse");
        assert_eq!(
            data.tests.len(),
            41,
            "case count in test/data/Agnostos.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Agnostos",
                "unexpected game system in Agnostos.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Agnostos"), &tc.input, &mut src) {
