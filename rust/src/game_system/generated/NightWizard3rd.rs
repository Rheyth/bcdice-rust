...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/NightWizard3rd.toml");
    ...
    /// `test/data/NightWizard3rd.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/NightWizard3rd.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("NightWizard3rd.toml must parse");
        assert_eq!(
            data.tests.len(),
            3,
            "case count in test/data/NightWizard3rd.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "NightWizard3rd",
                "unexpected game system in NightWizard3rd.toml"
            );
            ...
            match eval_command(&GameSystemId::new("NightWizard3rd"), &tc.input, &mut src) {
