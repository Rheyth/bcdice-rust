...
mod tests {
    ...
    /// `test/data/BeginningIdol.toml` の全ケースがパスすること。
    #[test]
    fn all_toml_cases_pass() {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .unwrap()
            .join("test/data/BeginningIdol.toml");
        if !path.exists() {
            // worktree外でクレート単体ビルドされた場合
            eprintln!("skip: test/data/BeginningIdol.toml not found");
            return;
        }

        let data = TestDataFile::load(&path).expect("BeginningIdol.toml must parse");
        assert_eq!(
            data.tests.len(),
            201,
            "case count in test/data/BeginningIdol.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "BeginningIdol",
                "unexpected game system in BeginningIdol.toml"
            );
            ...
            match eval_command(&GameSystemId::new("BeginningIdol"), &tc.input, &mut src) {
