...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/HouraiGakuen.toml");
    ...
    /// `test/data/HouraiGakuen.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/HouraiGakuen.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("HouraiGakuen.toml must parse");
        assert_eq!(
            data.tests.len(),
            47,
            "case count in test/data/HouraiGakuen.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "HouraiGakuen",
                "unexpected game system in HouraiGakuen.toml"
            );
            ...
            match eval_command(&GameSystemId::new("HouraiGakuen"), &tc.input, &mut src) {
