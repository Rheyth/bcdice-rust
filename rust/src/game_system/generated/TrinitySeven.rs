...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/TrinitySeven.toml");
    ...
    /// `test/data/TrinitySeven.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/TrinitySeven.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("TrinitySeven.toml must parse");
        assert_eq!(
            data.tests.len(),
            57,
            "case count in test/data/TrinitySeven.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "TrinitySeven",
                "unexpected game system in TrinitySeven.toml"
            );
            ...
            match eval_command(&GameSystemId::new("TrinitySeven"), &tc.input, &mut src) {
