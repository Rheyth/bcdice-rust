...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/NightWizard.toml");
    ...
    /// `test/data/NightWizard.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/NightWizard.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("NightWizard.toml must parse");
        assert_eq!(
            data.tests.len(),
            102,
            "case count in test/data/NightWizard.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "NightWizard",
                "unexpected game system in NightWizard.toml"
            );
            ...
            match eval_command(&GameSystemId::new("NightWizard"), &tc.input, &mut src) {
