...
mod tests {
    ...
    /// `test/data/Paradiso.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Paradiso.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Paradiso.toml must parse");
        assert_eq!(
            data.tests.len(),
            12,
            "case count in test/data/Paradiso.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Paradiso",
                "unexpected game system in Paradiso.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Paradiso"), &tc.input, &mut src) {
