...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/MetalHeadExtream.toml");
    ...
    /// `test/data/MetalHeadExtream.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/MetalHeadExtream.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("MetalHeadExtream.toml must parse");
        assert_eq!(
            data.tests.len(),
            106,
            "case count in test/data/MetalHeadExtream.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "MetalHeadExtream",
                "unexpected game system in MetalHeadExtream.toml"
            );
            ...
            match eval_command(&GameSystemId::new("MetalHeadExtream"), &tc.input, &mut src) {
