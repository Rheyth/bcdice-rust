...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/EtrianOdysseySRS.toml");
    ...
    /// `test/data/EtrianOdysseySRS.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/EtrianOdysseySRS.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("EtrianOdysseySRS.toml must parse");
        assert_eq!(
            data.tests.len(),
            33,
            "case count in test/data/EtrianOdysseySRS.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "EtrianOdysseySRS",
                "unexpected game system in EtrianOdysseySRS.toml"
            );
            ...
            match eval_command(&GameSystemId::new("EtrianOdysseySRS"), &tc.input, &mut src) {
