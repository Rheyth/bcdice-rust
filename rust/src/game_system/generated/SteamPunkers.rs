//! P4で手書き移植した `lib/bcdice/game_system/SteamPunkers.rb`。
//!
//! メタデータ（id/name/sort_key/help_message/prefixes/settings）は
//! `rust/tools/generate_game_systems.rb` が生成したスタブの値をそのまま保っている。
//!
//! 移植したもの:
//! - `SteamPunkers#eval_game_system_specific_command`
//!   （`#roll_tables(TABLES)` → `#roll_sp`）
//! - `SteamPunkers#roll_sp`（判定コマンド `SPn>=t`）
//! - `SPTable`（`DiceTable::D66RangeTable` 派生。RANGE 8区分は固定）
//! - `TABLES`（PAT〜OFT の31表）
...
/// Ruby `BCDice::GameSystem::SteamPunkers`（ID: `SteamPunkers`）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct SteamPunkers;

impl GameSystem for SteamPunkers {
...
"
    ...
    fn prefixes(&self) -> &'static [&'static str] {
        &[
            "SP", "PAT", "PST", "PCT", "NIT", "NAT", "NFT", "NGT", "NST", "NJT", "PTT", "PGT",
            "PNT", "PDT", "PET", "PACT", "BGT", "BAT", "BET", "BCT", "BST", "BWT", "FST", "FPT",
            "RST", "RPT", "EST", "EPT", "ODT", "OST", "OIT", "OFT",
        ]
    }

    crate::impl_prefixes_pattern!();

    /// Ruby `SteamPunkers#eval_game_system_specific_command`。
    fn eval_game_system_specific_command(
        &self,
        command: &str,
        rng: &mut Randomizer,
    ) -> Result<Option<SpecificCommandOutput>, EvalError> {
        // Ruby: result = roll_tables(command, TABLES); return result if result
        ...
        if let Some(r) = roll_sp(command, rng)? {
...
/// Ruby `SteamPunkers::SPTable < DiceTable::D66RangeTable`。
///
/// `RANGE = [11..14, 15..22, 23..32, 33..36, 41..44, 45..52, 53..56, 61..66]`
/// は固定なので、8要素の項目スライスだけを持つ。
struct SPTable {
    name: &'static str,
...
impl SPTable {
    /// Ruby `SPTable.new(name, items)`。
    ///
    /// Ruby は `items.size != RANGE.size` で例外を投げるが、
    /// ここではすべての表を8要素の配列として静的に定義する。
    const fn new(name: &'static str, items: &'static [&'static str]) -> Self {
        Self { name, items }
    }
}

/// Ruby `SPTable::RANGE`。
const SP_RANGES: [RangeInc; 8] = [
    RangeInc::new(11, 14),
    RangeInc::new(15, 22),
    RangeInc::new(23, 32),
    RangeInc::new(33, 36),
...
impl RollableTable for SPTable {
    /// Ruby `D66RangeTable#roll(randomizer)`。
...
static PAT: SPTable = SPTable::new("年齢表", &PAT_ITEMS);

static PST_ITEMS: [&str; 8] = [
    "【男性】 背が高く、がっしりしている。",
    "【女性】 蠱惑的な身体をしている。",
    "【男性】 平均的な中肉中背である。",
    "【女性】 女性らしい体形をしている。",
...
static PST: SPTable = SPTable::new("性別表", &PST_ITEMS);

static PCT_ITEMS: [&str; 8] = [
    "【日本皇国】 あなたは日本人である。",
...
static PCT: SPTable = SPTable::new("国籍表", &PCT_ITEMS);

static NIT_ITEMS: [&str; 8] = [
    "アーロン/アビゲイル/アダム",
    "カラム/オーレリア/バーナード",
    "クライヴ/ブレンダ/ビーチャム",
    "カーティス/キャロライン/ビリンガム",
...
static NIT: SPTable = SPTable::new("名前表・イギリス(男性名/女性名/名字)", &NIT_ITEMS);

static NAT_ITEMS: [&str; 8] = [
    "ジェラルド/デイジー/キャンベル",
    "クラーク/ベアトリス/ベックフォード",
    "ヴィクター/ケイト/ガードナー",
    "ザック/ルシール/キンケイド",
...
static NAT: SPTable = SPTable::new("名前表・アメリカ(男性名/女性名/名字)", &NAT_ITEMS);

static NFT_ITEMS: [&str; 8] = [
    "ベルナール/ベアトリス/バシュラール",
    "セドリック/セシリア/ベルモンド",
    "エルネスト/コレット/ボニツェール",
    "フレデリック/エルザ/ボワイエ",
...
static NFT: SPTable = SPTable::new("名前表・フランス(男性名/女性名/名字)", &NFT_ITEMS);

static NGT_ITEMS: [&str; 8] = [
    "アルベルト/アデーレ/アッカーマン",
    "バシリウス/アルマ/バルツァー",
    "ブルーノ/クリステル/ビットナー",
    "エトムント/エルネスタ/コルネリウス",
...
static NGT: SPTable = SPTable::new("名前表・ドイツ(男性名/女性名/名字)", &NGT_ITEMS);

static NST_ITEMS: [&str; 8] = [
    "イリイチ/バルバラ/バビチェフ",
    "ミハイル/ジナイーダ/ボブロフ",
    "セルゲイ/クラーラ/ヴォルコフ",
    "イヴァン/レイラ/ガボエフ",
...
static NST: SPTable = SPTable::new("名前表・ソビエト(男性名/女性名/名字)", &NST_ITEMS);

static NJT_ITEMS: [&str; 8] = [
    "タキジ/タキ/トウドウ",
    "シンヤ/アヤ/マツモト",
    "レイジ/カオルコ/サガラ",
    "コウタロウ/セン/オオシマ",
...
static NJT: SPTable = SPTable::new("名前表・日本(男性名/女性名/名字)", &NJT_ITEMS);

static PTT_ITEMS: [&str; 8] = [
...
static PTT: SPTable = SPTable::new("過去表『トゥルース』", &PTT_ITEMS);

static PGT_ITEMS: [&str; 8] = [
    "【後悔】 ある蒸気事件を解決できなかった。結果、あなたは大切な人を失ってしまった。",
...
static PGT: SPTable = SPTable::new("過去表『ガーディアン』", &PGT_ITEMS);

static PNT_ITEMS: [&str; 8] = [
...
static PNT: SPTable = SPTable::new("過去表『ノーブル』", &PNT_ITEMS);

static PDT_ITEMS: [&str; 8] = [
...
static PDT: SPTable = SPTable::new("過去表『デヴォート』", &PDT_ITEMS);

static PET_ITEMS: [&str; 8] = [
...
static PET: SPTable = SPTable::new("過去表『エセティック』", &PET_ITEMS);

static PACT_ITEMS: [&str; 8] = [
    "【誓い】 とある陰謀により、あなたは大切な何かを失った。いつか陰謀を暴く……それがあなたの誓いだ。",
    "【研究】 あなたは蒸気犯罪で用いられるスチームギアを撲滅するために、常に研究を続けている。",
...
static PACT: SPTable = SPTable::new("過去表『チャレンジ』", &PACT_ITEMS);

static BGT_ITEMS: [&str; 8] = [
...
static BGT: SPTable = SPTable::new("経緯表『ギガントアームズ』", &BGT_ITEMS);

static BAT_ITEMS: [&str; 8] = [
...
static BAT: SPTable = SPTable::new("経緯表『アーマードリム』", &BAT_ITEMS);

static BET_ITEMS: [&str; 8] = [
...
static BET: SPTable = SPTable::new("経緯表『エッジモービル』", &BET_ITEMS);

static BCT_ITEMS: [&str; 8] = [
...
static BCT: SPTable = SPTable::new("経緯表『クロノウェポン』", &BCT_ITEMS);

static BST_ITEMS: [&str; 8] = [
...
static BST: SPTable = SPTable::new("経緯表『スパイテック』", &BST_ITEMS);

static BWT_ITEMS: [&str; 8] = [
...
static BWT: SPTable = SPTable::new("経緯表『スチームウェア』", &BWT_ITEMS);

static FST_ITEMS: [&str; 8] = [
...
static FST: SPTable = SPTable::new("特徴表S", &FST_ITEMS);

static FPT_ITEMS: [&str; 8] = [
    "【傷】 顔、あるいは体のどこかに傷がある。",
...
static FPT: SPTable = SPTable::new("特徴表P", &FPT_ITEMS);

static RST_ITEMS: [&str; 8] = [
...
static RST: SPTable = SPTable::new("関係性表S", &RST_ITEMS);

static RPT_ITEMS: [&str; 8] = [
...
static RPT: SPTable = SPTable::new("関係性表P", &RPT_ITEMS);

static EST_ITEMS: [&str; 8] = [
...
static EST: SPTable = SPTable::new("感情表S", &EST_ITEMS);

static EPT_ITEMS: [&str; 8] = [
...
static EPT: SPTable = SPTable::new("感情表P", &EPT_ITEMS);

static ODT_ITEMS: [&str; 8] = [
...
static ODT: SPTable = SPTable::new("災厄表", &ODT_ITEMS);

static OST_ITEMS: [&str; 8] = [
...
static OST: SPTable = SPTable::new("場面表", &OST_ITEMS);

static OIT_ITEMS: [&str; 8] = [
...
static OIT: SPTable = SPTable::new("交流表", &OIT_ITEMS);

static OFT_ITEMS: [&str; 8] = [
...
static OFT: SPTable = SPTable::new("激憤表", &OFT_ITEMS);

static TABLES: &[(&str, &SPTable)] = &[
    ("PAT", &PAT),
    ("PST", &PST),
    ("PCT", &PCT),
    ("NIT", &NIT),
...
/// Ruby `SteamPunkers#roll_sp`（判定コマンド `SPn>=t`）。
fn roll_sp(command: &str, rng: &mut Randomizer) -> Result<Option<EvalResult>, EvalError> {
    // Ruby: /^SP(\d+)(?:>=(\d+))?$/i
    static RE: OnceLock<Regex> = OnceLock::new();
    let re = RE.get_or_init(|| Regex::new(r"(?i)\ASP(\d+)(?:>=(\d+))?\z").expect("valid regex"));
    let Some(m) = re.captures(command) else {
        return Ok(None);
    };

    let dice_count: i64 = m[1].parse().unwrap_or(i64::MAX);
    let target_number: Option<i64> = m.get(2).and_then(|x| x.as_str().parse().ok());
    ...
    let successes = dice_list.iter().filter(|&&x| x == 6).count() as i64 * 2
        + dice_list.iter().filter(|&&x| x == 5).count() as i64;
    let failures = dice_list.iter().filter(|&&x| x <= 4).count() as i64;

    let mut result = if dice_list.iter().all(|&x| x == 1) {
        ...
        // Ruby: Result.new（textはnil。compactで連結対象から外れる）
        EvalResult::new()
    };

    // Ruby: ["(#{command})", "[#{dice_list_text}]", "成功数:...", result.text].compact.join(" ＞ ")
    let mut parts = vec![
        format!("({command})"),
        format!("[{dice_list_text}]"),
        format!("成功数:{successes}, 失敗数:{failures}"),
    ];
    ...
    // Ruby: result.critical = dice_list.include?(6)
...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../test/data/SteamPunkers.toml");
        path.exists().then_some(path)
    }

    /// `test/data/SteamPunkers.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/SteamPunkers.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("SteamPunkers.toml must parse");
        assert_eq!(
            data.tests.len(),
            71,
            "case count in test/data/SteamPunkers.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "SteamPunkers",
                "unexpected game system in SteamPunkers.toml"
            );
            ...
            match eval_command(&GameSystemId::new("SteamPunkers"), &tc.input, &mut src) {
