...
impl GameSystem for GURPS {
    ...
    fn result_nd6(
        ...
        if values.len() != 3 || cmp_op != CmpOp::Le {
            return None;
        }
        Some(CheckOutcome::Result(Box::new(check_result(
            total,
            dice_total,
...
static HELP_MESSAGE: &str = r"・判定においてクリティカル・ファンブルの自動判別、成功度の自動計算。(3d6<=目標値／目標値-3d6)
...
fn reverse_pattern() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r"^([\d+-]+)-3D6?([\d+-]*)$").expect("valid regex"))
}
fn fear_pattern() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r"^FEAR(\+?\d+)?$").expect("valid regex"))
}
fn react_pattern() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r"^REACT([+-]?\d*)$").expect("valid regex"))
}

fn check_result(total: crate::Int, dice_total: i64, target: i64) -> EvalResult {
    let degree = target - total.clone();
    let text = if (dice_total <= 6 && target >= 16)
        || (dice_total <= 5 && target >= 15)
        || dice_total <= 4
    {
        return EvalResult::critical(format!("クリティカル(成功度：{degree})"));
    } else if target - dice_total <= -10 || (dice_total >= 17 && target <= 15) || dice_total >= 18 {
        return EvalResult::fumble(format!("ファンブル(失敗度：{degree})"));
    } else if dice_total >= 17 {
        format!("自動失敗(失敗度：{degree})")
    } else if total <= crate::Int::from(target) {
        return EvalResult::success(format!("成功(成功度：{degree})"));
    } else {
        format!("失敗(失敗度：{degree})")
    };
...
fn eval_specific(
    ...
    if let Some(m) = reverse_pattern().captures(command) {
        let target = arithmetic::eval(&m[1], RoundType::Floor)?
            .as_ref()
            .map(crate::randomizer::sat_i64)
            .unwrap_or(0);
        let modifier = arithmetic::eval(&m[2], RoundType::Floor)?
            .as_ref()
            .map(crate::randomizer::sat_i64)
            .unwrap_or(0);
        let dice = rng.roll_barabara(3, 6)?;
        let dice_total: i64 = dice.iter().sum();
        let total = dice_total + modifier;
        let mut result = check_result(crate::Int::from(total), dice_total, target);
        result.text = format!(
            "(3D6{}<={target}) ＞ {dice_total}[{}] ＞ {total} ＞ {}",
    ...
    if let Some(m) = fear_pattern().captures(command) {
        let modifier = m.get(1).and_then(|v| v.as_str().parse().ok()).unwrap_or(0);
        let number = rng.roll_sum(3, 6)? + modifier;
        let index = if number > 40 { 36 } else { number - 4 };
        return Ok(Some(SpecificCommandOutput::text(format!(
            "恐怖表({number}) ＞ {}",
            FEAR[index as usize]
        ))));
    }
    if let Some(m) = react_pattern().captures(command) {
        let modifier = m.get(1).and_then(|v| v.as_str().parse().ok()).unwrap_or(0);
        let number = rng.roll_sum(3, 6)? + modifier;
        let reaction = match number {
            i64::MIN..=0 => "最悪",
            1..=3 => "とても悪い",
            4..=6 => "悪い",
            7..=9 => "良くない",
...
fn table(command: &str) -> Option<(&'static str, &'static [&'static str])> {
    match command {
        "CRT" => Some(("クリティカル表", CRT)),
        "HCRT" => Some(("頭部打撃クリティカル表", HCRT)),
        "FMB" => Some(("ファンブル表", FMB)),
        "MFMB" => Some(("呪文ファンブル表", MFMB)),
        "YSCRT" => Some(("妖魔夜行スペシャルクリティカル表", YSCRT)),
        "YSFMB" => Some(("妖魔夜行スペシャルファンブル表", YSFMB)),
        "YFMB" => Some(("妖術ファンブル表", YFMB)),
        "HIT" => Some(("命中部位表", HIT)),
        _ => None,
    }
}
