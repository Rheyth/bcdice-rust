...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/ArknightsFan.toml");
    ...
    /// `test/data/ArknightsFan.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/ArknightsFan.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("ArknightsFan.toml must parse");
        assert_eq!(
            data.tests.len(),
            95,
            "case count in test/data/ArknightsFan.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "ArknightsFan",
                "unexpected game system in ArknightsFan.toml"
            );
            ...
            match eval_command(&GameSystemId::new("ArknightsFan"), &tc.input, &mut src) {
