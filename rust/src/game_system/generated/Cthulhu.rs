...
mod tests {
    ...
    /// `test/data/Cthulhu.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/Cthulhu.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Cthulhu.toml must parse");
        assert_eq!(
            data.tests.len(),
            105,
            "case count in test/data/Cthulhu.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Cthulhu",
                "unexpected game system in Cthulhu.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Cthulhu"), &tc.input, &mut src) {
