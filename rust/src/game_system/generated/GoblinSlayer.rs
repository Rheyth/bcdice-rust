...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/GoblinSlayer.toml");
    ...
    /// `test/data/GoblinSlayer.toml` の全ケースが通ること。
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/GoblinSlayer.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("GoblinSlayer.toml must parse");
        assert_eq!(
            data.tests.len(),
            60,
            "case count in test/data/GoblinSlayer.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "GoblinSlayer",
                "unexpected game system in GoblinSlayer.toml"
            );
            ...
            match eval_command(&GameSystemId::new("GoblinSlayer"), &tc.input, &mut src) {
