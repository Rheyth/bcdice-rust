...
mod tests {
    ...
    /// `test/data/Alsetto.toml` の全ケースが通ること。
    #[test]
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            eprintln!("skip: test/data/Alsetto.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("Alsetto.toml must parse");
        assert_eq!(data.tests.len(), 25, "case count in test/data/Alsetto.toml");

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "Alsetto",
                "unexpected game system in Alsetto.toml"
            );
            ...
            match eval_command(&GameSystemId::new("Alsetto"), &tc.input, &mut src) {
