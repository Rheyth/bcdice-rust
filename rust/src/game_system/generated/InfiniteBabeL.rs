//! P4で手書き移植した `lib/bcdice/game_system/InfiniteBabeL.rb`。
//!
//! メタデータ（id/name/sort_key/help_message/prefixes/settings）は
//! `rust/tools/generate_game_systems.rb` が生成したスタブの値をそのまま保っている。
//! 生成スクリプトを再実行するとこのファイルはスタブへ戻るので注意。
//!
//! Rubyでは `InfiniteBabeL < ShoujoTenrankai`（出来事表ロジックを継承）だが、
//! Rustには継承がないため `super::` のヘルパ関数を再利用する。
//!
//! 移植したもの:
//! - `InfiniteBabeL` の `EVENT_TABLES`（27表・親の eval をそのまま利用）

use super::ShoujoTenrankai::{get_event_table_result, get_tension_text, EventTable};
use crate::eval::EvalError;
use crate::game_system::{GameSystem, SpecificCommandOutput};
use crate::randomizer::Randomizer;

/// Ruby `BCDice::GameSystem::InfiniteBabeL`（ID: `InfiniteBabeL`）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct InfiniteBabeL;

impl GameSystem for InfiniteBabeL {
    ...
    fn sort_key(&self) -> &'static str {
        "しようねんのためのしようしよてんらんかいいふんいんふいにつとはへる"
    }
...
"
    ...
    /// Ruby `ShoujoTenrankai#eval_game_system_specific_command` を継承。
...
/// Ruby `InfiniteBabeL::EVENT_TABLES`。
static EVENT_TABLES: &[(&str, EventTable)] = &[
    ("HOT", EventTable { name: "暑い日", table: &[
    ...
    ("COLD", EventTable { name: "寒い日", table: &[
    ...
    ("MIDNIGHT", EventTable { name: "夜明け", table: &[
    ...
    ("ALLEY", EventTable { name: "路地", table: &[
        "【奇・1】窓が開け放たれている。",
        "【奇・2】パタン、と扉の閉まる音がした。",
    ...
    ("SKY", EventTable { name: "空が見える場所", table: &[
    ...
    ("BOTANY", EventTable { name: "植物がある場所", table: &[
    ...
    ("WAHER", EventTable { name: "水がある場所", table: &[
        "【奇・1】水に何かが浮いている。",
    ...
    ("DIM", EventTable { name: "薄暗い場所", table: &[
    ...
    ("FIRE", EventTable { name: "火がある場所", table: &[
        "【奇・1】ぱちり、と火の粉が爆ぜた。",
        "【奇・2】あったけー。（パトス +1）",
    ...
    ("LABO", EventTable { name: "ラボトラリー", table: &[
    ...
    ("CHIMNEY", EventTable { name: "煙突街", table: &[
    ...
    ("LIBRARY", EventTable { name: "ライブラリー", table: &[
    ...
    ("SLUM", EventTable { name: "スラム", table: &[
        "【奇・1】口笛を吹かれた。（エロス +1）",
        "【奇・2】下品な笑い声が響き渡っている。",
    ...
    ("JUNKYARD", EventTable { name: "ジャンクヤード", table: &[
    ...
    ("IROMACHI", EventTable { name: "イロマチ", table: &[
        "【奇・1】物憂げな楽の音が聞こえてくる。",
    ...
    ("MONASTERY", EventTable { name: "修道院", table: &[
        "【奇・1】水鏡が静かに景色を映している。",
        "【奇・2】足音が響いた。",
    ...
    ("ROSE", EventTable { name: "ローズガーデン", table: &[
...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path =
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../test/data/InfiniteBabeL.toml");
        path.exists().then_some(path)
    }

    /// `test/data/InfiniteBabeL.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/InfiniteBabeL.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("InfiniteBabeL.toml must parse");
        assert_eq!(
            data.tests.len(),
            81,
            "case count in test/data/InfiniteBabeL.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "InfiniteBabeL",
                "unexpected game system in InfiniteBabeL.toml"
            );
            ...
            match eval_command(&GameSystemId::new("InfiniteBabeL"), &tc.input, &mut src) {
