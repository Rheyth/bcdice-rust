...
mod tests {
    ...
    fn toml_path() -> Option<PathBuf> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("test/data/StratoShout_Korean.toml");
    ...
    /// `test/data/StratoShout_Korean.toml` の全ケースが通ること。
    ///
    ...
    fn all_toml_cases_pass() {
        let Some(path) = toml_path() else {
            ...
            eprintln!("skip: test/data/StratoShout_Korean.toml not found");
            return;
        };

        let data = TestDataFile::load(&path).expect("StratoShout_Korean.toml must parse");
        assert_eq!(
            data.tests.len(),
            23,
            "case count in test/data/StratoShout_Korean.toml"
        );

        let mut failures: Vec<String> = Vec::new();
        for (i, tc) in data.tests.iter().enumerate() {
            assert_eq!(
                tc.game_system, "StratoShout:Korean",
                "unexpected game system in StratoShout_Korean.toml"
            );
            ...
            match eval_command(
                &GameSystemId::new("StratoShout:Korean"),
                &tc.input,
                &mut src,
            ) {
        ...
        assert!(
            failures.is_empty(),
            "{}/{} StratoShout_Korean cases failed:\n{}",
            failures.len(),
            data.tests.len(),
            failures.join("\n")
        );
